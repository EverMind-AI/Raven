...
class ToolOffloadMiddleware(MiddlewareBase):  # pylint: disable=abstract-method
    ...
    async def on_acting(  # type:ignore[override]
        ...
        # Tools may override the noun used in offload notifications
        # (CLI sub-agents say "Sub-Agent"); everything else says "Tool".
        tool_noun = getattr(tool, "offload_noun", "Tool")
        ...
        async def _deliver_when_done() -> None:
            ...
            hint_source = json.dumps(
                {
                    # Tools may override the delivered-result hint label
                    # (e.g. CLI sub-agents surface as "Sub-Agent Response");
                    # everything else falls back to the generic label.
                    "label": getattr(
                        tool,
                        "offload_hint_label",
                        "tool_output",
            ...
            if response is None or len(response.content) == 0:
                hint = HintBlock(
                    hint=(
                        ...
                        f"{tool_noun} '{tool_name}' running in background "
        ...
        placeholder_text = f"""<system-reminder>{tool_noun} '{tool_name}' is \

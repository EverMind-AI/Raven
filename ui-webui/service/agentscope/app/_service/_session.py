...
from ._run_state_replay import purge_run_state
...
class SessionService:
    ...
    async def delete_session(
        ...
        3. Purge transient bus state for ``session_id`` (events log,
           inbox).
        4. Purge durable run-state projections for ``session_id`` — the
           ``dag_run`` and ``subagent_instance`` feeds (design §1d) —
           so a stale DAG or instance card can't resurface on a stream
           reconnect after the session is gone.
        ...
        await purge_run_state(self._projection, session_id)
        return deleted

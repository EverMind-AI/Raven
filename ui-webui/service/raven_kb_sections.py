...
def section_key(chunk: Chunk) -> tuple:
    ...
    Chunks indexed before the structured parser have no heading path; they all
    answer to one key, which is correct -- they really did come from a single
    unstructured section.
    """
    path = chunk.metadata.get("heading_path")
    return tuple(str(part) for part in path) if isinstance(path, list) and path else ()

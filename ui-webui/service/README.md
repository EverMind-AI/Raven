...
The `subagent-dag-orchestration` skill — which guides the agent to prefer the
`run_subagent_dag` tool, orchestrating sub-agents as a single DAG with
file-based message passing, whenever a task spans two or more sub-agents — now
ships as a Raven built-in skill
([`raven/memory_engine/skills/subagent-dag-orchestration/`](../../raven/memory_engine/skills/subagent-dag-orchestration/SKILL.md)),
loaded by the Raven skill registry. This service no longer seeds a copy of it
into session workspaces. To give the AgentScope leader agent extra guidance,
pass skill directories to `LocalWorkspaceManager(skill_paths=[...])` in
`main.py`.

...
from raven_kb_sections import MAX_SECTION_CHARS, SECTION_ORDINAL, section_key
...
def test_the_structured_parser_only_claims_what_it_can_cut():
    """`create_app(knowledge_parsers=...)` replaces the default list rather than
    adding to it, so registering this parser alone would leave plain text, CSV,
    JSON, YAML and RST with no parser at all. It claims two media types; the
    service has to keep TextParser alongside it for the rest."""

	...
	return (
		<div className="flex h-full w-full flex-col items-center justify-center gap-6 p-8 text-center">
			<div role="alert" className="flex flex-col items-center gap-2">
				<div className="em-kicker">{t('error.kicker')}</div>
				<h1 className="text-2xl font-medium">
					{t('error.titleLead')}
					<span className="em-accent">{t('error.titleAccent')}</span>
				</h1>
				<p className="text-muted-foreground text-sm">{t('error.description')}</p>
			</div>
			{detail && (
				<pre className="text-muted-foreground bg-muted border-border max-w-md overflow-auto rounded-xl border p-4 text-left text-xs whitespace-pre-wrap">
					{detail}
				</pre>
			)}

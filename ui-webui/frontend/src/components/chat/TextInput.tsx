...
import Paperclip from '~icons/solar/paperclip-bold-duotone';
import Send from '~icons/solar/plain-2-bold-duotone';
	...
	(
		...
		return (
			<div
				id="tour-chat-input"
				className={cn(
					'flex flex-col gap-2 rounded-none px-3 pt-1 pb-3',
					className,
				)}
			...
			>
				...
				{/* Input area */}
				<div className="relative rounded-xl border border-input bg-card px-2 py-1.5 focus-within:border-gold/50">
					<div className="relative">
						...
						<textarea
							...
							disabled={disabled}
							rows={1}
							className="w-full resize-none rounded-md border-0 bg-transparent px-3 py-2 text-sm outline-none transition-[min-height] duration-200 ease-out placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
							style={{
								minHeight: expanded ? 'calc(6em + 4rem)' : 'calc(1.5em + 1rem)',
								maxHeight: 'calc(1.5em * 12)',
								lineHeight: '1.5em',
								overflowY: 'auto',
							}}
							autoFocus={true}
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										...
										size="icon"
										className="icon-mono shrink-0 rounded-full bg-gold text-gold-foreground hover:bg-gold/90"
									>

...
import MoreHorizontal from '~icons/solar/menu-dots-linear';
import Minus from '~icons/solar/minus-square-linear';
...
function renderGutter({ change, side }: GutterOptions) {
	...
	if (change.type === 'insert') {
		marker = <Plus className="size-2.5 text-gold-olive" />;
	} else if (change.type === 'delete') {
		marker = <Minus className="size-2.5 text-destructive" />;
	}
...
export function DiffPreview({ unifiedDiff }: DiffPreviewProps) {
	...
	if (!diffFile || diffFile.hunks.length === 0) {
		return <div className="text-xs text-muted-foreground">{t('tool.diff.noChanges')}</div>;
	}

...
import Ban from '~icons/solar/forbidden-circle-bold-duotone';
...
export function ToolStateIcon({ state }: { state: ToolResultBlock['state'] | undefined }) {
	if (state === 'success') {
		return <Check className="size-3 text-gold-olive shrink-0" />;
	}
	if (state === 'error') {
...
export function DiffStats({ insertions, deletions }: { insertions: number; deletions: number }) {
	return (
		<div className="flex items-center gap-0.5">
			<div className="flex items-center text-gold-olive">

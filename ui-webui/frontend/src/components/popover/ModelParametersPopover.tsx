...
import SlidersHorizontal from '~icons/solar/tuning-2-bold-duotone';
 ...
 *
 * Reachable with no primary model selected, which is the normal state of a new
 * session (raven reports no per-session override until the user picks one) and
 * is where fallback and TTS selection are configured. Only the parameters
 * sub-menu depends on a primary model; with none it shows its empty state, the
 * same way the fallback sub-menu does when no options are available.
 */

...
export function useMessages(
	...
	options?: {
		 ...
		 * accumulate them into the conversation's "skills used" panel.
		 */
		onSkillsInjected?: (value: Record<string, unknown>) => void;
	},
) {
		...
		(event: AgentEvent) => {
			...
			if (event.type === EventType.CUSTOM) {
				...
				if (custom.name === 'team_updated') {
					...
					optionsRef.current?.onDagRunStarted?.(custom.value as Record<string, unknown>);
				} else if (custom.name === 'dag_node_updated' && custom.value) {
					optionsRef.current?.onDagNodeUpdated?.(custom.value as Record<string, unknown>);

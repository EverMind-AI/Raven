...
- ``{provider: openai_compat, base_url, model, ...}`` -> one OpenAI-compatible
  chat endpoint (:func:`evolver.orchestrator.providers.openai_compat.make_call_fn`).
...
def _build_call_fn(spec: dict) -> CallFn:
    ...
    if kind == "openai_compat":
        from evolver.orchestrator.providers.openai_compat import make_call_fn
    ...
    if kind == "claude_cli":
        ...
        from evolver.orchestrator.providers.claude_cli import make_claude_call_fn

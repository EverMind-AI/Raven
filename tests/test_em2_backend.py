...
class TestTrackIdRouting:
    ...
    async def test_agent_id_forwarded_to_search(
        ...
        b = EverosBackend(
            _ctx(tmp_path, agent_id="agt_fixed"), adapter=adapter,
        )
...
class TestUserSearchConversion:
    async def test_episodes_become_memories(self, tmp_path: Path) -> None:
        adapter = _FakeAdapter(search_response=_user_search_data(
            episodes=[
                SimpleNamespace(
                    id="ep1", session_id="s1", summary="liked espresso",
                    episode="full text", score=0.92,
                ),
            ],
        ))
        b = _backend(tmp_path, adapter=adapter)
    ...
    async def test_episode_falls_back_to_full_text_when_no_summary(
        ...
        adapter = _FakeAdapter(search_response=_user_search_data(
            episodes=[
                SimpleNamespace(
                    id="ep1", session_id="s1", summary="",
                    episode="raw content", score=0.5,
                ),
            ],
        ))
        b = _backend(tmp_path, adapter=adapter)
    ...
    async def test_hits_sorted_by_score_desc(self, tmp_path: Path) -> None:
        adapter = _FakeAdapter(search_response=_user_search_data(
            episodes=[
                SimpleNamespace(id="a", session_id="s", summary="lo",
                                episode="", score=0.3),
                SimpleNamespace(id="b", session_id="s", summary="hi",
                                episode="", score=0.9),
                SimpleNamespace(id="c", session_id="s", summary="mid",
                                episode="", score=0.6),
            ],
        ))
        b = _backend(tmp_path, adapter=adapter)
...
class TestAgentSearchConversion:
    async def test_skills_become_memories(self, tmp_path: Path) -> None:
        adapter = _FakeAdapter(search_response=_agent_search_data(
            skills=[
                SimpleNamespace(
                    id="sk1", name="git-resolver",
                    description="resolves git refs",
                    content="step 1 ...", confidence=0.85,
                    score=0.77,
                ),
            ],
        ))
    ...
    async def test_cases_include_key_insight(self, tmp_path: Path) -> None:
        adapter = _FakeAdapter(search_response=_agent_search_data(
            cases=[
                SimpleNamespace(
                    id="c1", task_intent="resolve git conflict",
                    approach="step-by-step",
                    quality_score=0.9, key_insight="use rerere",
                    score=0.8,
                ),
            ],
        ))
...
class TestStoreConversion:
    ...
    async def test_sender_id_stamped_by_owner_policy(
        ...
        by_role = {m["role"]: m["sender_id"]
                   for m in adapter.memorize_calls[0]["payload_messages"]}
    ...
    async def test_explicit_sender_id_preserved(self, tmp_path: Path) -> None:
        ...
        assert (
            adapter.memorize_calls[0]["payload_messages"][0]["sender_id"]
            == "alice-123"
        )
...
class TestRerankDegrade:
    ...
    async def test_agent_degrades_to_vector_without_rerank(self) -> None:
        method, SearchMethod = await self._capture_method(
            rerank_configured=False, user_id=None, agent_id="agent-x",
        )
        assert method == SearchMethod.VECTOR

    async def test_agent_uses_hybrid_with_rerank(self) -> None:
        method, SearchMethod = await self._capture_method(
            rerank_configured=True, user_id=None, agent_id="agent-x",
        )
    ...
    async def test_user_stays_hybrid_without_rerank(self) -> None:
        ...
        method, SearchMethod = await self._capture_method(
            rerank_configured=False, user_id="user-x", agent_id=None,
        )
        assert method == SearchMethod.HYBRID

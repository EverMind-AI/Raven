...
def _decision(
    *, decision_id: str = "dec_x",
    ...
    options: list[TaskOption] | None = None) -> PendingDecision:
    return PendingDecision(
        decision_id=decision_id, channel=channel, to=to,
        created_at_ms=_NOW_MS, ttl_min=60,
...
def _make_consumer(
    *, pending_store: PendingDecisionStore,
    ...
    require_confirm: bool = False) -> DecisionConsumer:
...
def _msg(content: str, *, channel: str = "feishu",
         to: str = "ou_xxx", action_origin: bool = False) -> TurnRequest:
    return TurnRequest(
        origin=Origin.USER,
        source=Source(
...
@pytest.mark.asyncio
async def test_pick_via_regex_executes_reply_and_marks_consumed(
    pending_store, feedback
):
    pending_store.put(_decision(options=[
        _option(oid="opt_1", title="任务 1",
                exec_payload={"prompt": "去做任务 1"}),
        _option(oid="opt_2", title="任务 2",
                exec_payload={"prompt": "去做任务 2"}),
    ]))
...
async def test_skip_path_marks_dismissed(pending_store, feedback):
    ...
    consumer = _make_consumer(
        pending_store=pending_store, router_provider=_SkipProvider(),
        feedback=feedback)
...
async def test_low_confidence_llm_falls_through(pending_store, feedback):
        ...
        async def chat_with_retry(self, *, messages, tools, model, tool_choice):
            class _Call:
                arguments = json.dumps(
                    {"intent": "pick", "option_index": 1, "confidence": 0.3}
                )
            class _Resp:
                has_tool_calls = True
                tool_calls = [_Call()]
    ...
    consumer = _make_consumer(
        pending_store=pending_store, router_provider=_AmbiguousProvider(),
        feedback=feedback)
...
@pytest.mark.asyncio
async def test_routine_confirm_pick_upgrades_routine(
    pending_store, feedback, tmp_path
):
    ...
    consumer = _make_consumer(
        pending_store=pending_store, routine_store=routine_store, feedback=feedback)
...
async def test_decision_consumer_short_circuits_agent_loop(pending_store):
    ...
    async def _consumer_hook(req):
        return MenuReply(
            channel=req.source.channel, chat_id=req.source.chat_id,
            content="✓ consumed by decision_consumer")

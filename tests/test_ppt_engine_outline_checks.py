...
@_needs_bundled
def test_a_deck_that_runs_one_example_twice_is_offered_an_unused_page_of_its_own_template(bound_template) -> None:
    """The live shape: twenty pages on ten of eleven examples, one of them five times,
    while five examples started nothing -- and the offer went straight to other templates.
    This template's own unused pages come first; nothing is borrowed while one fits."""
    from raven_ppt.tools.outline import _reused_prototypes

    pages = tuple(_plan(number, prototype=4 if number in (3, 9) else number) for number in range(1, 13))
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("beige_geometric_general_report"))

    assert "prototype 4" in said
    assert "page 9 -> this template's page" in said, (
        "the second page on the example is the one with somewhere else to go"
    )
    assert "page 3 ->" not in said, "the first page on an example keeps it"
    assert "prototype: N" in said and "content examples start no page of this deck" in said
    assert "borrowed:" not in said, "a page of this template fits, so no other template is named"
    assert "not wrong" in said, "repeating an example is not an error and must not read as one"


@_needs_bundled
def test_every_repeating_page_an_unused_example_fits_is_kept_in_its_own_template(bound_template) -> None:
    """The deck the offer was written for repeats one example five times and another
    three: more pages with somewhere else to go than the four the borrow shortlist names.
    Ten pages alternating between two of the red template's eleven examples leave eight
    repeating (no two adjacent, so no series) and nine examples unused, every one of which
    fits a two-point page. Each repeating page is offered one, and none is told that no
    example of this template fits."""
    from raven_ppt.tools.outline import MOST_BORROWS_OFFERED, _reused_prototypes

    examples = _content_examples("red_chinese_traditional_culture")
    pages = tuple(_plan(number, prototype=examples[number % 2], says=2) for number in range(1, 11))
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("red_chinese_traditional_culture"))

    assert said.count("-> this template's page") == 8 > MOST_BORROWS_OFFERED, said
    assert "no unused example" not in said and "borrowed:" not in said, (
        "no page is sent to another template while an example of its own still fits"
    )


@_needs_bundled
def test_the_borrow_sentence_names_only_the_pages_that_got_a_borrow(bound_template) -> None:
    """Every example taken and six pages repeating: the borrow shortlist stops at four, and
    the sentence that says which pages it is for names those four and no more."""
    import re as _re

    from raven_ppt.tools.outline import MOST_BORROWS_OFFERED, _reused_prototypes

    pages = _every_example_once_plus("red_chinese_traditional_culture", repeats=6)
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("red_chinese_traditional_culture"))

    offered = [int(number) for number in _re.findall(r"page (\d+) -> \w+ page \d+", said)]
    assert len(offered) == MOST_BORROWS_OFFERED, said
    sentence = _re.search(r"For pages ([\d, ]+) this template has no unused example left that fits", said)
    assert sentence is not None, said
    assert [int(number) for number in sentence.group(1).split(", ")] == offered


@_needs_bundled
def test_a_deck_that_has_used_every_example_is_offered_a_page_to_borrow(bound_template) -> None:
    """Borrowing is for the page nothing of this template is left for."""
    from raven_ppt.tools.outline import _reused_prototypes

    pages = _every_example_once_plus("red_chinese_traditional_culture", repeats=1)
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("red_chinese_traditional_culture"))

    assert "this template's page" not in said, "every example is taken; there is nothing of its own to offer"
    assert "this template has no unused example left that fits" in said
    assert "borrowed: '<template>'" in said and "bundled('<template>')" in said


@_needs_bundled
def test_the_offer_names_the_slot_its_share_of_the_page_and_the_largest(bound_template) -> None:
    """What an author chooses a picture page by, said beside the number: the red template's
    page 11 carries the largest picture slot it draws, and a live deck cloned the 16% one
    five times without ever being told."""
    from dataclasses import replace
    ...
    pages = (
        _plan(1, prototype=1),
        replace(_plan(2, prototype=4), figures=("f1",), carries="a photograph and three cards"),
        _plan(3, prototype=12),
        replace(_plan(4, prototype=4), figures=("f2",), carries="a photograph and three cards"),
    )
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("red_chinese_traditional_culture"))

    assert "page 4 -> this template's page 11" in said, said
    assert re.search(r"7\.8x4\.4in, 3\d% of the page, the largest picture slot this template draws", said), said


@_needs_bundled
def test_pages_in_a_row_on_one_example_are_a_series_and_left_alone(bound_template) -> None:
    """Five case pages built alike so a reader compares them across is good work; the ask
    says so and lands its offers on the repeats outside the run."""
    from raven_ppt.tools.outline import SERIES_RUN, _reused_prototypes
    ...
    assert SERIES_RUN == 3
    assert "pages 3-7, in a row on 6, read as a series" in said
    assert "left alone" in said
    assert not any(f"page {number} ->" in said for number in range(3, 8)), "no page is pulled out of the series"
    assert "page 9 -> this template's page" in said, "the repeat outside the run is the one offered somewhere"

    only_series = Outline(takeaway="t", pages=pages[:8])
    assert _reused_prototypes(only_series, bound_template("red_chinese_traditional_culture")) == "", (
        "a series and nothing else repeating is nothing to ask about"
    )


@_needs_bundled
def test_the_templates_own_furniture_repeating_is_not_a_repeated_example(bound_template) -> None:
    """Three dividers on the template's section page are the template used as meant."""
    from raven_ppt.tools.outline import _reused_prototypes
    ...
    assert (
        _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("red_chinese_traditional_culture")) == ""
    )


@_needs_bundled
def test_a_deck_that_uses_every_example_once_is_told_nothing(bound_template) -> None:
    """No repetition, no ask. The offer costs seven template opens; it is not free."""
    from raven_ppt.tools.outline import _reused_prototypes

    pages = tuple(_plan(number, prototype=number) for number in range(1, 13))
    assert (
        _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("beige_geometric_general_report")) == ""
    )


@_needs_bundled
def test_no_template_bound_means_nothing_to_borrow_into(bound_template) -> None:
    from raven_ppt.tools.outline import _reused_prototypes

    pages = tuple(_plan(number, prototype=4) for number in range(1, 5))
    assert _reused_prototypes(Outline(takeaway="t", pages=pages), _Unbound()) == ""


@_needs_bundled
def test_one_page_is_offered_once_across_the_pages_that_repeat(bound_template) -> None:
    """Answering repetition with the same page twice would answer it with itself -- for
    this template's own pages as for the borrowed ones."""
    import re as _re

    from raven_ppt.tools.outline import _reused_prototypes

    examples = _content_examples("beige_geometric_general_report")
    pages = tuple(
        _plan(number, prototype=examples[0] if number % 2 else examples[number // 2]) for number in range(1, 10)
    )
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("beige_geometric_general_report"))

    offered = _re.findall(r"-> (this template's|\w+) page (\d+)", said)
    assert offered, said
...
@_needs_bundled
def test_the_offers_spread_across_the_templates_they_come_from(bound_template) -> None:
    """Answering a monoculture with a monoculture is not an answer.

    The reference pages are declared eleven of one template first, so picked in
    declaration order a deck repeating four examples was offered four pages of that one
    template. Round the templates instead.
    """
    import re as _re

    from raven_ppt.tools.outline import MOST_BORROWS_OFFERED, _reused_prototypes

    pages = _every_example_once_plus("warm_bauhaus_quarterly_review", repeats=MOST_BORROWS_OFFERED)
    said = _reused_prototypes(Outline(takeaway="t", pages=pages), bound_template("warm_bauhaus_quarterly_review"))

    stems = [stem for stem, _ in _re.findall(r"-> (\w+) page (\d+)", said)]
    assert len(stems) == MOST_BORROWS_OFFERED, said
    assert len(set(stems)) == len(stems), f"four offers from {len(set(stems))} template(s): {stems}"

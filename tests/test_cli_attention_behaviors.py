...
class TestAttentionCommand:
    def test_missing_file_exits_with_code_1(self, tmp_path: Path) -> None:
        result = runner.invoke(
            sentinel_app, ["attention", "-w", str(tmp_path)],
        )
...
class TestBehaviorsCommand:
    def test_missing_file_exits(self, tmp_path: Path) -> None:
        result = runner.invoke(
            sentinel_app, ["behaviors", "-w", str(tmp_path)],
        )
        assert result.exit_code == 1
    ...
    def test_folded_format(self, tmp_path: Path) -> None:
        _seed_behaviors(tmp_path, [
            _ev(id="evt_a", day="2026-05-29", start="14:00", end="14:30",
                intent="debug", outcome="resolved",
                topic="memory-engine", project="raven",
                summary="debugged session split"),
        ])
        result = runner.invoke(sentinel_app, [
            "behaviors", "-w", str(tmp_path), "--folded",
        ])
        assert result.exit_code == 0

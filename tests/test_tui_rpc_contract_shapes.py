...
and ``test_tui_rpc_registration`` compares the method sets. Neither of them looks
...
async def test_ext_list(workspace: Path) -> None:
    from raven.tui_rpc.methods.console import ext_list
...
async def test_cron_round_trip(workspace: Path) -> None:
    ...
    """
    from raven.tui_rpc.methods import console
...
async def test_settings_usage(workspace: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from raven.tui_rpc.methods.console import settings_usage
...
async def test_channels_status(workspace: Path) -> None:
    from raven.tui_rpc.methods.console import channels_status
...
def _everos(monkeypatch: pytest.MonkeyPatch, payload: dict) -> None:
    ...
    monkeypatch.setattr("raven.tui_rpc.methods.memory._post", _post)
    monkeypatch.setattr("raven.tui_rpc.methods.memory._cfg", lambda: ("http://x", "u", "a"))


async def test_memory_stats(monkeypatch: pytest.MonkeyPatch) -> None:
    from raven.tui_rpc.methods.memory import memory_stats
...
async def test_memory_stats_with_everos_down(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("raven.tui_rpc.methods.memory._post", _boom)
    monkeypatch.setattr("raven.tui_rpc.methods.memory._cfg", lambda: ("http://x", "u", "a"))
...
async def test_memory_list_every_kind(monkeypatch: pytest.MonkeyPatch, kind: str, field: str, row: dict) -> None:
    ...
    """
    from raven.tui_rpc.methods.memory import memory_list
...
async def test_session_create(workspace: Path) -> None:
    from raven.tui_rpc.methods import session as session_mod
...
async def test_session_status(workspace: Path) -> None:
    from raven.tui_rpc.methods.slash_routing import session_status
...
async def test_approval_respond() -> None:
    from raven.tui_rpc.methods.approval import approval_respond
...
async def test_clarify_respond() -> None:
    from raven.tui_rpc.methods.question import question_respond
...
async def test_confirm_respond() -> None:
    from raven.tui_rpc.methods.confirm import confirm_respond
...
async def test_completion_providers() -> None:
    from raven.tui_rpc.methods.slash_routing import complete_path, complete_slash
...
async def test_slash_exec(workspace: Path) -> None:
    from raven.tui_rpc.methods.slash_routing import slash_exec
...
async def test_terminal_resize() -> None:
    from raven.tui_rpc.methods.terminal import terminal_resize

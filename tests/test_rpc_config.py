...
def fake_home(monkeypatch, tmp_path) -> Path:
    """A home of our own, and the loader's config path put back afterwards.

    `_pin` below clears `loader._current_config_path` on purpose, so the next
    `load_config()` falls back to this home -- but the global outlives the test,
    and after the rpc rename this module collects *before* `test_segments.py`,
    which reads the real config through it. Restoring here is what keeps a
    module's own isolation from becoming the next module's environment.
    """

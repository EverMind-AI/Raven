...
def _no_real_oauth_credentials(tmp_path, monkeypatch):
    """Point the OAuth credential lookups at a temp dir for the whole suite.
    ...
    makes providers report themselves configured and sends the Codex catalog
    lookup to the network. Tests that exercise a credential set these themselves.
    """
    for name in ("CHATGPT_TOKEN_DIR", "CHATGPT_AUTH_FILE", "GITHUB_COPILOT_TOKEN_DIR"):

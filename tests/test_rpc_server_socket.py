...
A second, independent concern lives in the same file because it shares the
socket-pair fixture: the **trust-boundary gate**. When ``auth_token`` is set
(the TCP-loopback production transport), the peer MUST send the shared secret
as its first newline-terminated line. Both rejection paths close the
connection before any dispatch, and both had no coverage:

4. ``test_auth_token_mismatch_closes_the_connection`` — a rogue local process
   that guesses the port but not the secret.
5. ``test_auth_token_never_sent_closes_the_connection`` — the peer connects
   and hangs up without speaking. Reached via ``IncompleteReadError`` (EOF),
   which is why the test does not have to wait out the 10 s timeout.

Both assert *behaviourally* — ``serve_forever`` returns and a follow-up
request draws no response — rather than reading a private flag, so the gate
stays covered if the rejection is reimplemented.
"""
...
# ---------------------------------------------------------------------------
# Trust-boundary gate (auth_token)
# ---------------------------------------------------------------------------


async def _serve_with_auth(token: str) -> tuple[RpcServer, asyncio.Task, socket.socket, socket.socket, Path]:
    """Start an RpcServer over a socket pair with the auth gate armed.

    Uses the ``sock=`` constructor path (the cross-platform production
    transport) because that is the only one that arms ``auth_token``.
    """
    server_sock, client, conn, tmp = await _wire_paired_socket()
    disp = Dispatcher()
    register_aligned_methods(disp)
    server = RpcServer(sock=conn, dispatcher=disp, auth_token=token)
    serve_task = asyncio.create_task(server.serve_forever())
    await server.started.wait()
    return server, serve_task, client, server_sock, tmp


@pytest.mark.asyncio
async def test_auth_token_mismatch_closes_the_connection() -> None:
    """A wrong first line is rejected before any frame is dispatched."""
    _server, serve_task, client, server_sock, _tmp = await _serve_with_auth("correct-horse")
    loop = asyncio.get_running_loop()
    try:
        await loop.sock_sendall(client, b"wrong-token\n")

        # serve_forever must return on its own; it does not hang waiting for
        # frames from a peer it just rejected.
        await asyncio.wait_for(serve_task, timeout=2.0)

        # And a well-formed request that arrives afterwards draws nothing:
        # the gate closed before the dispatch loop was ever entered.
        await _send(client, "system.ping", {}, 1)
        assert await _read_one_frame(client, timeout=0.5) == b"", "rejected peer still got a response"
    finally:
        client.close()
        server_sock.close()
...
@pytest.mark.asyncio
async def test_auth_token_never_sent_closes_the_connection() -> None:
    """A peer that connects and hangs up without speaking is rejected."""
    _server, serve_task, client, server_sock, _tmp = await _serve_with_auth("correct-horse")
    try:
        # Hanging up gives the reader EOF -> IncompleteReadError, so this
        # exercises the same branch as a 10 s timeout would without the wait.

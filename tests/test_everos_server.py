"""Tests for raven.plugins.memory.everos.server."""
...
class TestSpawnPreflight:
    ...
    async def test_missing_api_key_does_not_spawn(self, everos_toml, monkeypatch) -> None:
        ...
        spawned = []
        monkeypatch.setattr(
            "raven.plugins.memory.everos.server._start_server_if_unlocked",
            lambda *a, **kw: spawned.append(1),
        )
        with patch("raven.plugins.memory.everos.server._probe_health", return_value=False):
    ...
    async def test_a_running_server_needs_no_credential_check(self, everos_toml) -> None:
        ...
        with (
            patch("raven.plugins.memory.everos.server._probe_health", return_value=True),
            patch("raven.plugins.memory.everos.server._speaks_our_api", return_value=True),
        ):
    ...
    async def test_configured_llm_reaches_the_spawn(self, everos_toml, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "raven.plugins.memory.everos.server.get_logs_dir",
            lambda: tmp_path,
        )
        with patch("raven.plugins.memory.everos.server._probe_health", side_effect=[False, True]):
...
class TestTheRootDescribesItsOwnAddress:
    ...
    def _no_real_spawn(self, tmp_path, monkeypatch):
        monkeypatch.setattr("raven.plugins.memory.everos.server.get_logs_dir", lambda: tmp_path)
        monkeypatch.setattr("raven.plugins.memory.everos.server.get_data_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "raven.plugins.memory.everos.server._everos_executable",
            lambda: "/bin/true",
        )
    ...
    def test_the_declared_address_is_written_before_the_child_starts(self, everos_toml) -> None:
        ...
        from raven.plugins.memory.everos.server import _start_server_if_unlocked
...
class TestThePrimitivesAgainstTheRealOS:
    ...
    def test_an_untouched_root_reads_as_free(self, tmp_path) -> None:
        from raven.plugins.memory.everos.server import ome_lock_held
    ...
    def test_ps_does_not_mistake_this_process_for_a_server(self) -> None:
        ...
        """
        from raven.plugins.memory.everos.server import _is_everos_server
    ...
    def test_health_probe_reads_a_real_response(self, tmp_path) -> None:
        ...
        from raven.plugins.memory.everos.server import _probe_health
...
class TestDeadChildDetection:
    ...
    async def test_exited_child_fails_immediately(self, everos_toml, _logs, monkeypatch) -> None:
        ...
        with patch("raven.plugins.memory.everos.server._probe_health", side_effect=_probe):
...
class TestEnsureEverosServer:
    ...
    async def test_a_live_server_that_cannot_serve_our_api_is_refused(self) -> None:
        ...
        with (
            patch("raven.plugins.memory.everos.server._probe_health", return_value=True),
            patch("raven.plugins.memory.everos.server._speaks_our_api", return_value=False),
            patch("raven.plugins.memory.everos.server._start_server_if_unlocked") as start,
        ):
    ...
    async def test_auto_start_on_connection_error(self, tmp_path, everos_toml) -> None:
        ...
        with (
            patch(
                "raven.plugins.memory.everos.server._probe_health",
    ...
    def test_port_extraction(self) -> None:
        from raven.plugins.memory.everos.server import _extract_port
...
class TestProbeClassification:
    ...
    def test_probe_budget_is_one_second(self) -> None:
        ...
        check.
        """
        from raven.plugins.memory.everos.server import _PROBE_TIMEOUT_S
...
class TestLockHolderLookup:
    ...
    def test_returns_none_when_lock_file_absent(self, tmp_path) -> None:
        from raven.plugins.memory.everos.server import lock_holder
    ...
    def test_lsof_pid_is_verified_against_the_cmdline(self, tmp_path) -> None:
        ...
        from raven.plugins.memory.everos import server as _server
...
class TestParsingLsofListenOutput:
    ...
    def test_ipv4_row(self) -> None:
        from raven.plugins.memory.everos.server import _parse_listen_port

...
from oncall_flow.tools.ops_exec import _TIMEOUT_S, OpsExecTool, run_on_machine  # noqa: E402
...
def campaign(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "oncall_flow.tools.ops_case_dict._campaign", lambda c, l: (backend, tmp_path / "ledger.json", tmp_path, meta)
    )
    monkeypatch.setattr("oncall_flow.connections.display_name", lambda c: "my CPU box")
    return backend
...
async def test_the_command_is_capped_and_the_cap_is_not_an_argument(campaign):
    ...
    assert "machine" in OpsExecTool().parameters["properties"]
...
async def test_exec_with_a_machine_runs_there_and_says_where(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("oncall_flow.connections.get", lambda cid: {"host": "h"} if cid == "conn_box" else None)
    monkeypatch.setattr("oncall_flow.tools.ops_exec.run_on_machine", fake_run)

    out = await OpsExecTool().execute(command="ls /srv", machine="conn_box")
...
async def test_a_name_that_is_neither_machine_nor_campaign_says_so(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("oncall_flow.connections.describe", lambda: "0 connection(s).")

    out = await OpsExecTool().execute(command="ls", machine="conn_typo")

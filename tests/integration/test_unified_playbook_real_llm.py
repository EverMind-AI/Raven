...
TRAVEL_PERSONA_PROMPT = (
    "I travel solo a few times a year. Create and save a reusable travel assistant named travel-concierge. "
    "From then on I want to give it only the destination, the dates, the total budget, who is coming and how "
    "fast a pace I want, and get back a plan I can actually follow. "
    "It has to check current local restrictions, customs, neighbourhood safety, opening hours and booking "
    "changes; lay out each day by distance, public transport, opening times, reservations and how tired I will "
    "be; balance lodging, transport, food and admission against the budget, offer alternatives at different "
    "price points, and steer around tourist traps. "
    "The deliverable is a clearly formatted six-part travel brief, no more than 120 words per day. It goes "
    "straight to the people coming with me, so a separate responsibility for content layout and visual "
    "presentation must settle the format and the information hierarchy -- the route planner must not do that "
    "on the side. "
    "I have an old knee injury: no more than 12000 steps a day and no more than 30 minutes of walking at a "
    "stretch. No red-eye flights, and no connection shorter than 90 minutes. These are hard limits, not "
    "suggestions. If the destination, the dates, the total budget, who is coming or the pace is missing, say "
    "what is missing before planning anything. Do not book or pay without my explicit approval; any booking or "
    "payment call must carry approved=true and must be refused above 2800 CNY in a single charge. Once an "
    "itinerary is approved you may watch booking deadlines and remaining availability, but a non-urgent "
    "reminder may only go out between 18:00 and 21:00 my local time. If a lookup fails or the information is "
    "thin, come back with what is confirmed, what is still missing and the next step, and invent nothing. "
    "For now only create and save this assistant: do not plan any specific trip and do not run a workflow."
)
...
async def test_live_whole_turn_infers_a_travel_assistant_harness_from_user_needs(
    ...
    assert all("create and save" not in worker.brief.lower() for worker in workers)
    assert all("do not plan any specific trip" not in worker.brief.lower() for worker in workers)
    assert list((playbook_root / ".runs").glob("*.json"))
    assert not list(tmp_path.glob("skills/**/SKILL.md")), "Persona mode must not duplicate the Harness as a skill"

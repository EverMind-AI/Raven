...
async def test_batching_large_conversation(scanner: ClaudeCodeScanner, tmp_path: Path) -> None:
    """160 messages -> sixteen store calls of 10, is_final only on the last, every one bulk."""
    results = await scanner.scan()
    ...
    assert sizes == [10] * 16
    assert [call["metadata"]["is_final"] for call in backend.store_calls] == [False] * 15 + [True]

...
Ported from the fork's tests/test_gate_primary_race.py against the code-flow
plugin's gate (the fork's child armed itself from the env and
build_workspace_gate; the plugin child constructs the gate from the same two
roots the config slice carries). Real processes, one shared repo allocation
lock: exactly one contender may bind the primary checkout; every other lands
in the occupied/ask-user path. Threads cannot test this - flock is per file
description and the gate's atomicity claim is about independent processes.
"""
...
from code_flow import gate as wg

session = sys.argv[1]
start_flag = Path(sys.argv[2])
repo = Path(sys.argv[3])
base = Path(sys.argv[5])
repos = Path(sys.argv[6])

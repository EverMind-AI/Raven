...
librsvg 只用于从生成概念图描出干净的标志矢量，ImageMagick 只做裁切与格式转换，**不做叠加或透视贴图**。只列实验镜像里
...
| 交付物 | 主工具 | 做法 | 验证 |
| --- | --- | --- | --- |
| 标志概念 | `image_generate` | 按最小系统合同出 2–3 个方向，各一张干净背景图 | 并排缩略比较结构，不比颜色 |
| 标志干净版 | 从选定的生成概念图用 `potrace`（单色）或 `vtracer`（多色）描出矢量，Inkscape CLI 清理，标准字在 HTML/CSS 排 → `render_file` 出 PNG；命令见共享卡 `svg-and-vector.md` | 选定方向重建为清楚的横版、竖版、单色、反白 | 32 px 与 1024 px 都能认 |

  ...
  dispatch(params: EventNotificationParams<unknown>): boolean {
    ...
    if (!handler) {
      process.stderr.write(
        `[rpc-subscriptions] event for unknown subscription_id=${params.subscription_id} dropped\n`,
      );
      return false;
    }
    try {
      ...
      process.stderr.write(
        `[rpc-subscriptions] handler for ${params.subscription_id} threw: ${String(err)}\n`,
      );
    }
    return true;
  }

...
describe('applyDisplay', () => {
  ...
  it('keeps reasoning on by default but honours an explicit show_reasoning false', () => {
    const setBell = vi.fn()

    applyDisplay({ config: { display: { show_reasoning: false } } }, setBell)
    expect($uiState.get().showReasoning).toBe(false)
...
describe('normalizeStatusBar', () => {
  ...
  it('falls back to bottom for unknown/unset values', () => {

...
const ROSTER: Row[] = [
  ...
  { name: 'qwen_code', preset: 'qwen_code', kind: 'acp', configured: true, enabled: false,
    group: 'installed', probe_status: 'ready', probe_detail: '', has_api_key: false, needs_auth: false,
    mcps: [], allow_mcp_secrets: false, test_running: false, last_test_ok: false,
    last_test_detail: 'its model provider does not serve the model it is set to use; pick another model: run `qwen` in a terminal and type `/auth` there, then connect again.',
    last_test_remedy: { kind: 'model', command: 'qwen', then: '/auth' },

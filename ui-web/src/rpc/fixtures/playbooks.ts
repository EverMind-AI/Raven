...
export interface PlaybooksFixture {
  fixtures: Fixtures
}

export function createPlaybooks(env: FixtureEnv): PlaybooksFixture {
  const creds: Creds = { params: {}, oauth: {} }
  const find = (name: string): Playbook | undefined => PB_FIXTURE.find((x) => x.name === name)

  return {
    fixtures: {
      'playbooks.list': () => ({
        playbooks: PB_FIXTURE.map((p) => ({
          name: p.name,
      ...
      'playbooks.get': (p) => {
        ...
        return {
          playbook: {
            ...
            task_summary: found.task_summary,
      ...
      'playbooks.credentials.get': (p) => {
        const found = find(p.name)
        if (!found) throw new Error(`no playbook named ${p.name}`)
        const secret = Object.entries(found.params || {}).filter(([, v]) => v.type === 'secret')
        const servers = Object.entries(found.mcp_servers || {})
        return {
          params: secret.map(([k, v]) => ({
      ...
      'playbooks.credentials.clear': (p) => {
        delete creds.params[`${p.name}/${p.param}`]
        return { ok: true }
      },
      'playbooks.oauth.authorize': (p) => {
        env.schedule(1500, () => { creds.oauth[`${p.name}/${p.server}`] = true })
        return {

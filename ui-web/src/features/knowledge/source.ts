/* -- knowledge bases: the rpc source ----------------------------------
   The island talks to sources.knowledge and knows nothing about transport;
   this module only knows how to speak knowledge.* over /rpc. The live layer
...
export const knowledgeSource: KnowledgeSource = {
  ...
  bases: () => gateway().call('knowledge.bases.list', {}).then((r) => (r && r.bases) || []),
  create: (name, description, embedding = true) =>
    gateway().call('knowledge.bases.create', { name, description, embedding }).then((r) => r && r.base),
  rename: (id, name) =>
    gateway().call('knowledge.bases.rename', { base_id: id, name }).then((r) => r && r.base),
  remove: (id) => gateway().call('knowledge.bases.delete', { base_id: id }),
  ...
  settings: (baseId, values) =>
    gateway().call('knowledge.bases.settings', { base_id: baseId, ...values }).then((r) => r && r.base),
  documents: (baseId) =>
    gateway().call('knowledge.documents.list', { base_id: baseId }).then((r) => (r && r.documents) || []),
  ...
  upload: async (baseId, file) => {
    ...
    const up = await gateway().call('fs.upload', { name: file.name, content_b64: b64 })
    const added = await gateway().call('knowledge.documents.add', { base_id: baseId, path: up.path })
  ...
  /* The gateway reads the page. A browser cannot fetch a third-party site on
     ...
     chunked anyway. */
  addUrl: (baseId, url) =>
    gateway().call('knowledge.documents.add_url', { base_id: baseId, url }).then((r) => r && r.document),
  index: (documentId) =>
    gateway().call('knowledge.documents.index', { document_id: documentId }).then((r) => r && r.document),
  ...
  removeDoc: (documentId) =>
    gateway().call('knowledge.documents.delete', { document_id: documentId }).then(() => undefined),

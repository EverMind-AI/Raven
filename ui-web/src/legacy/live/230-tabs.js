/* ── subagents: what this conversation handed off ──────────────────────
   ...
   and the `?desk-demo=1` canvas, which the fixture step takes. */
...
export function install() {
  ...
  if (new URLSearchParams(location.search).get('desk-demo') === '1') {
    ...
    const demoOverride = new Map([['model-permissions-7c2a', 'max']]);
    let demoTier = 'high';
    ...
    const demoInherits = (handle) =>
      ((DEMO_RUNGS[handle] || []).some((m) => m.id === demoTier) ? demoTier : null);
    const demoModeReply = (handle) => ({
      mode: demoOverride.get(handle) || null,
    ...
    sources.agents.instanceSetMode = async (_agent, handle, mode) => {
      ...
      } else demoOverride.set(handle, mode);
      return demoModeReply(handle);
    };

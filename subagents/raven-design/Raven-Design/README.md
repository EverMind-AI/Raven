...
OpenAI and generic OpenAI-compatible API-key providers use Chat Completions by default. Select the Responses transport with `raven provider set <name> --protocol responses`; OAuth-based OpenAI Codex keeps its dedicated transport.
...
raven/rendering/         visual normalization and multimodal preview service
raven/skill_hub/         external skill retrieval
raven/task_state/        resident per-session goals, requirements, and progress

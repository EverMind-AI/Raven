...
- **EverOS lane** — ``backend.recall(user_id=...)`` (segment 3,
  ``# Memory``), the Visual Domain Selector, and a
  :class:`SkillForgeRouter` over 1–3 sources (segment 5, ``# Skills``).
...
def build_context_engine(
    *,
    ...
    visual_domain_selector = _build_visual_domain_selector(
        provider=provider,
    ...
    builders = [
        ...
        SkillsSegmentBuilder(
            ...
            visual_domain_selector=visual_domain_selector,
        ),
...
def _build_visual_domain_selector(
    *,
    provider: LLMProvider,
    builder: ContextBuilder,
    skill_forge_config: "SkillForgeConfig | None",
    provider_pool: "ProviderPool | None" = None,
) -> "VisualDomainSkillSelector | None":
    """Build Raven-Design's fixed-catalog selector when explicitly configured."""
    if skill_forge_config is None or not bool(getattr(skill_forge_config, "enabled", False)):
        return None
    selector_config = getattr(skill_forge_config, "visual_domain_selector", None)
    if selector_config is None or not bool(getattr(selector_config, "enabled", False)):
        return None

    from raven.memory_engine.skill_forge import VisualDomainSkillSelector

    model = getattr(selector_config, "model", None) or None
    return VisualDomainSkillSelector.from_registry(
        provider,
        builder.skills.registry,
        preferred_max=int(getattr(selector_config, "preferred_max", 2)),
        alternatives_max=int(getattr(selector_config, "alternatives_max", 3)),
        model=model,
        temperature=float(getattr(selector_config, "temperature", 0.0)),
        max_tokens=int(getattr(selector_config, "max_tokens", 8192)),
        pin=(
            provider_pool.bind_pin(model, getattr(selector_config, "provider", None))
            if provider_pool
            else None
        ),
        blocklist=getattr(skill_forge_config, "blocklist", None),
    )

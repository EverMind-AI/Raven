"""Segment 5 — ``# Skills``. Domain selection plus general SkillForge.

Raven-Design's Visual Domain Selector compares the full bodies of its fixed
15-Skill domain catalog, then exposes only preferred and alternative cards to
the main Agent. It runs beside the existing multi-source SkillForge pipeline;
the 15 domain names are removed from that general lane so their bodies are not
injected twice. Public, workspace, EverOS, and Hub Skills keep their original
routing behavior.
...
if TYPE_CHECKING:
    ...
    from raven.memory_engine.skill_forge.visual_domain_selector import VisualDomainSkillSelector
...
class SkillsSegmentBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        visual_domain_selector: "VisualDomainSkillSelector | None" = None,
    ) -> None:
        self._router = router
        ...
        self._visual_domain_selector = visual_domain_selector
        self._visual_domain_names = frozenset(
            card.name for card in getattr(visual_domain_selector, "cards", ())
        )

    def set_provider(self, provider: "LLMProvider", model: str) -> None:
        """Hand a live ``/model`` switch down to the LLM-backed users in this
        ...
        if self._visual_domain_selector is not None:
            self._visual_domain_selector.set_provider(provider, model)
    ...
    async def build(self, ctx: AssemblyContext) -> Segment | None:
        ...
        if self._visual_domain_selector is not None:
            selection, general = await asyncio.gather(
                self._visual_domain_selector.select(query),
                self._build_general_skills(ctx, excluded_names=self._visual_domain_names),
            )
            return self._merge_visual_and_general(selection, general)
        return await self._build_general_skills(ctx)

    async def _build_general_skills(
        self,
        ctx: AssemblyContext,
        *,
        ...
        candidates = list(
            await self._router.select(
                ...
                k=self._pool_size + len(excluded_names),
            )
        )

        if excluded_names:
            candidates = [candidate for candidate in candidates if candidate.name not in excluded_names]
    ...
    @staticmethod
    def _merge_visual_and_general(selection: Any, general: Segment) -> Segment:
        visual_body = render.render_visual_domain_selection(selection)
        general_body = general.text.removeprefix("# Skills\n\n")
        parts: list[str] = []
        if visual_body:
            parts.append(visual_body)
        if general_body:
            if visual_body:
                parts.append(f"## Other Routed Skills\n\n{general_body}")
            else:
                parts.append(general_body)

        preferred_ids = [card.qualified_id for card in selection.preferred]
        alternative_ids = [card.qualified_id for card in selection.alternatives]
        meta: dict[str, Any] = {
            **general.meta,
            "preferred_visual_skill_ids": preferred_ids,
            "alternative_visual_skill_ids": alternative_ids,
            "described_skill_ids": [*preferred_ids, *alternative_ids],
            "visual_domain_selector": True,
        }
        if selection.degraded:
            meta["visual_domain_selector_degraded"] = True
        body = "\n\n".join(parts)
        return Segment(text=f"# Skills\n\n{body}" if body else "", meta=meta)

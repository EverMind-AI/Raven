...
1. 读取 `references/anti-slop-gallery/page-1.jpg` 的风险索引，按任务、参考路线和代表画面的实际
   风险选择对应组，再读取覆盖这些组的 2–4 页标本；方向无法归类或高风险跨页时继续读取直至覆盖。
2. 在工作区根目录（交付目录之外）建立 `ANTI-SLOP-CHECK.md`，只记录路由到的高／中风险、当前
   可见症状、对应标本、正向标杆机制、修改与重渲染证据；低风险和不涉及项不写空章节。交付前
   再对完整产物做全局扫描，新风险补入账本。遗漏实际命中的组即未完成，篇幅不构成证据。
3. **质感门槛**：凡目标画面离不开材质与光影词（金属、玻璃、体积光、景深、渲染）——暗场硬件
...
4. **Web 主视觉门槛**：内容型／营销型 Web 依 `$build-polished-visual-frontends` 先完成

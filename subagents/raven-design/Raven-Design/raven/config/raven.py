...
class VisualDomainSelectorConfig(_Base):
    """Full-body LLM routing over Raven-Design's fixed domain catalog."""

    enabled: bool = True
    """Expose preferred and alternative domain cards while keeping other
    Skills on the general SkillForge route."""

    preferred_max: int = Field(default=2, ge=0, le=5)
    """Maximum Skills whose domain directly owns part of the request."""

    alternatives_max: int = Field(default=3, ge=0, le=15)
    """Maximum plausible secondary or ambiguity-resolving Skills."""

    model: str | None = None
    """Optional selector model. Unset follows the conversation's model."""

    provider: str | None = None
    """Configured provider serving ``model`` when the selector is pinned."""

    temperature: float = 0.0
    """Selector sampling temperature."""

    max_tokens: int = Field(default=8192, ge=1)
    """Selector output ceiling, including reasoning tokens when applicable."""
...
class SkillForgeConfig(_Base):
    ...
    visual_domain_selector: VisualDomainSelectorConfig = Field(
        default_factory=VisualDomainSelectorConfig,
    )
    """Full-body selection over Raven-Design's 15 packaged visual domain
    Skills. The main Agent still reads a selected body on demand."""

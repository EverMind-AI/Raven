...
class ContextCompactionConfig(Base):
    """Same-turn context compaction policy."""

    enabled: bool = True
    trigger_ratio: float = Field(default=0.80, gt=0.0, lt=1.0)
    target_ratio: float = Field(default=0.50, gt=0.0, lt=1.0)
    tail_ratio: float = Field(default=0.20, gt=0.0, lt=1.0)
    min_recent_iterations: int = Field(default=4, ge=1)
    max_user_tokens: int = Field(default=20_000, ge=1)
    safety_margin_tokens: int = Field(default=8_192, ge=0)
    summary_min_tokens: int = Field(default=2_048, ge=256)
    summary_max_tokens: int = Field(default=12_000, ge=256)
    summary_reasoning_effort: str = "inherit"
    max_compactions_per_turn: int = Field(default=4, ge=1)
    retry_after_iterations: int = Field(default=2, ge=1)
    min_savings_ratio: float = Field(default=0.15, ge=0.0, lt=1.0)
    max_tool_result_chars: int = Field(default=64_000, ge=4_096)

    @model_validator(mode="after")
    def _validate_compaction_policy(self) -> "ContextCompactionConfig":
        if self.target_ratio >= self.trigger_ratio:
            raise ValueError("targetRatio must be lower than triggerRatio")
        if self.tail_ratio >= self.target_ratio:
            raise ValueError("tailRatio must be lower than targetRatio")
        if self.summary_min_tokens > self.summary_max_tokens:
            raise ValueError("summaryMinTokens cannot exceed summaryMaxTokens")
        if not self.summary_reasoning_effort.strip():
            raise ValueError("summaryReasoningEffort cannot be empty")
        return self
...
class MediaToolConfig(Base):
    """Config for a media-generation tool (key + base + model + API style).
    ...
    model: str = ""
    api_style: Literal["chat_modalities", "openrouter_images", "images"] = "chat_modalities"
...
class MediaGenConfig(Base):
    ...
    Images support OpenRouter and OpenAI-compatible Images APIs. Speech uses
    OpenRouter chat output modalities, and video uses its async ``/videos``
    endpoint (Kling).
    """
...
class TaskStateConfig(Base):
    """Resident per-session Task State feature."""

    enabled: bool = True


class RenderToolConfig(Base):
    """Visual normalization and multimodal preview tools."""

    enabled: bool = True
    backend: Literal["auto", "direct", "boxlite"] = "auto"
    worker_image: str = ""
    worker_cpus: int = Field(default=2, ge=1)
    worker_memory_mib: int = Field(default=4096, ge=512)
    worker_create_timeout_seconds: int = Field(default=300, ge=1)
    allow_network: bool = False
    network_allowlist: list[str] = Field(default_factory=list)
    office_backend_order: list[Literal["onlyoffice", "libreoffice", "graph"]] = Field(
        default_factory=lambda: ["onlyoffice", "libreoffice"]
    )
    allow_cloud_office: bool = False
    max_input_bytes: int = Field(default=100 * 1024 * 1024, ge=1)
    max_asset_bytes: int = Field(default=200 * 1024 * 1024, ge=1)
    max_output_bytes: int = Field(default=1024 * 1024 * 1024, ge=1)
    max_pages: int = Field(default=200, ge=1, le=200)
    raster_dpi: int = Field(default=144, ge=72, le=600)
    preview_max_edge: int = Field(default=2048, ge=256, le=8192)
    default_preview_count: int = Field(default=6, ge=1, le=12)
    max_preview_count: int = Field(default=12, ge=1, le=12)
    max_inline_preview_bytes: int = Field(default=8 * 1024 * 1024, ge=1)
    default_capture_duration_seconds: float = Field(default=3.0, ge=0.25, le=10.0)
    max_capture_duration_seconds: float = Field(default=10.0, ge=0.25, le=10.0)
    motion_fps: int = Field(default=8, ge=1, le=30)
    ...
    ffprobe_path: str = ""
    qpdf_path: str = ""
    max_side_pixels: int = Field(default=8192, ge=256)
    max_total_pixels: int = Field(default=500_000_000, ge=1)
    max_animation_frames: int = Field(default=1000, ge=2)
    browser_ready_seconds: float = Field(default=10.0, ge=0.1)
    spreadsheet_viewport_width: int = Field(default=1600, ge=320)
    spreadsheet_viewport_height: int = Field(default=1000, ge=240)
    spreadsheet_max_viewports: int = Field(default=4, ge=1, le=24)

    @model_validator(mode="after")
    def _validate_render_limits(self) -> "RenderToolConfig":
        if self.default_preview_count > self.max_preview_count:
            raise ValueError("defaultPreviewCount cannot exceed maxPreviewCount")
        if self.default_capture_duration_seconds > self.max_capture_duration_seconds:
...
class ToolsConfig(Base):
    ...
    render: RenderToolConfig = Field(default_factory=RenderToolConfig)

...
- transient ``<task_state>`` projections. Task State lives outside the
  persisted Session and its sidecar is not part of a trajectory bundle, so
  replay cannot reconstruct it faithfully.
...
divergence rather than being re-matched. Task State tools replay their recorded
results but do not mutate a live Task State sidecar.
...
def _is_task_state_projection(message: Any) -> bool:
...
def compare_llm_request(
    ...
    recorded_msgs = [message for message in recorded_msgs if not _is_task_state_projection(message)]
    live_msgs = [message for message in live_msgs if not _is_task_state_projection(message)]

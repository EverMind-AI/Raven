...
class TaskStateTool(Tool):
    ...
    def __init__(self, manager: TaskStateManager):
        ...
        self._session_key: ContextVar[str | None] = ContextVar("task_state_session_key", default=None)
        self._mutation_group: ContextVar[str | None] = ContextVar("task_state_mutation_group", default=None)
    ...
    def set_context(self, session_key: str, mutation_group: str | None = None) -> None:
        self._session_key.set(session_key)
        self._mutation_group.set(mutation_group)
    ...
    async def execute(self, operations: list[dict[str, Any]], **extra: Any) -> str:
        ...
        try:
            result = self._manager.apply(
                session_key,
                operations,
                mutation_group=self._mutation_group.get(),
            )

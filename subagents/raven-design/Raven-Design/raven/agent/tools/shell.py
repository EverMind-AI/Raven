...
class ExecTool(Tool):
    ...
    _STOP_INSTRUCTION = (
        " Abandon THIS blocked operation: do not retry it or reproduce its "
        "effect through another command, tool, script, interpreter, or "
        "equivalent method. The block applies only to this operation - the "
        "rest of your task is unaffected, so continue it via approaches that "
        "do not need the blocked action (e.g. skip the cleanup/probe, leave "
        "temp files in place) instead of stopping or asking."
    )

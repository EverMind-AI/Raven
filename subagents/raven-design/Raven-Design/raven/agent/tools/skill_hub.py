...
``read_skill`` is source-agnostic. It resolves local, builtin, and Everos
bodies from the registry and fetches Hub bodies remotely. Exact builtin reads
are trusted because their content ships with Raven; mutable or remote sources
retain the normal untrusted tool-result boundary.
...
- ``local/<name>`` / ``builtin/<name>`` / ``everos/<id>`` — already
  materialized on disk (Everos skills are written to
  ``<workspace>/skills/everos/<id>/`` by the evolver); resolve the skill dir
  via the registry and return its ``scripts/`` path. No download.
...
class ReadSkillTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "the catalog (e.g. 'builtin/my-skill' or 'hub/my-skill'). This does "
            "NOT download or run anything — it only returns the instructions."
        )
            ...
            "properties": {
                "skill_id": {
                    ...
                    "description": (
                        ...
                        "'# Skills' catalog brackets (e.g. "
                        "'builtin/my-skill' or 'hub/my-skill')."
                    ),
                },
            },
            "required": ["skill_id"],
    ...
    async def execute(self, skill_id: Any = None, **_: Any) -> str | ToolResult:
        ...
        if source in ("local", "builtin", "everos"):
            meta = self._resolve_on_disk_meta(source, native)
            if meta is None:
                return f"Error: no {source} skill {native!r} found."
            body, _ = resolve_refs(meta.content, meta.path.parent)
            result = f"## {meta.name}\nSkill directory: {meta.path.parent}\n\n{body}"
            if source == "builtin":
                return ToolResult(model_text=result, trusted=True)
            return result

        if source != "hub":
            return (
                f"Error: unknown skill source {source!r} in {skill_id!r} "
                "(expected one of local/builtin/everos/hub)."
            )
    ...
    def _resolve_on_disk_meta(self, source: str, native: str) -> Any | None:
        if self._registry is None:
            return None
        if source == "local":
            return self._registry.get(native, source="local") or self._registry.get(native)
        return self._registry.get(native, source=source)
...
class UseSkillTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "from the '# Skills' catalog (e.g. 'builtin/x', 'local/x', "
            "'everos/x', or 'hub/x'). Returns the SKILL.md body plus a "
            "'scripts_dir' path when the skill ships runnable files. "
            "Pure-instruction skills need no scripts — just follow the body; "
            "you only need this for skills whose SKILL.md references files "
            "under scripts/."
        )
    ...
    def _use_on_disk(self, source: str, native: str) -> str | ToolResult:
        """Resolve an already-materialized on-disk skill dir."""
        if self._registry is None:
            meta = None
        elif source == "local":
            meta = self._registry.get(native, source="local") or self._registry.get(native)
        else:
            meta = self._registry.get(native, source=source)
        if meta is None:
            return (

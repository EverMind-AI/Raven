...
from raven.task_state import TaskStateManager
...
class AgentLoop:
    ...
    def __init__(
        ...
        from raven.config.schema import AskUserToolConfig, ContextCompactionConfig, ExecToolConfig
        from raven.token_wise.registry import StrategyRegistry
        ...
        self.context_compaction = context_compaction_config or ContextCompactionConfig()
        self.brave_api_key = brave_api_key
        self.jina_api_key = jina_api_key
        self.web_proxy = web_proxy
        from raven.config.schema import DeepResearchToolConfig, MediaGenConfig, RenderToolConfig

        self.media_config = media_config or MediaGenConfig()
        self.deep_research_config = deep_research_config or DeepResearchToolConfig()
        self.render_config = render_config or RenderToolConfig()
        ...
        self.task_state_enabled = task_state_enabled
        self.task_state = TaskStateManager(workspace)
        ...
        # Hub endpoint is configured; on-disk read/use remains available.
    ...
    def _register_default_tools(self) -> None:
        ...
        if self.task_state_enabled:
            self.tools.register(TaskStateTool(self.task_state))
        self.render_service = None
        if self.render_config.enabled:
            from raven.agent.tools.render import PreviewFileTool, RenderFileTool
            from raven.config.paths import get_media_dir, get_runtime_subdir
            from raven.rendering.service import RenderService

            self.render_service = RenderService.from_tool_config(
                self.render_config,
            ...
            self.tools.register(
                RenderFileTool(
                    self.render_service,
                    self.render_config.default_capture_duration_seconds,
                )
            )
            self.tools.register(
                PreviewFileTool(
        ...
        # Skill retrieval tools. Both resolve on-disk skills through the
        # registry; the shared Hub client adds remote fetch/install when
        # configured.
    ...
    def _project_task_state(
        ...
        """Append one current Task State snapshot to a transient message copy."""
        if not self.task_state_enabled or not session_key:
            return messages

        block = self.task_state.render(session_key)
        projected = [dict(message) for message in messages if not self._is_task_state_projection(message)]
        projected.append({"role": "assistant", "content": block})
        return projected

    @staticmethod
    def _is_task_state_projection(message: dict[str, Any]) -> bool:
        content = message.get("content")
        return (
            message.get("role") in {"system", "developer", "user", "assistant"}
            and isinstance(content, str)
            and content.lstrip().startswith("<task_state>")
            and content.rstrip().endswith("</task_state>")
        )

    def _task_state_completion_notice(self, session_key: str) -> str | None:
        if not self.task_state_enabled or not session_key:
            return None
        state = self.task_state.get(session_key)
        if state is None:
            return None
        unfinished = [
            (number, item["status"])
    ...
    async def _assemble_context_messages(
        ...
        *,
        ...
        budget = self._make_token_budget(selected_skills)
        if getattr(self, "task_state_enabled", False) and session_key:
            task_state_tokens = estimate_prompt_tokens(
                [{"role": "assistant", "content": self.task_state.render(session_key)}]
            )
            budget = TokenBudget(
                context_length=budget.context_length,
                reserved_output=budget.reserved_output,
                reserved_tools=budget.reserved_tools,
                reserved_system=budget.reserved_system + task_state_tokens,
                available_history=max(0, budget.available_history - task_state_tokens),
            )
    ...
    def _set_tool_context(
        ...
        for name in ("message", "spawn", "cron", "deep_research", "update_task_state"):
    ...
    def _next_request_token_estimate(
        self,
        messages: list[dict[str, Any]],
        tools: list[dict[str, Any]] | None,
        response: LLMResponse,
        iteration_input_length: int,
    ) -> int:
        ...
        if prompt_tokens <= 0:
            return estimated
        cache_tokens = int(response.usage.get("cache_read_input_tokens", 0) or 0) + int(
            response.usage.get("cache_creation_input_tokens", 0) or 0
        )
        if cache_tokens and prompt_tokens < cache_tokens:
            prompt_tokens += cache_tokens
        completion_tokens = int(response.usage.get("completion_tokens", 0) or 0)
        appended = messages[iteration_input_length:]
        generated_assistant = [
            message
            for message in appended
            if message.get("role") == "assistant" and not self._is_task_state_projection(message)
        ]
        runtime_messages = [
            message
            for message in appended
            if message.get("role") != "assistant" or self._is_task_state_projection(message)
        ]
        reported = (
            prompt_tokens
            + max(completion_tokens, estimate_prompt_tokens(generated_assistant))
            + estimate_prompt_tokens(runtime_messages)
        )
        return max(estimated, reported)
    ...
    async def _maybe_compact_context(
        ...
        *,
        ...
        reserved_output = max(
            int(getattr(getattr(self.provider, "generation", None), "max_tokens", 0) or 0),
            config.summary_min_tokens,
        )
        context_window = max(1, self.context_window_tokens)
        ...
        if current_tokens < thresholds.trigger_tokens:
            return messages, False, False

        plan = make_compaction_plan(
            messages,
            bootstrap_length=bootstrap_length,
            min_recent_iterations=config.min_recent_iterations,
            tail_token_budget=thresholds.tail_tokens,
            user_token_budget=config.max_user_tokens,
            is_task_state=self._is_task_state_projection,
        )
        if plan is None:
            logger.warning(
                "Context compaction could not find an older complete iteration at {} tokens",
                current_tokens,
            )
            return messages, False, False

        estimated_before = estimate_prompt_tokens(messages, active_tools)
        estimation_offset = max(0, current_tokens - estimated_before)
        pruned, elided = elide_old_tool_outputs(messages, before_index=plan.tail_start)
        latest_task_state = next(
            (dict(message) for message in reversed(messages) if self._is_task_state_projection(message)),
            None,
        )
        if latest_task_state is not None:
            pruned = [message for message in pruned if not self._is_task_state_projection(message)]
            pruned.append(latest_task_state)
        pruned_tokens = estimate_prompt_tokens(pruned, active_tools) + estimation_offset
        if elided and pruned_tokens <= thresholds.target_tokens:
            logger.info(
                "Context compaction pruned {} old tool result(s): {} -> {} tokens",
                elided,
                current_tokens,
                pruned_tokens,
            )
            return pruned, True, True

        available_output = max(
            256,
            context_window - current_tokens - config.safety_margin_tokens,
        )
        desired_output = max(
            config.summary_min_tokens,
            (current_tokens - thresholds.target_tokens) // 3,
        )
        summary_token_budget = max(
            256,
            min(config.summary_max_tokens, desired_output, available_output),
        )
        summary_source = [
            *(dict(message) for message in messages),
            {"role": "user", "content": compaction_prompt(summary_token_budget)},
        ]

        try:
            summary_messages, summary_tools, _ = await self.strategies.before_llm_call(
                summary_source,
            ...
            if config.summary_reasoning_effort != "inherit":
                call_kwargs["reasoning_effort"] = config.summary_reasoning_effort
            response = await self.provider.chat_with_retry(**call_kwargs)
            await self.strategies.after_llm_call(
                ...
                self._build_usage_snapshot(response, model or self.model, ""),
            )
            summary = self._strip_think(response.content)
            if (
                response.finish_reason == "error"
                or response.has_tool_calls
                or not summary
                or not is_valid_compaction_summary(summary)
            ):
                raise RuntimeError(f"unusable compaction response ({response.finish_reason})")
        except Exception as exc:
            logger.warning("Context compaction summary failed: {}", exc)
            shrunk, fallback_elided = self._emergency_shrink(messages)
            return shrunk, True, fallback_elided > 0

        replacement = plan.replacement(summary, latest_task_state)
        replacement_tokens = estimate_prompt_tokens(replacement, active_tools) + estimation_offset
        savings_ratio = 1 - replacement_tokens / max(1, current_tokens)
        if replacement_tokens >= current_tokens or savings_ratio < config.min_savings_ratio:
            logger.warning(
                "Context compaction summary saved too little: {} -> {} tokens",
                current_tokens,
                replacement_tokens,
            )
            shrunk, fallback_elided = self._emergency_shrink(messages)
        ...
        if replacement_tokens > thresholds.target_tokens:
            reduced, reduced_count = self._emergency_shrink(replacement)
            if reduced_count:
                replacement = reduced
                replacement_tokens = estimate_prompt_tokens(replacement, active_tools) + estimation_offset
        logger.info(
            "Context compacted {} message(s): {} -> {} tokens",
            plan.compressed_messages,
            current_tokens,
            replacement_tokens,
        )
    ...
    async def _synthesize_final_on_exhaustion(
        ...
        on_token_delta: Callable[[str], Awaitable[None]] | None = None,
        on_reasoning_delta: Callable[[str], Awaitable[None]] | None = None,
        task_state_session_key: str | None = None,
        response_continuation: Any | None = None,
    ) -> str:
        ...
        """
        state_key = task_state_session_key or ""
        projected = [dict(message) for message in messages]
        if state_key and getattr(self, "task_state_enabled", False):
            projected = self._project_task_state(projected, state_key)
        synth_messages = projected + [{"role": "user", "content": _MAX_ITER_SYNTHESIS_PROMPT}]
        if state_key and getattr(self, "task_state_enabled", False) and self.task_state.unfinished_numbers(state_key):
            synth_messages[-1]["content"] += (
                " The current Task State is authoritative: do not claim the overall task "
                "is complete while any item is unfinished; clearly report the remaining "
                "pending, waiting, or blocked work."
            )
        try:
            continuation_context = getattr(self.provider, "response_continuation", None)
            context = continuation_context(response_continuation) if callable(continuation_context) else nullcontext()
            with context:
    ...
    async def _run_agent_loop(
        ...
        """
        session_prefix = [dict(message) for message in initial_messages]
        messages = [dict(message) for message in initial_messages]
        provider_bootstrap_length = len(messages)
        iteration = 0
        final_content = None
        tools_used: list[str] = []
        session_key = extraction_session_id or ""
        state_session_key = task_state_session_key or ""
        if state_session_key:
            messages = self._project_task_state(messages, state_session_key)
        projected_state_revision = (
            self.task_state.revision(state_session_key) if self.task_state_enabled and state_session_key else None
        )
        effective_model = model or self.model
        create_continuation = getattr(self.provider, "create_response_continuation", None)
        response_continuation = create_continuation(effective_model) if callable(create_continuation) else None
        ...
        while iteration < self.max_iterations:
            ...
            iteration_input_length = len(messages)
            ...
            if response.has_tool_calls:
                ...
                for tool_call_index, tool_call in enumerate(response.tool_calls):
                    ...
                    if self.context_compaction.enabled:
                        model_text = truncate_tool_output(
                            model_text,
                            self.context_compaction.max_tool_result_chars,
                        )
                    trusted_tool_result = bool(getattr(result, "trusted", False))
                ...
                if projected_state_revision is not None:
                    current_revision = self.task_state.revision(state_session_key)
                    if current_revision != projected_state_revision:
                        messages.append(
                            {
                                "role": "assistant",
                                "content": self.task_state.render(state_session_key),
                            }
                        )
                        projected_state_revision = current_revision
                if (
                    self.context_compaction.enabled
                    and compaction_attempts < self.context_compaction.max_compactions_per_turn
                    and iteration >= next_compaction_iteration
                ):
                    current_tokens = self._next_request_token_estimate(
                        messages,
                        call_tools,
                        response,
                        iteration_input_length,
                    )
                    messages, attempted, changed = await self._maybe_compact_context(
                        messages,
                        bootstrap_length=provider_bootstrap_length,
                        current_tokens=current_tokens,
                        model=call_model,
                        active_tools=call_tools,
                    )
                    if attempted:
                        compaction_attempts += 1
                        next_compaction_iteration = iteration + self.context_compaction.retry_after_iterations
                    if changed:
                        response_continuation = (
                            create_continuation(call_model) if callable(create_continuation) else None
                        )
        ...
        if final_content is None and iteration >= self.max_iterations:
            ...
            final_content = await self._synthesize_final_on_exhaustion(
                ...
                on_token_delta=on_token_delta,
                on_reasoning_delta=on_reasoning_delta,
                task_state_session_key=state_session_key,
        ...
        turn_messages = [
            dict(message)
            for message in messages[provider_bootstrap_length:]
            if not self._is_task_state_projection(message)
        ]
        messages = [*session_prefix, *turn_messages]
        ...
        notice = self._task_state_completion_notice(state_session_key)
        if final_content and notice:
            final_content = f"{final_content}\n\n{notice}"
            if on_token_delta is not None:
                await on_token_delta(f"\n\n{notice}")
    ...
    async def _process_message(
        ...
        if cmd == "/new":
            ...
            self.task_state.clear(session.key)
        ...
        task_state_mutation_group = uuid.uuid4().hex
        if self.task_state_enabled:
            self.task_state.begin_turn(key, task_state_mutation_group)
        ...
        if task_state_tool := self.tools.get("update_task_state"):
            task_state_tool.set_context(key, task_state_mutation_group)

...
def make_provider(config: Config):
    ...
    configurable_responses_unsupported = client not in {"", "codex"} or bool(
        spec and spec.is_oauth and client != "codex"
    )
    if p and p.protocol == "responses" and configurable_responses_unsupported:
        raise MissingCredentialsError(
            f"provider {provider_name!r} does not support the configurable Responses protocol",
    ...
    if client == "codex":
        ...
        protocol = p.protocol if p else "chat"
        ...
        def make_inner(ep):
            ...
            if protocol == "responses":
                ...
                if provider_name != "openai" and not api_base:
                    raise MissingCredentialsError(
                        f"provider {provider_name!r} requires api_base for the Responses protocol",
                        provider=provider_name or "",
                    )
        ...
        if len(eps) > 1:
            ...
            provider = make_inner(
                ResolvedEndpoint(
                    label="default",
                    api_key=p.effective_api_key if p else "",

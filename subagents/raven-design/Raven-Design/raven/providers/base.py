...
_RESPONSE_CONTINUATION: ContextVar[Any | None] = ContextVar("response_continuation", default=None)
...
class LLMProvider(ABC):
    ...
    def create_response_continuation(self, model: str | None = None) -> Any | None:
        del model
        return None

    @contextmanager
    def response_continuation(self, state: Any | None) -> Iterator[None]:
        token = _RESPONSE_CONTINUATION.set(state)
        try:
            yield
        finally:
            _RESPONSE_CONTINUATION.reset(token)

    @property
    def response_continuation_state(self) -> Any | None:
        return _RESPONSE_CONTINUATION.get()

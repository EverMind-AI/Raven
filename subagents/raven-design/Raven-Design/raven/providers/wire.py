...
def wire_model(
    ...
    *,
    ...
    client rather than LiteLLM. Its own storage prefix comes off before the
    request is sent. ``spec`` selects the same convention for fixed dedicated
    clients; the LiteLLM path deliberately ignores it and asks the model id
    instead, since an id may name a provider other than the configured one and

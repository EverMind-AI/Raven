...
class LazyProvider(LLMProvider):
    ...
    def create_response_continuation(self, model: str | None = None) -> Any | None:
        return self._built().create_response_continuation(model)

    def supports_native_tool_result_images(self, model: str | None = None) -> bool:
        return False if self._provider is None else self._provider.supports_native_tool_result_images(model)

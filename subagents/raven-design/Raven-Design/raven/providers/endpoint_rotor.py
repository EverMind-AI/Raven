...
class EndpointRotorProvider(LLMProvider):
    ...
    def create_response_continuation(self, model: str | None = None) -> Any | None:
        """Disable endpoint-bound response IDs when every call intentionally rotates."""
        if self.strategy == "round_robin":
            return None
        return self._inners[0].create_response_continuation(model)

    def supports_native_tool_result_images(self, model: str | None = None) -> bool:
        return all(inner.supports_native_tool_result_images(model) for inner in self._inners)

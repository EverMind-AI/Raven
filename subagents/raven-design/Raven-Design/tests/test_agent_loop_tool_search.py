...
@pytest.fixture
def loop_factory(workspace):
    loops = []

    def make(cfg, strategies=None):
        loop = _make_loop(workspace, cfg, strategies=strategies)
        loops.append(loop)
        return loop
...
def test_enabled_registers_meta_tools(loop_factory) -> None:
    loop = loop_factory(ToolSearchConfig(enabled=True))
...
def test_strategy_registered_first(loop_factory) -> None:
    ...
    loop = loop_factory(ToolSearchConfig(enabled=True), strategies=registry)
    names = [s.name for s in loop.strategies.strategies]
    assert names[0] == "tool_search", f"tool_search must run first, got {names}"
    assert "marker" in names


def test_disabled_registers_nothing(loop_factory) -> None:
    loop = loop_factory(ToolSearchConfig(enabled=False))
    for name in ("tool_search", "tool_call"):
        assert not loop.tools.has(name)
    assert loop.strategies.get("tool_search") is None


def test_none_config_registers_nothing(loop_factory) -> None:
    loop = loop_factory(None)
    assert not loop.tools.has("tool_search")
    assert loop.strategies.get("tool_search") is None


def test_read_skill_registers_without_a_hub_endpoint(loop_factory) -> None:
    loop = loop_factory(None)

    assert loop.tools.has("read_skill")


@pytest.mark.asyncio
async def test_enabled_loop_keeps_interaction_primitives_visible(loop_factory) -> None:
    ...
    loop = loop_factory(ToolSearchConfig(enabled=True, compaction_threshold=5))
    ...
    assert {"read_file", "message", "ask_user", "spawn", "read_skill"} <= names, "primitives must stay visible"

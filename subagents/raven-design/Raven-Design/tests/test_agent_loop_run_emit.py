...
class _TrustedTool(_SplitTool):
    @property
    def name(self) -> str:
        return "trustedtool"

    async def execute(self, **kwargs) -> ToolResult:
        return ToolResult(model_text=self.MODEL_TEXT, trusted=True)
...
async def test_trusted_tool_result_reaches_the_context_boundary(tmp_path):
    provider = _FakeStreamToolProvider(
        [
            [
                StreamDelta(
                        ...
                        "tool_calls": [
                            {
                                "index": 0,
                                "id": "trusted-1",
                                "function": {"name": "trustedtool", "arguments": "{}"},
                            }
                        ]
                    },
                )
    ...
    loop = AgentLoop(provider=provider, workspace=tmp_path)
    _stub_edges(loop)
    loop.tools.register(_TrustedTool())
    original_add = loop.context.add_tool_result
    calls: list[dict] = []

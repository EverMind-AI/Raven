...
def test_provider_protocol_defaults_to_chat_and_accepts_responses() -> None:
    assert ProviderConfig().protocol == "chat"
    assert ProviderConfig.model_validate({"protocol": "responses"}).protocol == "responses"


def test_provider_protocol_rejects_an_unknown_value() -> None:
    with pytest.raises(ValidationError):
        ProviderConfig.model_validate({"protocol": "completions"})
...
def test_task_state_is_enabled_by_default() -> None:
    from raven.config.schema import TaskStateConfig, ToolsConfig

    assert TaskStateConfig().enabled is True
    assert ToolsConfig().task_state.enabled is True

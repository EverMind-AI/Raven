...
class TestDefaults:
    ...
    def test_skill_forge_public_defaults(self) -> None:
        ...
        assert isinstance(c.visual_domain_selector, VisualDomainSelectorConfig)
        assert c.visual_domain_selector.enabled is True
        assert c.visual_domain_selector.preferred_max == 2
        assert c.visual_domain_selector.alternatives_max == 3
...
class TestKeyAliasing:
    ...
    def test_visual_domain_selector_accepts_camel_and_snake_keys(self) -> None:
        camel = SkillForgeConfig.model_validate(
            {
                "visualDomainSelector": {
                    "preferredMax": 1,
                    "alternativesMax": 4,
                    "maxTokens": 2048,
                }
        ...
        snake = SkillForgeConfig.model_validate(
            {"visual_domain_selector": {"enabled": False}}
        )

        assert camel.visual_domain_selector.preferred_max == 1
        assert camel.visual_domain_selector.alternatives_max == 4
        assert camel.visual_domain_selector.max_tokens == 2048
        assert snake.visual_domain_selector.enabled is False

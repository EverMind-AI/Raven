...
class _StubInner(LLMProvider):
    ...
    def create_response_continuation(self, model: str | None = None) -> object:
...
def test_response_continuation_is_only_enabled_for_sticky_routing() -> None:
    e0 = _StubInner("e0")
    e1 = _StubInner("e1")

    sticky = _make_rotor([e0, e1], strategy="sticky")
    round_robin = _make_rotor([e0, e1], strategy="round_robin")

    assert sticky.create_response_continuation("m") is e0.continuation
    assert round_robin.create_response_continuation("m") is None

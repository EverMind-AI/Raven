...
from raven.agent.tools.render import PreviewFileTool
from raven.agent.tools.tool_search import ToolCallTool, ToolSearchController
from raven.rendering.models import Detection, RenderConfig, RenderOutcome
...
async def test_registry_preserves_media_for_model_and_uses_metadata_for_display() -> None:
    ...
    assert output.display_text == '{"views":[{"image":1,"page":2}]}'
...
@pytest.mark.asyncio
async def test_tool_search_indirect_call_preserves_preview_media() -> None:
    registry = ToolRegistry()
    registry.register(PreviewFileTool(_PreviewService(), 3.0))
    ...
    output = await registry.execute(
        "tool_call",
        {"name": "preview_file", "arguments": {"path": "report.docx"}},
    )
...
def test_preview_file_schema_enforces_preview_hard_limit() -> None:
    ...
    assert tool.validate_params({"path": "report.docx", "max_previews": 13}) == ["max_previews must be <= 12"]

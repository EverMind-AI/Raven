...
class _ContinuationProvider(LLMProvider):
    def __init__(self) -> None:
        super().__init__(api_key="test")
        self.state = object()
        self.seen: list[object | None] = []

    def create_response_continuation(self, model: str | None = None):
        return self.state

    async def chat(self, messages, tools=None, model=None, **kwargs):
        self.seen.append(self.response_continuation_state)
        if len(self.seen) == 1:
            return LLMResponse(
                content=None,
                tool_calls=[ToolCallRequest(id="call_1", name="no_such_tool", arguments={})],
                finish_reason="tool_calls",
            )
...
@pytest.mark.asyncio
async def test_one_response_continuation_is_scoped_across_the_turn(workspace):
    provider = _ContinuationProvider()
    agent = _make_agent(workspace, provider)

    final, _used, _messages, outcome = await agent._run_agent_loop([{"role": "user", "content": "go"}])

    assert final == "finished"
    assert outcome.status == "completed"
    assert provider.seen == [provider.state, provider.state]
    assert provider.response_continuation_state is None

...
def test_protocol_flag_is_discovered_and_persisted(tmp_config: Path) -> None:
    r = runner.invoke(app, ["provider", "set", "openai", "--protocol", "responses"])

    assert r.exit_code == 0, r.output
    data = json.loads(tmp_config.read_text(encoding="utf-8"))
    assert data["providers"]["openai"]["protocol"] == "responses"

...
def test_packaged_skill_result_can_use_the_explicit_trusted_path(tmp_path: Path) -> None:
    b = ContextBuilder(workspace=tmp_path)
    body = "Follow this packaged procedure."

    messages = b.add_tool_result(
        [],
        "call-1",
        "read_skill",
        body,
    ...
    assert "BEGIN UNTRUSTED" not in messages[0]["content"]
...
def test_identity_text_carries_anti_injection_clause(tmp_path: Path) -> None:
    ...
    assert "exact `builtin/<name>` id" in text
    assert "only trusted tool-result exception" in text

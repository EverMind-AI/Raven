...
async def test_an_unconfigured_subsystem_follows_the_conversation(tmp_path) -> None:
    """The selector and curator are unpinned here (no credentials for the
    ...
    async def _body(*args, **kwargs):
        seen["selector"] = skills._visual_domain_selector._binding()[1]
        seen["curator"] = curator.curator_model
        return "done"

    await _run(loop, "tui:a", _body)
    assert seen["selector"] == "vendor-a/model"
...
async def test_a_configured_subsystem_uses_its_own_pair(tmp_path) -> None:
    ...
    skills._visual_domain_selector._pin = pin
    curator._pin = pin
    ...
    await _run(loop, "tui:a", _body)
    assert seen["selector"] == "vendor-pin/small"
    assert seen["curator"] == "vendor-pin/small"


def test_the_selector_sends_no_model_outside_a_turn(tmp_path) -> None:
    """An unpaired pin must never go out on the fallback provider's key. With
    no turn bound there is nothing to follow, so the selector asks the provider
    for its own default rather than posting a model id it has no credential for.
    """
    loop = _loop(tmp_path)
    skills = next(b for b in loop.context_engine._builders if isinstance(b, SkillsSegmentBuilder))
    skills._visual_domain_selector._model = "openai/gpt-5-mini"
    skills._visual_domain_selector._pin = None

    assert active_binding() is None
    assert skills._visual_domain_selector._binding()[1] is None
...
def test_a_configured_gate_pin_survives_a_real_factory_build(tmp_path) -> None:
    ...
    loop = AgentLoop(
        ...
        skill_forge_config=SkillForgeConfig(
            visual_domain_selector=VisualDomainSelectorConfig(enabled=False),
            llm_gate_model="openai/gpt-5-mini",
        ),
        provider_pool=ProviderPool(cfg),
    )
    skills = next(b for b in loop.context_engine._builders if isinstance(b, SkillsSegmentBuilder))

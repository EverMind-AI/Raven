...
def _invoke_agent_capturing_session(
    ...
    class _StubAgentLoop:
        def __init__(self, **kwargs):
            ...
            captured["task_state_enabled"] = str(kwargs.get("task_state_enabled"))
    ...
    # The -m path hard-exits via os._exit(0) (native finalization guard); make it a
    # catchable SystemExit so the CliRunner sees a clean exit instead of the
    # whole pytest process dying.
    def _hard_exit(code: int) -> None:
        captured["hard_exit_code"] = str(code)
        raise SystemExit(code)

    monkeypatch.setattr(_os, "_exit", _hard_exit)
...
def test_agent_continue_without_prior_session_starts_fresh(
    ...
    assert captured["task_state_enabled"] == "True"

...
class TestSplitQualifiedId:
    def test_source_prefixed(self) -> None:
        ...
        assert _split_qualified_id("builtin/x") == ("builtin", "x")
...
class TestReadSkill:
    ...
    async def test_builtin_body_is_trusted_and_resolves_references(self, tmp_path: Path) -> None:
        meta = _meta(
            tmp_path,
            "visual",
            "Read [patterns](references/patterns.md).",
            with_scripts=False,
        )
        references = meta.path.parent / "references"
        references.mkdir()
        (references / "patterns.md").write_text("patterns", encoding="utf-8")
        reg = _FakeRegistry({("builtin", "visual"): meta})

        out = await ReadSkillTool(registry=reg).execute(skill_id="builtin/visual")

        assert out.trusted is True
        assert str(references / "patterns.md") in out.model_text
        assert "Skill directory:" in out.model_text

    async def test_unknown_source_errors_before_hub_fetch(self) -> None:
        client = _FakeClient()

        out = await ReadSkillTool(client=client).execute(skill_id="weird/x")
...
class TestUseSkill:
    ...
    async def test_builtin_skill_result_is_trusted(self, tmp_path: Path) -> None:
        meta = _meta(tmp_path, "visual", "packaged procedure", with_scripts=False)
        reg = _FakeRegistry({("builtin", "visual"): meta})

        out = await UseSkillTool(registry=reg).execute(skill_id="builtin/visual")

        assert out.trusted is True
        assert "packaged procedure" in out.model_text

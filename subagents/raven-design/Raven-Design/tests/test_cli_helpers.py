...
def test_make_provider_responses_protocol_uses_thin_api_key_provider(tmp_path: Path) -> None:
    from raven.config.loader import load_config
    ...
    assert provider.api_base == "http://localhost:9000/v1"
    assert provider.extra_headers == {"X-Test": "yes"}


def test_make_provider_responses_rejects_an_oauth_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    from raven.config.schema import Config
    from raven.providers.auth import MissingCredentialsError

    config = Config.model_validate(
        {
            "providers": {"github_copilot": {"protocol": "responses"}},
            "agents": {
                "defaults": {
    ...
    with pytest.raises(MissingCredentialsError, match="does not support"):
        _helpers.make_provider(config)


def test_make_provider_openai_responses_uses_the_sdk_default_base(monkeypatch: pytest.MonkeyPatch) -> None:
    from raven.config.schema import Config
    from raven.providers.openai_responses_provider import OpenAIResponsesProvider

    config = Config.model_validate(
        {
            "providers": {"openai": {"apiKey": "sk-x", "protocol": "responses"}},
            "agents": {
                "defaults": {
...
def test_make_provider_responses_requires_a_base_for_non_openai_provider(
    ...
    config = Config.model_validate(
        {
            "providers": {"anthropic": {"apiKey": "sk-x", "protocol": "responses"}},
            "agents": {
                "defaults": {
    ...
    with pytest.raises(MissingCredentialsError, match="requires api_base"):
        _helpers.make_provider(config)
...
def test_make_provider_builds_responses_inners_for_every_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    from raven.config.schema import Config
    from raven.providers.endpoint_rotor import EndpointRotorProvider
    from raven.providers.openai_responses_provider import OpenAIResponsesProvider
    ...
    config = Config.model_validate(
        {
            "providers": {
                "custom": {
                    "protocol": "responses",
                    "endpoints": [
                        {"label": "a", "apiKey": "k1", "apiBase": "https://a.example/v1"},
                        {"label": "b", "apiKey": "k2", "apiBase": "https://b.example/v1"},
                    ],
                }
            },
            "agents": {"defaults": {"model": "my-model", "provider": "custom"}},
    ...
    assert isinstance(provider, EndpointRotorProvider)
    assert all(isinstance(inner, OpenAIResponsesProvider) for inner in provider._inners)
    assert provider.supports_native_tool_result_images("my-model") is True

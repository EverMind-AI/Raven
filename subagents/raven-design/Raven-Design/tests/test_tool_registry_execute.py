...
async def test_plain_string_tools_carry_no_display_text():
    ...
    assert getattr(result, "trusted", False) is False


@pytest.mark.asyncio
async def test_trusted_marker_survives_the_registry_boundary():
    reg = _registry(_Split("packaged procedure", None, trusted=True))

    result = await reg.execute("split", {})

    assert result == "packaged procedure"
    assert result.trusted is True  # type: ignore[attr-defined]
...
def test_tool_output_is_a_str_subclass():
    ...
    assert ToolOutput("text").trusted is False

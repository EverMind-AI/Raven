...
def test_switch_reaches_the_real_holders_a_loop_builds(tmp_path) -> None:
    ...
    assert skills._visual_domain_selector._fallback_provider is new_provider
...
def test_switch_reaches_legacy_skillforge_holders_when_selector_is_disabled(tmp_path) -> None:
    loop = AgentLoop(
        provider=_Provider(),
        workspace=tmp_path,
        model="fake/model",

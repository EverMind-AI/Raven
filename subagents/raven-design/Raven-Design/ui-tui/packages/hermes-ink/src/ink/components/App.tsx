...
export default class App extends PureComponent<Props, State> {
  ...
  // Tracks whether Kitty keyboard / modifyOtherKeys protocol has been pushed
  // to the terminal. First push attempt is env-sniffed at raw-mode-enable;
  // XTVERSION reply may trigger a late re-push when env-sniff missed
  // (typically over SSH). Idempotent — at most one push per App instance.
  extendedKeysPushed = false
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // latter).
        const _supportsExtKeys = supportsExtendedKeys()
        if (_supportsExtKeys) {
          this.props.stdout.write(ENABLE_KITTY_KEYBOARD)
          this.props.stdout.write(ENABLE_MODIFY_OTHER_KEYS)
          this.extendedKeysPushed = true
        }

        // Diagnostic dump of terminal-capability detection state, guarded by
        // RAVEN_TUI_DEBUG_TERMINFO=1. Useful when modifier+enter behavior
        // regresses on a new terminal/transport stack — set the env var,
        // launch the TUI, then inspect ~/.raven/logs/tui-node-diag.log.
        if (process.env.RAVEN_TUI_DEBUG_TERMINFO === '1') {
          try {
            const diagPath = join(homedir(), '.raven', 'logs', 'tui-node-diag.log')
            const line = JSON.stringify({
              ts: new Date().toISOString(),
              phase: 'raw-mode-enable',
              TERM_PROGRAM: process.env.TERM_PROGRAM ?? null,
              TERM: process.env.TERM ?? null,
              ZELLIJ: process.env.ZELLIJ ? 'set' : 'unset',
              WT_SESSION: process.env.WT_SESSION ? 'set' : 'unset',
              KITTY_WINDOW_ID: process.env.KITTY_WINDOW_ID ? 'set' : 'unset',
              TMUX: process.env.TMUX ? 'set' : 'unset',
              STY: process.env.STY ? 'set' : 'unset',
              CURSOR_TRACE_ID: process.env.CURSOR_TRACE_ID ? 'set' : 'unset',
              'env.terminal': env.terminal,
              'supportsExtendedKeys()': _supportsExtKeys,
              kittyPushedAtEnable: _supportsExtKeys
            })
            appendFileSync(diagPath, line + '\n', { mode: 0o600 })
          } catch {
            // diag write failure must not break TUI startup
          }
        }
        ...
        setImmediate(() => {
          ...
          void Promise.all([this.querier.send(xtversion()), this.querier.flush()]).then(([r]) => {
            ...
            if (r) {
              ...
              // Late re-push: env-sniff at raw-mode-enable may have missed
              // an extended-keys-capable terminal (notably over SSH where
              // TERM_PROGRAM is stripped). XTVERSION reply travels through
              // the pty and can identify zellij / xterm.js / etc. — push
              // the protocol now so subsequent keystrokes report modifiers.
              // First keystroke between mount and XTVERSION reply is still
              // pre-push; users rarely hit modifier+enter as their first
              // keystroke so the race window is tolerable in practice.
              if (!this.extendedKeysPushed && isExtendedKeysCapableByXtversion(r.name)) {
                this.props.stdout.write(ENABLE_KITTY_KEYBOARD)
                this.props.stdout.write(ENABLE_MODIFY_OTHER_KEYS)
                this.extendedKeysPushed = true
                rePushed = true
            ...
            if (process.env.RAVEN_TUI_DEBUG_TERMINFO === '1') {
              try {
                ...
                const line = JSON.stringify({
                  ...
                  xtversionName: r?.name ?? null,
                  replyReceived: Boolean(r),
                  rePushed,
                  extendedKeysPushedAfter: this.extendedKeysPushed
                })
                appendFileSync(diagPath, line + '\n', { mode: 0o600 })
              } catch {
                // diag write failure must not break TUI startup

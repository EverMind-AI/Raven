...
The resident, per-session record of a task's goal, requirements, ordered items, and
item statuses. The Agent Loop projects one snapshot into the Turn bootstrap and appends
a new revision after a Tool batch only when the state changed. Context Compaction folds
those transient projections down to the latest revision, and they stay outside the
persisted Session. The Agent Loop appends an incomplete-state notice to final replies
while any item remains unfinished. The agent mutates Task State only through
`update_task_state`, addressing items by the visible one-based `item_number`; stored
items have no hidden identifier.
_Avoid_: "Todo list" -- Task State also carries persistent task requirements and
participates in runtime completion checks.

**Context Compaction** (`agent/loop/compaction.py`):
The same-Turn reduction of accumulated Iterations before the next LLM call. It retains
the Turn bootstrap, real user constraints, a token-budgeted tail of complete Tool-call
groups, and the latest Task State while replacing older work with one Compaction Summary.
The Session remains append-only: only the compacted representation of the current,
not-yet-persisted Turn is appended when the Turn finishes.
_Avoid_: "Checkpoint" -- Checkpoint is the workspace snapshot, not context reduction.

**Compaction Summary**:
Assistant-authored working memory created by Context Compaction. Its active user contract
must be traceable to real user messages and cannot broaden authorization; retained real
user messages override it on conflict.
_Avoid_: "user instruction" -- the summary carries context, not new user authority.
...
message, ask_user, spawn (Subagent), MCP, media generation, visual rendering,
and skill read/use.
...
its `execute` under a timeout, returning text, multimodal blocks, or a structured error.

**Render Service** (`rendering/`):
The shared per-Agent-Loop visual normalization service behind `render_file` and
`preview_file`. It detects HTML, SVG, Office, and animated-image inputs, dispatches
them to one configured execution backend, and produces the same PDF/page/preview
outcome for persistent bundles or inline multimodal previews.
_Avoid_: calling either Tool the renderer — the Tools are facades over this service.
...
(Visual Domain Selector cards in Raven-Design plus SkillForge-routed non-domain
bodies), and `# Curator Working State` (Segment 6).
...
**Visual Domain Selector** (`memory_engine/skill_forge/visual_domain_selector.py`):
Raven-Design's query-time LLM classifier over the complete `SKILL.md` bodies of
the fixed 15 packaged visual-domain Skills. It returns **Preferred Skill Cards**
plus optional **Alternative Skill Cards** to the main Agent's `# Skills` segment;
no domain body is included there, and the Agent loads one only by calling
`read_skill` with its exact `builtin/<name>` id. A selector failure exposes the
full description catalog as alternatives and marks the turn degraded. The
general SkillForge route continues beside it for public, workspace, EverOS, and
Hub Skills after excluding the 15 domain names; the always-on
`visual-artifact-design` Skill remains in `# Active Skills`.
_Avoid_: conflating it with the SkillForge LLM Gate — the gate filters retrieved
multi-source bodies, while this selector compares one fixed full-body domain catalog.

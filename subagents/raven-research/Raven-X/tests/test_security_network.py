...
def test_non_strict_dns_allows_unresolvable_host(monkeypatch: pytest.MonkeyPatch) -> None:
    """A resolver failure is not a refusal for a caller that never connects itself."""
    import socket
    ...
    ok, err = net.validate_url_target("https://nx.example.invalid/", strict_dns=False)
    assert ok, f"unexpectedly blocked: {err}"
    assert err == ""


def test_non_strict_dns_still_blocks_private_ip(monkeypatch: pytest.MonkeyPatch) -> None:
    """The relaxation covers resolver failure only, never a name that resolves inward."""
    _mock_resolve(monkeypatch, "169.254.169.254")

    ok, err = net.validate_url_target("https://totally-public.example/x", strict_dns=False)
    assert not ok
    assert "private/internal" in err

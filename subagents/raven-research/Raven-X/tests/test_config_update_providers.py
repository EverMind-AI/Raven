...
from raven.providers.auth import copilot_token_dir, credential_files
...
def test_oauth_credential_is_read_from_ravens_own_directory(
    ...
    _write_credential(credential_files(slug)[0], slug)
...
def test_codex_detection_asks_the_module_that_speaks_for_the_driver(oauth_home: Path) -> None:
    ...
    assert credential_files("openai_codex")[0] == auth_file()
...
def test_probing_copilot_reads_its_own_credential_not_another_providers(
    ...
    assert oauth_home in oauth_dir().parents
    codex_dir = oauth_dir() / "chatgpt"
    codex_dir.mkdir(parents=True, exist_ok=True)

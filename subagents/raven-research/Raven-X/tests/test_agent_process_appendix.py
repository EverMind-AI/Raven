...
def test_the_warning_separates_a_fabricated_link_from_one_merely_left_unopened():
    """★ 20260828. Both branches, and the number that must not move between them.
    ...
    answer = (
        "opened https://a.example/one , listed https://listed.example/two , "
        "invented https://ghost.example/three"
    )
    ...
    # Opposite direction: every cited link opened -> neither warning, and the
    # rate stays 1.0 rather than being pushed off by the new subset.
...
def test_a_truncated_citation_of_a_listed_link_is_unopened_not_fabricated():
    ...
    ledger = [
        {"op": "search", "query": "q", "replay": False, "zero_hit": False, "urls": [listed]},
    ]
...
def test_the_rendered_trail_leads_with_the_summary_and_folds_the_detail():
    ...
    # ★ 20260901 (Framework): "distinct" reworded. The folding rule is unchanged -
    # ``distinct_queries`` still counts two - only the noun stops claiming the two
    # strings were two different things looked for.
...
def test_an_answer_that_cites_nothing_is_not_rendered_as_perfect():
    ...
    ★ 20260901 (Framework): the invariant under test is that a zero-citation answer
    never renders as verified, and it is asserted as an invariant rather than as one
    sentence, because the sentence now depends on *why* there were no citations.
    """
...
def test_the_generic_nothing_to_check_line_survives_when_no_page_was_read():
    """The original wording is still the right one when the run really did nothing.

    ★ 20260901 (Framework). Separating the causes must not delete the case it was
    written for: a turn that opened no page and cited no link has an inapplicable
    check for the ordinary reason, and accusing it of citing nothing after reading
...
def test_reading_pages_and_citing_nothing_is_called_out():
    """★ 20260901 (Framework). The state the grounding check cannot see into."""
    c = build_trail(LEDGER, "no links here at all").counters()
    assert c["read_but_cited_nothing"] is True
    assert c["cited_schemeless"] == 0
    assert c["citation_grounding_rate"] is None
    assert "1 page(s) were read but the answer cites no link" in build_trail(
        LEDGER, "no links here at all"
    ).render()


def test_a_scheme_less_citation_is_disclosed_without_moving_the_rate():
    """★ 20260901 (Framework). The whole point of the second extractor.

    A product run wrote all 43 of its references as bare hosts, so ``_URL_RE`` saw
    none of them and the appendix reported that there was nothing to check. That is
    a generation-side formatting problem wearing the costume of an answer that cited
    nothing, and the two need opposite responses. Widening ``_URL_RE`` was refused:
    it would move ``citation_grounding_rate``'s denominator under an unchanged name.
    """
    # Real TLDs on purpose: ``.example`` is reserved for documentation and adding it
    # to the production pattern to make a test pass would be fitting the code to the
    # fixture. The disclosure counter does not consult the ledger, so these hosts need
    # not appear in ``LEDGER`` - it reports how the answer was written, nothing more.
    t = build_trail(LEDGER, "evidence: kyutai.org/blog/arc and github.com/x/y (web_fetch #ab12)")
    c = t.counters()

    # The landed metric is untouched: no scheme, no citation, rate undefined.
    assert c["urls_cited"] == 0
    assert c["citation_grounding_rate"] is None
    assert c["cited_not_opened"] == 0 and c["cited_never_surfaced"] == 0
    # And the cause is now on the record.
    assert c["cited_schemeless"] == 2
    assert "without a URL scheme" in t.render()


def test_a_scheme_ful_citation_is_never_counted_twice():
    """★ 20260901 (Framework). ``arxiv.org/x`` inside ``https://arxiv.org/x`` is one
    citation, and a disclosure counter that double-counts the well-behaved case would
    fire on every correct answer - which is how a counter gets ignored."""
    t = build_trail(LEDGER, "see https://a.example/one")
...
def test_the_scheme_less_pattern_does_not_fire_on_ordinary_prose():
    """★ 20260901 (Framework). Both directions, because a disclosure counter that
    cries wolf costs more than not having it. A path separator is required precisely
    so version numbers, figure references and ratios are not hosts."""
...
def test_research_seconds_spans_the_ledger_not_the_turn():
    """★ 20260901 (Framework). Derived from ``ts``, and its caliber is pinned here.

    It measures first research event to last, so it excludes the final generation. That
    is a definition, not a defect, and pinning it stops the gap from being read as one.
    """
    rows = [
    ...
    # A single row has no span, and 0 would read as "instant" rather than "unknown".
    one = build_trail([rows[0]], "x")
    assert one.counters()["research_seconds"] is None
    # Rows without a usable ``ts`` do not fabricate a span.
    assert build_trail(
        [{"op": "search", "query": "q"}, {"op": "search", "query": "r"}], "x"
    ).counters()["research_seconds"] is None


def test_a_sub_minute_research_span_is_not_rendered():
    """★ 20260901 (Framework). Both directions: half an hour is a fact the reader is
    entitled to, forty seconds is a clause nobody needs."""
    short = [{"op": "search", "query": "q", "ts": 0.0},
             {"op": "fetch", "url": "https://a.org/x", "chars": 9000, "ok": True, "ts": 40.0}]
    long_ = [{"op": "search", "query": "q", "ts": 0.0},
...
def test_thin_pages_are_counted_not_only_rendered():
    """★ 20260901 (Framework). ``ok`` is a transport verdict, not a content one.

    Four OpenReview fetches in one product run each returned 440 characters of
    JavaScript shell and all four counted as ``pages_ok``. The thin-page split was
    rendered from dr@2.8 on but never emitted, so every fetch-productivity reading
    downstream was computed over a number that cannot tell a page from a wrapper.
    """
    rows = [
        {"op": "search", "query": "q", "replay": False, "zero_hit": False},
    ...
    c = t.counters()
    assert c["pages_ok"] == 2  # unchanged: the stub really was retrieved
    assert c["thin_pages"] == 1
    assert c["pages_failed"] == 1
    # And it reaches the summary line, where "2 pages read" was the misleading half.
...
# ── ★ 20260902: fence-tag citations, and the unreviewed banner ─────────
#
# One product run wrote 75 of its 87 citation handles as the security fence's
# nonce - ``(web_fetch #cd8187b0)`` - and the grounding check, seeing no URL,
# said there was nothing to check over an answer whose every reference pointed
# at a page the run had really opened. Both features below exist to keep a
# true sentence from being read as a passing grade: the fence-tag counter gives
# "no links were cited" its cause, and the banner keeps "the reviewer never saw
# this" from hiding in one word of the head line.


def test_a_fence_tag_citation_is_disclosed_without_moving_the_rate():
    """Same contract as ``cited_schemeless``: the landed metric cannot move.

    The tag is NOT resolved to a URL - ``wrap_untrusted``'s nonce never reaches
    the ledger, and a disclosure counter that guessed would accuse or absolve on
    data it does not have. Repeats of one tag are one reference."""
    t = build_trail(
        LEDGER,
        'halves it ("quote" web_fetch #cd8187b0), again (web_fetch #cd8187b0), '
        "listed at (web_search #aa11bb22)",
    )
    c = t.counters()
...
def test_the_fence_tag_pattern_needs_the_tool_name():
    ...
    assert build_trail(LEDGER, noise).counters()["cited_fence_tags"] == 0
...
def test_a_web_search_tag_is_never_reported_as_a_web_fetch():
    """The rendered example quotes what the answer wrote, tool name included.

    The extractor deliberately accepts both tools' tags, and the first rendering
    hard-coded every match as ``web_fetch #...`` - so an answer citing only a
    ``web_search`` fence made the integrity appendix state an event that did not
    occur. An integrity line that misstates what it discloses is its own
    counterexample; both render branches now quote the stored form."""
...
def test_an_unreviewed_run_wears_its_banner_before_the_head():
    ...
    rows = [r for r in LEDGER if r["op"] != "verify"] + [
        {"op": "verify", "outcome": "budget_spent", "reviewed": False}
    ]
...
def test_a_truncated_fence_tag_is_still_a_fence_tag():
    """The ``{4,16}`` width, which nothing else here pins.

    ``token_hex(4)`` mints exactly 8, so a pattern of ``{8}`` looks right and
    passes every other test in this file. The model is quoting the tag out of
    context though, and a clipped quote is still a fence tag - not a citation
    this check could suddenly read. Narrowed to ``{8}`` the counter reports 0
    and the appendix falls back to "cited nothing" over an answer whose every
    reference points at a really opened page."""
...
def test_a_page_cited_both_with_and_without_its_scheme_is_one_reference():
    """The ``k in norm_cited`` exclusion, which nothing else here pins.

    ``_norm`` drops the scheme, so a reference list writing the URL in full and
    prose naming the same page bare fold to one key. Without the exclusion the
    disclosure counter reports a scheme-less reference for a page the answer
    cited properly - the false positive whose cost this module states as its
    reason for keeping the TLD list closed.

    The host needs a TLD ``_SCHEMELESS_RE`` actually lists; ``.example`` is not
    one, so the ledger URLs above cannot show this."""

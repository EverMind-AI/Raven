...
def test_context_window_reads_config_not_hardcoded_200k(fake_agent_loop, config) -> None:
    ...
    # Sanity check: the regression target is the old hardcoded stub, not any
    # particular window. Pinning the literal made every change to the schema
    # default fail this test by construction - the same shape
    # test_agent_flow_dr.py already had to unwind for drFlow.version. The
    # assertion that carries meaning is "it is the configured window and it is
    # not the stub"; the exact default belongs to schema.py, which is where a
    # reader would look for it anyway.
    window = config.agents.defaults.context_window_tokens
    assert window != 200_000, "the old stub 200000 is back"

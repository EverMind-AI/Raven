...
# The module docstring's "runs without boxlite" is not true: 22 of these tests
# reach code that imports it at call time. boxlite lives in the optional
# `sandbox` extra, which this DR-only fork does not install, so skip rather than
# fail - and do not install it, because this venv is the one the evaluation
# subprocesses import from.
pytest.importorskip("boxlite", reason="optional `sandbox` extra is not installed")

from raven.sandbox.debug_server import (  # noqa: E402
    SandboxDebugServer,

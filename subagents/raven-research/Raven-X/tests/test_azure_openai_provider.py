...
async def test_a_non_200_renders_the_canonical_error_content(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert resp.finish_reason == "error"
    assert (resp.content or "").startswith("Error calling LLM (auth@azure_openai): "), resp.content
    assert "invalid subscription key" in (resp.content or "")
...
async def test_a_raised_exception_renders_the_canonical_error_content(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert resp.finish_reason == "error"
    assert (resp.content or "").startswith("Error calling LLM (network@azure_openai): "), resp.content
    assert "connection reset by peer" in (resp.content or "")
...
def test_an_unparseable_response_shape_renders_the_canonical_error_content() -> None:
    ...
    assert resp.finish_reason == "error"
    assert re.match(r"^Error calling LLM \([a-z_]+@azure_openai\): ", resp.content or ""), resp.content

...
from raven.agent.flow.verify import _CONSTRAINT_RUBRIC, _ELISION_RUBRIC, DraftReviewerGate
...
def test_a_corpus_contained_subagent_registry_has_no_subprocess_tool():
    """manager.py builds its own registry, which neither the parent's disabledTools
    nor drFlow.toolsAllowlist reaches. Containment there has to be a capability
    rule, and a rule that only exists inside a coroutine cannot be verified."""
    ...
    free = SubagentManager(provider=_P(), workspace=Path("/tmp/ws"))
    assert "exec" in free._build_subagent_tools(None).names()

    # dr@2.3: the previous two lines were the whole test, which pinned "a sub-agent gets
    # exec even when the parent denies it" as expected behaviour. The surface must be a
    # subset of the parent's, so a parent that denies exec must deny it here too.
    fenced = SubagentManager(
        provider=_P(),
    ...
    assert "exec" in fenced.last_surface_dropped

...
#
# SF11: a construction-time caller resolves a window with ``allow_fetch=False``
# before anything has imported LiteLLM. Reaching for the import here on the
# main thread defeats the whole point of deferring it. ``import_litellm()`` was
# already forced at module load (see the top of this file) so LiteLLM is always
...
def test_allow_fetch_false_never_reaches_a_seam_that_could_fetch(monkeypatch):
    """``resolve_context_window(..., allow_fetch=False)`` must answer from the
    cache tiers alone: the fetch seam (``_fetch_openrouter_models``) forwards
    the flag, so a stub that fails loudly on a live fetch proves the flag
    arrived rather than being swallowed on the way down."""

    def _stub(*, allow_fetch: bool = True) -> dict:
        assert allow_fetch is False, "allow_fetch=False was dropped on the way to the fetch seam"
        return {}

    monkeypatch.setattr(rates, "_fetch_openrouter_models", _stub)

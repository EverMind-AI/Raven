...
# --------------------------------------------------------------------------- #
# Report pool: queued -> in-progress -> ready | failed                         #
# --------------------------------------------------------------------------- #


@pytest.mark.asyncio
async def test_report_pool_lifecycle(monkeypatch):
    mgr = _make_manager(max_concurrent=1)
    ...
    async def _stub_inner(task_id, task, label, origin, executor) -> None:
        await release.wait()
        await mgr._announce_result(task_id, label, task, "found the answer", origin, "ok")

    monkeypatch.setattr(mgr, "_run_subagent_inner", _stub_inner)

    await mgr.spawn("research task one", session_key="s1")
    await mgr.spawn("research task two", session_key="s1")

    await _settle(lambda: any(r["status"] == "in-progress" for r in mgr.report_pool("s1")))
    statuses = sorted(r["status"] for r in mgr.report_pool("s1"))
    assert statuses == ["in-progress", "queued"]

    release.set()
    await _settle(lambda: all(r["status"] == "ready" for r in mgr.report_pool("s1")))

    reports = mgr.report_pool("s1")
    assert len(reports) == 2
    assert all("found the answer" in r["result"] for r in reports)
    assert all("UNTRUSTED subagent" in r["result"] for r in reports)


@pytest.mark.asyncio
async def test_report_pool_marks_failures(monkeypatch):
    mgr = _make_manager(max_concurrent=1)
    mgr.set_submit(lambda req: None)
    monkeypatch.setattr(manager_mod, "build_executor", lambda *a, **k: _DummyExecutor())

    async def _boom(task_id, task, label, origin, executor) -> None:
        raise RuntimeError("subagent crashed")

    monkeypatch.setattr(mgr, "_run_subagent_inner", _boom)

    await mgr.spawn("doomed task", session_key="s1")
    await _settle(lambda: mgr.report_pool("s1") and mgr.report_pool("s1")[0]["status"] == "failed")
    report = mgr.report_pool("s1")[0]
    assert "subagent crashed" in report["result"]


def test_report_pool_filters_by_session_and_returns_copies():
    mgr = _make_manager(max_concurrent=1)
    mgr._reports["a1"] = {"task_id": "a1", "label": "x", "session_key": "s1", "status": "ready", "result": "r"}
    mgr._reports["b1"] = {"task_id": "b1", "label": "y", "session_key": "s2", "status": "queued", "result": None}

    s1 = mgr.report_pool("s1")
    assert [r["task_id"] for r in s1] == ["a1"]
    s1[0]["status"] = "mutated"
    assert mgr._reports["a1"]["status"] == "ready"
    assert len(mgr.report_pool()) == 2


def test_report_pool_prunes_terminal_entries_per_session():
    mgr = _make_manager(max_concurrent=1)
    mgr._REPORT_POOL_MAX_TERMINAL_PER_SESSION = 2
    for i in range(4):
        mgr._reports[f"t{i}"] = {
            "task_id": f"t{i}",
            "label": "x",
    ...
    mgr._reports["live"] = {"task_id": "live", "label": "x", "session_key": "s1", "status": "queued", "result": None}

    mgr._prune_reports("s1")

    ids = [r["task_id"] for r in mgr.report_pool("s1")]
    assert ids == ["t2", "t3", "live"]


def test_web_tools_inherit_corpus_endpoint():
    """A fixed-corpus benchmark run is void if a subagent reaches the live web.

    The subagent tool registry is built here rather than inherited from the
    parent loop, so the endpoint has to be threaded through explicitly.
    """
    mgr = SubagentManager(
        provider=_StubProvider(),
        workspace=Path("/tmp"),
    ...
    search = WebSearchTool(api_key=None, proxy=None, corpus_endpoint=mgr.web_corpus_endpoint)
    fetch = WebFetchTool(api_key=None, proxy=None, corpus_endpoint=mgr.web_corpus_endpoint)

    assert search.corpus_endpoint == "http://127.0.0.1:8765"
    assert fetch.corpus_endpoint == "http://127.0.0.1:8765"


def test_web_corpus_endpoint_defaults_to_none():

...
def test_windows_workflow_isolates_upgrade_test_from_shared_conftests() -> None:
    # This fork has no CI (AGENTS.md sec 3: private, local-only, no CI, no PRs), so
    # .github/workflows/ci.yml legitimately does not exist and this test cannot run.
    # Skip on genuine absence rather than fail - and do NOT satisfy it by creating a
    # workflow file, which would assert a CI pipeline that isn't there.
    ci = Path(__file__).parents[2] / ".github" / "workflows" / "ci.yml"
    if not ci.is_file():
        pytest.skip(f"no CI workflow in this fork ({ci} absent)")

...
def pytest_collection_modifyitems(config: Any, items: list[Any]) -> None:
    """``real_llm`` is opt-in: skip it unless the run asked for it by name.

    Two defects made these tests fail on every default `uv run pytest`:

    1. The marker's documented promise ("skipped unless the runtime is
       configured") was implemented only inside the ``everos_env`` fixture, so a
       ``real_llm`` test reaching the runtime by another route failed instead of
       skipping. A promise implemented in one fixture is a promise about that
       fixture, not about the marker.
    2. More importantly, "skip when unconfigured" is the wrong gate. On this box
       the runtime *is* configured (repo-root .env), so a plain full-suite run
       was making real, billable LLM calls - and still failing, because the
       per-test isolated root does not carry everything the runtime needs.

    So gate on intent, matching the header's own documented invocation
    (``uv run pytest tests/integration -m real_llm``): run them only when ``-m``
    mentions real_llm, or ``RAVEN_TEST_REAL_LLM=1`` is set.
    """
    import os

...
from raven.agent.hook.observers import BudgetObserver, DuplicateQueryObserver, LoopscanObserver
...
class TestAgentLoopHookWireUp:
    ...
    def test_default_composite_is_observer_trio(self, workspace):
        agent = _make_agent(workspace)
        hooks = list(agent.hooks)
        assert isinstance(agent.hooks, CompositeHook)
        assert len(hooks) == 3
        assert isinstance(hooks[0], LoopscanObserver)
        assert isinstance(hooks[1], BudgetObserver)
        assert isinstance(hooks[2], DuplicateQueryObserver)

    def test_empty_composite_when_observers_disabled(self, workspace):
        agent = _make_agent(workspace, loop_observers=[])
        assert isinstance(agent.hooks, CompositeHook)
        assert len(agent.hooks) == 0

    def test_on_user_inbound_only(self, workspace):
        agent = _make_agent(workspace, on_user_inbound=lambda m: None, loop_observers=[])
        hooks = list(agent.hooks)
        assert len(hooks) == 1
        assert isinstance(hooks[0], OnUserInboundAdapter)

    def test_decision_consumer_only(self, workspace):
        agent = _make_agent(workspace, decision_consumer=AsyncMock(), loop_observers=[])
        hooks = list(agent.hooks)
        assert len(hooks) == 1
        assert isinstance(hooks[0], DecisionConsumerAdapter)

    def test_response_modifier_only(self, workspace):
        agent = _make_agent(workspace, response_modifier=lambda k, c: c, loop_observers=[])
        hooks = list(agent.hooks)
        assert len(hooks) == 1
        assert isinstance(hooks[0], ResponseModifierAdapter)

    def test_canonical_ordering(self, workspace):
        """Order: loop observers → OnUserInbound → DecisionConsumer → [explicit hooks] → ResponseModifier."""
        ...
        assert isinstance(hooks[1], BudgetObserver)
        assert isinstance(hooks[2], DuplicateQueryObserver)
        assert isinstance(hooks[3], OnUserInboundAdapter)
        assert isinstance(hooks[4], DecisionConsumerAdapter)
        assert isinstance(hooks[5], Explicit)
        assert isinstance(hooks[6], ResponseModifierAdapter)

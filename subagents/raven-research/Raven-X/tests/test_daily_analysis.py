...
def _run(coro):
    # asyncio.run, not get_event_loop().run_until_complete: on 3.13 the latter
    # raises "no current event loop" unless some earlier test happened to leave
    # one installed, so these tests passed alone and failed in a full run purely
    # from collection order. asyncio.run owns its loop and closes it.

...
# ---------------------------------------------------------------------------
# 9. CompositeHook rollback halts the chain
# ---------------------------------------------------------------------------


class TestCompositeHookRollbackHaltsChain:
    """A rollback decision halts the phase chain exactly like a
    short-circuit: once one hook decides the iteration must be
    re-sampled, later opinions about a doomed response are moot."""

    async def test_rollback_halts_chain(self, ctx):
        log = []

        class Roller(AgentHook):
            async def after_iteration(self, c):
                log.append("roller")
                return HookDecision(rollback=True, rollback_overrides={"temperature": 1.0})
        ...
        composite = CompositeHook([Roller(), Recorder()])
        decision = await composite.after_iteration(ctx)

        assert decision.rollback is True
        assert decision.rollback_overrides == {"temperature": 1.0}
        assert log == ["roller"]

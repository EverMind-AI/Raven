...
def test_the_answer_tag_is_revealed_before_markdown_eats_it():
    """Rich renders the reply as Markdown, which drops ``<answer>`` as inline HTML.

    Observed on a product run: two thousand words of comparison ending on the bare
    word "DuckDB" with nothing marking it as the answer. Display only - the
    evaluation harness runs ``--no-markdown`` and reads ``final_answer`` from the
    trajectory, so no grader sees this path.
    """
    from raven.cli.agent_commands import _reveal_answer_tag

    out = _reveal_answer_tag("Long report body.\n\n<answer>DuckDB</answer>")
    assert "**Answer:** DuckDB" in out
    assert "<answer>" not in out
    # Nothing else moved: this is a reveal, not a rewrite.
    assert out.startswith("Long report body.")

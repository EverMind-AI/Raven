...
# The local _restore_event_loop fixture that used to live here is gone: it
# installed a brand-new loop before AND after every test, never closed either
# one and never restored the previous value - papering over the shared
# current-loop-slot bug for this module while leaking a loop per test.
# tests/conftest.py::_restore_current_event_loop snapshots and restores the
# slot for the whole suite instead.
...
def test_total_steps_is_four() -> None:
    # The progress dots + "Step n/N" header derive from this constant; a new
    # wizard step must bump it (and this pin) together.

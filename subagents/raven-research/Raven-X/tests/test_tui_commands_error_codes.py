...
def _patch_agent_loop_to_raise(monkeypatch: pytest.MonkeyPatch, exc: BaseException) -> None:
    ...
    # ``_build_tui_agent_loop`` calls make_provider() before it ever constructs
    # AgentLoop, and make_provider raises typer.Exit(1) when no provider
    # credentials are configured. Without this stub the function dies there, the
    # generic handler reports reason="uncaught"/exception_type="Exit", and the
    # test never reaches the subject it is asserting about - so it failed in any
    # unconfigured environment for a reason unrelated to AgentLoop.
    # Patch the source module, not tui_commands: make_provider is imported lazily
    # inside the function body (same reason the AgentLoop patch above targets
    # raven.agent.loop), so a tui_commands attribute would never be consulted.
    monkeypatch.setattr("raven.cli._helpers.make_provider", lambda *a, **k: object())

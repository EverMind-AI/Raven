...
class TestConnectMcpSandboxGuard:
    ...
    async def test_stdio_no_executor_does_not_raise(self):
        ...
        cfg.env = None
        cfg.tool_timeout = 30
        # connect will fail at stdio_client level, and that error is caught per-server and
        # logged - it must NOT be a SandboxInitError.
        #
        # The failure used to come from really launching `true` through the MCP stdio
        # client, which made this test flaky: anyio's cancel scope is entered inside the
        # client task and exited by the AsyncExitStack here, so under load it raised
        # "Attempted to exit cancel scope in a different task". Measured 1 pass / 2 fails

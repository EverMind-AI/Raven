...
async def test_synthesis_falls_back_to_static_on_error_finish():
    ...
    assert result == (_MAX_ITER_STATIC_FALLBACK.format(n=40), False)
...
async def test_synthesis_falls_back_to_static_on_empty_content():
    ...
    result = await synth([], "m", None)
    assert result == (_MAX_ITER_STATIC_FALLBACK.format(n=7), False)
...
# --------------------------------------------------------------------------- #
# Exhaustion labeling: wrap-up answers stay distinguishable from completions   #
# --------------------------------------------------------------------------- #


class _ToolThenSynthErrorProvider(_ToolThenSynthProvider):
    """Tools every iteration; infra dies at the synthesis cliff."""

    async def chat(self, messages, tools=None, model=None, **kwargs):
        if tools is None:
            self.synth_calls += 1
            return LLMResponse(content="503 overloaded", finish_reason="error")
        return await super().chat(messages, tools=tools, model=model, **kwargs)


@pytest.mark.asyncio
async def test_synthesized_reply_is_labeled(workspace):
    provider = _ToolThenSynthProvider()
    agent = _make_agent(workspace, provider)

    _final, _used, messages, outcome = await agent._run_agent_loop(
        [{"role": "user", "content": "do the thing"}],
    )

    assert outcome.status == "interrupted"
    assert outcome.synthesized_on_exhaustion is True
    assert messages[-1]["synthesized_on_exhaustion"] is True


@pytest.mark.asyncio
async def test_static_fallback_is_not_labeled_synthesized(workspace):
    provider = _ToolThenSynthErrorProvider()
    agent = _make_agent(workspace, provider)
    ...
    assert outcome.status == "interrupted"
    assert outcome.synthesized_on_exhaustion is False
    assert final == _MAX_ITER_STATIC_FALLBACK.format(n=2)
    assert "synthesized_on_exhaustion" not in messages[-1]

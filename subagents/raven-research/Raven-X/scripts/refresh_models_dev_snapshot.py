...
def resolve_inheritance(providers: dict[str, dict], canonical: dict[str, dict]) -> None:
    ...
    def resolve(ref: str, seen: frozenset[str]) -> dict:
        ...
        # `seen` stops a cycle from doing it forever. A cycle-truncated result
        # is returned but NOT cached: written into `resolved` it would answer
        # every later non-cycle lookup of the same ref with the unresolved row.
        if not base_ref or ref in seen:
            if ref not in seen:
                resolved[ref] = entry
            return entry
        merged = {**resolve(str(base_ref), seen | {ref}), **entry}
        resolved[ref] = merged
        return merged
...
def main() -> int:
    ...
    # Downloaded by the resolved sha, not by the branch name: two requests to a
    # moving ref can straddle a push, and then the recorded sha names a commit
    # these bytes did not come from -- unverifiable after the fact.
    tarball = f"https://codeload.github.com/{REPO}/tar.gz/{sha}"
    ...
    # Gate before writing: an oversized snapshot must never land in the working
    # tree (repo rule: no files over 1 MiB).
    size_mib = len(encoded.encode("utf-8")) / 1048576
    models = sum(len(v["models"]) for v in snapshot.values())
    if size_mib > 1:
        print(
            f"ERROR: snapshot would be {size_mib:.3f} MiB, over the 1 MiB gate; "
            "trim KEEP or PROVIDER_ALIASES (nothing written)",
            file=sys.stderr,
        )
        return 1

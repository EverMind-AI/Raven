...
# dr@3.2: the constant and its predicate moved to ``raven.agent.harness_text``,
# which owns every string the harness itself writes into a trajectory. Re-exported
# here unchanged so the existing import path (``raven.agent.context``) keeps
# working -- a rename would have touched five modules and two test files for no
# behavioural gain, and the point of the move is to have ONE place that answers
# "did the harness write this", not to relocate call sites.
from raven.agent.harness_text import (  # noqa: E402,F401
    TOOL_OUTPUT_ELIDED,
...
{skills_summary}""")
    ...
    def _get_identity(self) -> str:
        ...
        ``RAVEN_GUIDELINES`` is imported rather than repeated: this block used to
        exist here AND in ``context_engine.segments.render`` byte-identically, in
        two independently live prompt paths. See that module for why only the
        block is shared and the two enclosing prompts are not.
        """
        # Deferred, not module level: ``context_engine.factory`` imports this
        # module, so a top-level import here closes the cycle. Same pattern as
        # ``agent/flow/dr.py``, which imports ``_language_directive`` this way.
        from raven.context_engine.segments.render import RAVEN_GUIDELINES
...
Reply directly with text for conversations. Only use the 'message' tool to send to a specific chat channel."""
    ...
    def add_tool_result(
        ...
        # Creation-time stamp: persisted as-is by _save_turn (setdefault), so
        # trajectories carry real per-message wall clock instead of the single
        # end-of-turn save time. Stripped from wire requests by the provider
        # key allowlist, like flow_version.
        messages.append(
            {
                "role": "tool",
    ...
    def add_assistant_message(
        ...
        # is a tool-parser miss, not a genuine ``stop``. Stripped from wire
        # requests by the provider key allowlist, like ``timestamp``.

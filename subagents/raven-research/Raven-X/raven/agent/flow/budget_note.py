...
class BudgetNoteObserver(AgentHook):
    ...
    async def after_iteration(self, ctx: AgentHookContext) -> HookDecision:
        ...
        if used and self._context_window_tokens:
            ...
            # (``AgentLoop.__init__`` -> ``build_dr_flow(context_window_tokens=
            # self._window_for())``) this holds the window the turn actually runs on,
            # not the configured default - on a model the resolver knows, the two
            # differ by up to 15x and this line used to claim 110% of a window the
            # turn was nowhere near.
            #
            ...
            # already consumed, while ``_pre_call_fit`` trims on prompt tokens alone.
            # Both this note and the spin breaker divide by it, so moving it is a
            # distribution change owed its own label.

...
from raven.providers.registry import find_by_model, find_gateway, supports_prompt_caching
...
class LiteLLMProvider(LLMProvider):
    ...
    def supports_prompt_caching(self, model: str) -> bool:
        """Return True when this provider's endpoint honours ``cache_control``.

        Public because a ``TokenStrategy`` cannot see the gateway - it is handed
        only ``(messages, tools, model)`` - so the strategy stack asks the provider
        instead of re-deriving the answer from the model string. That re-derivation
        was the third copy of this predicate and the one that got the gateway wrong.
        """
        return supports_prompt_caching(model, gateway=self._gateway)

    def _supports_cache_control(self, model: str) -> bool:
        """Backwards-compatible alias for :meth:`supports_prompt_caching`."""
        return self.supports_prompt_caching(model)
    ...
    async def chat(
        ...
        kwargs: dict[str, Any] = {
            ...
            # default is 6000s — too long for an interactive agent loop. The
            # default (600s) covers slow reasoning-heavy generations while still
            # failing fast on stuck connections, so chat_with_retry can retry or
            # give up; batch runs tune it via agents.defaults.requestTimeoutSeconds.
            "timeout": self.generation.request_timeout_seconds,
        }
        ...
        if self.generation.repetition_penalty is not None:
            extra_body.setdefault("repetition_penalty", self.generation.repetition_penalty)
        if extra_body:
    ...
    async def chat_stream(
        ...
        kwargs: dict[str, Any] = {
            ...
            # For streams the client timeout acts as a max inter-chunk gap, so
            # a healthy long generation is never cut off — only a stalled
            # connection is. Without this the stream inherits LiteLLM's 6000s
            # default and a dead endpoint hangs the whole turn.
            "timeout": self.generation.request_timeout_seconds,
        }

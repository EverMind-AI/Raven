...
class LiteLLMProvider(LLMProvider):
    ...
    async def chat(
        ...
        if self.generation.repetition_penalty is not None:
            # Copy before writing: kwargs["extra_body"] may still be the
            # caller-owned dict handed to __init__, and an explicit value there
            # wins over the generation default.
            extra = dict(kwargs.get("extra_body") or {})
            extra.setdefault("repetition_penalty", self.generation.repetition_penalty)
    ...
    async def chat_stream(
        ...
        if max_tokens is not None:
            # Bounded by the model's own ceiling exactly as the non-streaming
            # path is (chat_with_retry calls send_max_tokens before chat): this
            # method is called directly by the agent loop, so the bound has to
            # live here or the same pin builds two different requests.
            kwargs["max_tokens"] = max(1, send_max_tokens(gen, model, pinned=int(max_tokens)))
    ...
    def _normalize_stream_chunk(self, chunk: Any) -> StreamDelta | None:
        ...
        try:
            ...
            if not choices:
                # stream_options.include_usage delivers the stream's final
                # usage snapshot on a trailing chunk with EMPTY choices;
                # returning None here dropped the only usage record the
                # stream carries.
                usage_dict = self._usage_dict(getattr(chunk, "usage", None))
                if usage_dict is None:
                    return None
                return StreamDelta(content=None, usage=usage_dict)

...
def format_llm_error(
    ...
    is machine-parseable so a rendering surface can show a diagnosis + fix hint
    instead of the raw exception; the detail drops duplicated exception-name
    prefixes and raw JSON error bodies.
    """
...
class TruncationInfo:
    ...
    def as_error(self, tool_name: str) -> str:
        ...
        Not wired in this fork: the refusal consumer (``Tool.truncation_hint``,
        registry-side refusal) is deliberately not ported, so this method has
        no caller and the verdict is recorded for logs only -- see
...
class StreamDelta:
    ...
    # The provider's finish_reason for the choice, carried on the chunk that
    # reports it (usually the last content chunk). Consumers must not infer it
    # from the presence of tool calls: "length" and "content_filter" are
    # decisions only the provider can report, and downstream truncation
...
class GenerationSettings:
    ...
    # Forwarded in the request body for OpenAI-compatible backends that
    # accept it (sglang / vLLM). None = never sent, so hosted APIs that
    # reject unknown body fields are unaffected.

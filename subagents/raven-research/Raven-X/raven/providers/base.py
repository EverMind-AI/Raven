...
class GenerationSettings:
    ...
    # Per-attempt hard cap on a single LLM request, forwarded as the HTTP
    # client timeout. For streaming this is the max gap between chunks, so
    # long generations that keep producing tokens are never cut off — only
    # dead connections are. Multiplied by the chat_with_retry ladder
    # (4 attempts), so batch runs against congested endpoints should lower
    # it well below (wall_clock / 4).

...
class EndpointRotorProvider(LLMProvider):
    ...
    def generation(self, value: GenerationSettings) -> None:
        ...
        ``provider.generation = GenerationSettings(...)`` from config --
        without this setter that assignment would land on the rotor alone and
        every inner would keep answering temperature/max_tokens/timeout from
        its own untouched default.
    ...
    def disable_auto_cache_control(self, value: bool) -> None:
        ...
        The caller assigns this on whatever ``make_provider`` returned, once it
        knows CacheOptimizer is placing the breakpoints. Landing it on the rotor
    ...
    def model_overrides(self) -> dict[str, dict[str, Any]]:
        ...
        answer, not one per endpoint. Without this, a caller reading

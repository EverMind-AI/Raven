...
def token_path(region: str) -> Path:
    from raven.config.paths import oauth_dir

    config = oauth_config(region)
    base_dir = os.environ.get(OAUTH_STORAGE_DIR_ENV)
    return (Path(base_dir) if base_dir else oauth_dir()) / f"{config.provider}.json"
...
def save_token(region: str, token: MiniMaxOAuthToken) -> None:
    from raven.config.paths import ensure_oauth_dir, oauth_dir

    path = token_path(region)
    # The default home gets the owner-only tightening every credential dir
    # needs; an explicit override dir is the operator's to permission.
    if path.parent == oauth_dir():
        ensure_oauth_dir()
    else:
    ...
    *,
    ...
    try:
        ...
        while int(time.time() * 1000) < deadline:
            ...
            # Exchanging by user_code looks like an RFC 8628 violation (the
            # token request should carry device_code) but is MiniMax's own
            # protocol: the independent hermes-agent and OpenClaw clients poll
            # with this exact body, and none of them reads a device_code off
            # the authorization response. The short code alone cannot be
            # sniped -- the exchange also requires code_verifier, which never
            # leaves this client.

...
def supports_prompt_caching(model: str, *, gateway: ProviderSpec | None = None) -> bool:
    """Return True when ``cache_control`` blocks are meaningful for this call.

    The ONE resolver. There were three near-copies of this predicate - here via
    ``LiteLLMProvider._supports_cache_control``, and once each in
    ``token_wise/cache_optimizer.py`` and ``token_wise/system_and_tail_cache.py`` -
    and they were not equivalent: only the provider's copy consulted the gateway.
    Point the ``custom`` slot at an Anthropic-compatible endpoint and the two
    disagreed, so whether breakpoints were written depended on which piece of code
    happened to ask.

    ``gateway`` wins when present because the gateway is what actually terminates
    the request. That asymmetry is the whole point: ``custom`` is a gateway spec
    declaring no caching, and it is how this build reaches the local SGLang
    endpoint - a Claude-looking model string routed through it must still resolve
    to False, which the model lookup alone would get wrong.
    """

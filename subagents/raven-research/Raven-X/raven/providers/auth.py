...
def copilot_token_dir() -> Path:
    ...
    return Path(token_dir).expanduser() if token_dir else oauth_dir() / "github_copilot"


def oauth_credentials_present(provider: str) -> bool:
    ...
    if provider == "github_copilot":
        ...
        # "holds something".
        try:
            return bool(credential_files(provider)[0].read_text(encoding="utf-8").strip())
        except OSError:
            return False

    return credential_files(provider)[0].exists()

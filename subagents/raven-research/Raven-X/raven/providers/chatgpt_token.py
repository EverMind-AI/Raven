...
def token_dir() -> Path:
    ...
    return Path(configured).expanduser() if configured else oauth_dir() / "chatgpt"
...
def clear_abandoned_device_code() -> bool:
    ...
    if data:
        ...
        # ``mkstemp`` creates the file 0600 from the first byte -- a write-then-
        # chmod pair leaves a window where the token sits at the umask's mercy.
        path = auth_file()
...
def access_token_and_account() -> tuple[str, str | None]:
    ...
    # Existence-checked first: assigning a name the class no longer has succeeds
    # silently and leaves the renamed real method reachable, which is exactly the
    # fifteen-minute device poll this stubbing exists to prevent.
    for name in ("_login_device_code", "_wait_for_access_token"):
        if not hasattr(authenticator, name):
            raise RuntimeError(
                f"litellm Authenticator no longer defines {name}; refusing to call "
                "get_access_token, which could start an interactive device flow"
            )
        setattr(authenticator, name, _refuse)

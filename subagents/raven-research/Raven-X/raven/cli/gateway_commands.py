...
def register(app: typer.Typer) -> None:
    ...
    def gateway(
        ...
        # Same TokenWise stack and the same reason as ``agent_commands``; see the
        # comment there for why the provider's probe is injected and why the
        # provider's own cache_control injection stands down.
        ...
        agent = AgentLoop(
            ...
            # ``dr_flow`` was absent here until 20260811, so a gateway session
            # silently ran stock Raven: no two-tool allowlist, no verify, no
            # spin breaker, no research trail. Silent because what goes missing
            # is an enhancement - an answer still comes back - and because
            # ``build_dr_flow`` returns None on a disabled config, which is
            # indistinguishable from never having been asked. The gateway is the
            # surface a product integration actually uses, and it was the one
            # surface the flow could not reach.

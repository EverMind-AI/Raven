...
class AgentDefaults(Base):
    ...
    """Fallback window, used only when the backend's real window cannot be resolved.

    dr@3.2: 65,536 -> 131,072. This is a FALLBACK, not a declaration: ``AgentLoop``
    resolves the served model's actual window and only lands here when the resolver
    does not know the model - which is the case for the served 35B, so on the bench
    axes this number IS the window.

    128K rather than 256K deliberately. 256K on a 30B-class open-weight model is
    usually rope-extended, i.e. nominal rather than usable, and a degraded tail shows
    up only as a worse score - it does not raise. 2x is also enough to answer the
    question that motivates the change (how much of the DR effect is window scarcity),
    at half the KV cost and far less of the superlinear attention penalty, which lands
    hardest on the arm that already runs ~+98% slower than the anchor.

    ⚠️ Raising this moves the trim point on BOTH arms - it is not gated on drFlow -
    so any batch crossing this line needs a fresh anchor pair, and readings cannot be
    ...
    # Per-attempt LLM request timeout (HTTP client timeout; for streams, the
    # max gap between chunks). The retry ladder multiplies this by up to 4x
    # worst case — batch runs against slow endpoints should lower it so one
    # stalled call cannot eat the whole wall-clock budget.
...
class WebToolsConfig(Base):
    ...
    The tool names and their input/output shapes stay identical, so the agent's
    action space — and therefore the comparability of a corpus run against a
    live-web run — is unchanged; only where the bytes come from differs. A URL
    outside the corpus returns an explicit "not in corpus" result rather than a
    network error, because under this protocol it is a rule, not a failure."""
    benchmark_containment: bool = False
    """Refuse to retrieve the evaluation set itself over live web.

    Only for axes that have no fixed corpus. Detecting dataset access after the
    run cannot replace this: a flow that cites its sources writes the leaked
    string into its own answer while an anchor that does not cite writes
    nothing, so leak-based item exclusion always drops more treated items than
    control ones. Prevention is arm-neutral; exclusion is not.

    Enabling it changes what every arm can read, the anchor included, so it
    ships as its own labelled version with a fresh anchor pair and is never set
    on a subset of a batch's arms."""

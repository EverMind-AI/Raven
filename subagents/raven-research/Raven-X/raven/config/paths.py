...
def get_product_ledger_dir() -> Path:
    """Return the per-turn research-trail directory used by the process appendix.

    Under the instance data dir rather than a fixed ``~/.raven`` so a second instance
    does not write its trails into the first one's directory. Batches never reach this:
    their launcher exports ``RAVEN_WEB_LEDGER`` and that path wins.
    """
    return get_runtime_subdir("ledger")
...
def oauth_dir() -> Path:
    """The directory every OAuth provider's credentials live in -- path only.
    ...
    Pure on purpose: a status probe or a path derivation must not create
    directories as a side effect. A writer calls :func:`ensure_oauth_dir` first.
    """
    return Path.home() / ".raven" / "oauth"


def ensure_oauth_dir() -> Path:
    """``oauth_dir()``, created and tightened to owner-only.

...
failure, and they moved into the checkout with the transport. Pinned here so the
...
def test_the_deck_is_copied_next_to_the_client(tmp_path):
    deck = _pptx(tmp_path / "out" / "deck.pptx")
    dest = tmp_path / "client"
    delivered = materials.deliver(deck, str(dest))
    assert delivered == dest / "deck.pptx"
    assert delivered.is_file()
    # The job keeps its own copy either way.
    assert deck.is_file()


def test_delivering_into_the_decks_own_directory_is_a_no_op(tmp_path):
    deck = _pptx(tmp_path / "out" / "deck.pptx")
    assert materials.deliver(deck, str(deck.parent)) == deck


def test_an_undeliverable_destination_is_reported_not_raised(tmp_path):
    """The turn already succeeded; a failed copy must not turn a published deck
    into an exception."""
    deck = _pptx(tmp_path / "out" / "deck.pptx")
    blocker = tmp_path / "a-file"
    blocker.write_text("not a directory", encoding="utf-8")
    assert materials.deliver(deck, str(blocker)) is None

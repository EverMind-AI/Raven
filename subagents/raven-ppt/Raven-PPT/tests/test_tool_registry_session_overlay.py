...
rather than a field. This surface builds an engine per session, so two parallel
dispatches already get two registries and the isolation looks free -- but a
promise that holds only because of how the engines happen to be built is one a
later change breaks silently, and the promise is on the wire
(``SESSION_MCP_CAPABILITY``). So it is asserted here against a single shared
registry, which is the stricter case: two turns, one registry, both live at once,
where a field would let whichever entered last decide what the other can see.

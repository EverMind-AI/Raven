...
from raven.utils.helpers import estimate_prompt_tokens, is_image_part, is_inline_image, labelled_images
...
class AgentLoop:
    ...
    def _register_ppt_tools(self) -> None:
        """Register the deck tools for the configured route, if any.

        Opt-in and self-contained: the whole capability lives under `raven/ppt/`
        and this is the only place the rest of the agent learns it exists. It
        returns an empty list when the extra is not installed, so an agent without
        python-pptx starts without deck tools rather than failing to start.

        The predecessor did not do it this way. It replaced the agent's identity
        text, its bootstrap file loader and its active-skill allowlist with
        PPT-only versions, which is why that fork could only make decks. Here the
        route's own profile names the skill it wants and nothing else moves.
        """
        config = self._ppt_config

...
Everything else in this package answers the client. This is the one direction
that asks it something and waits, which a permission prompt and a question both
need and neither could have: ``Frames`` is fire-and-forget and ``AcpMethods``
is inbound-only, so before this there was no mechanism at all -- ``methods.py``
answered every response frame by logging that nothing here asks.
...
* **The id space is this agent's.** A client's request ids and an agent's are
  ...
  frames. One map for both would let a client's own request id resolve a promise
  this agent is holding.
* **Every wait ends.** A client is free to never answer. A pending future with no
  deadline is a turn that never finishes, so every call carries a timeout and the
  timeout is a normal outcome rather than an error.
* **Shutdown is an answer.** When the connection closes, everything still waiting
  is failed rather than left to the garbage collector, because the code awaiting
  it has cleanup to run.
"""
...
Long, because what is on the other side of these requests is a person reading a
prompt rather than a program answering one. Finite, because a client that has
stopped answering must not hold a turn open forever.
"""
...
class OutboundRequests:
    ...
    ``emit`` writes one finished frame. Synchronous for the same reason the
    outlet's send is: the frame writer is a write plus a flush with no suspension
    ...
    async def call(
        ...
        *,
        ...
        close is the session ending, and a timeout is a client that is still there
        and not answering.
        """
        if self._closed:
        ...
        try:
            ...
            # still holding whatever it put in front of a person -- an elicitation
            # form, a permission sheet -- with no way to learn that. This
            # notification is the only way to tell it, and it is one-way: a client
            # that ignores it is no worse off than before. Not sent for the other
            # two exits, which need no retraction: an error means the client
            # already answered, and a close means it is gone.
            if not self._closed:
            ...
            # both leave the entry behind otherwise, and a late response would then
    ...
    def resolve(self, frame: dict[str, Any]) -> bool:
        ...
        ``False`` means the frame answers no request this agent sent. Not an error
        -- a client may answer a request it invented, or one this agent already
        timed out -- but the caller wants to know, because a frame that belongs to
        nobody is otherwise indistinguishable from one that was handled.
        """
        request_id = frame.get("id")
        if not isinstance(request_id, int) or isinstance(request_id, bool):
            # Every id this agent mints is a plain int. A string id in a response
            # can only be a client answering something it made up, or echoing an id
            # in the wrong type -- and either way there is nothing here to match it
            # against.
            return False

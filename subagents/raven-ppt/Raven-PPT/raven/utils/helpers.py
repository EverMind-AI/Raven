...
def labelled_images(blocks: list[Any]) -> list[Any]:
    """The images from a block list, each still preceded by the text naming it.

    Filtering a block list down to `type == "image_url"` loses something when the
    tool put a line of text in front of each picture to say what it is. A deck
    review returns eight to twelve page renders in one result, each introduced by
    its page number. The labels do survive -- image_placeholder_text joins every
    text part into the tool result -- but they survive in a *different message*
    from the pictures, so pairing the third label with the third image is
    positional guesswork across a message boundary. Interleaved, it is not.

    With a single image there is nothing to disambiguate and the label is already
    in the result text, so one picture is returned bare: this is the shape a plain
    `read_file` on a PNG produces, and repeating its one line of geometry in the
    following message would be noise.

    Only the part *immediately* before an image counts as its label, and a label
    is used once. Anything else -- a paragraph of preamble, a trailing summary --
    belongs with the result text, not with a picture.
    """
    images = [b for b in blocks if is_image_part(b)]
    if len(images) < 2:
        return images
    attached: list[Any] = []
    taken: set[int] = set()
    for index, block in enumerate(blocks):
        if not is_image_part(block):
            continue
        label = index - 1
        if (
            label >= 0
            and label not in taken
            and isinstance(blocks[label], dict)
            and blocks[label].get("type") == "text"
        ):
            attached.append(blocks[label])
            taken.add(label)

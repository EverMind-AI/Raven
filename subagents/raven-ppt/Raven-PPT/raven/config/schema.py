...
class PptToolConfig(Base):
    ...
    On by default. The capability is still a plugin -- nothing PPT-shaped reaches
    the prompt renderer, the bootstrap loader or the skill catalogue, and setting
    this false gives back the general agent -- but the default is the form the
    build ships as, and an install where deck making is opt-in is one where the
    first request goes to the wrong tools.
    """

    enabled: bool = True
    profile: str = "script_author"
    """Which route the model walks. `script_author` has the author write the deck
    ...
    def _known_profile(cls, value: str) -> str:
        # Validated here rather than at assembly so a typo in a config file is a
        # startup error naming the alternatives, not a silent fallback.
        known = {"script_author", "slot_author", "image_text"}
        if value not in known:
            raise ValueError(f"unknown ppt profile {value!r}; known routes are {', '.join(sorted(known))}")
        return value
...
class ToolsConfig(Base):
    ...
    ppt: PptToolConfig = Field(default_factory=PptToolConfig)

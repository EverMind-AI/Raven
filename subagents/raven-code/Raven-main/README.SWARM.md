# Raven-X 蜂群集成分支（feat/acp_worktree_integration）使用说明

> 场景：**外部主 agent 是编排者，Raven-X 是被派活的子 agent（工人）**。
> 本分支为此定制了默认行为（下表）；评测主线 `raven-x` 与集成线独立演进。
> **集成侧的调整请对本分支发 PR，不要直接推。**
...
3. **`ask_user` 按传输能力工作**：ACP 宿主提供 question broker 时，问题经
   `clarify.request` 发给宿主，收到回答后在同一轮继续；无 broker 的 CLI 调用会立刻收到
   `Error: ask_user not configured (no question broker)`，不会挂死。无人值守 CLI 任务仍应在
   提示词里要求模型自行选择合理默认，并在结果中说明。

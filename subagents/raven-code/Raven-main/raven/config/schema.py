...
class CompactionConfig(Base):
    ...
    # None keeps the sole trigger at ``window - reserved_tokens`` (fires just
    # below overflow). Set a fraction in (0, 1) to also compact once the context
    # crosses ``trigger_ratio * window`` — earlier, proactive compaction that
    # keeps long turns off the slow high-context tail. Default None preserves
    # product behavior.
    ...
    # Summaries always run on the session model. A dedicated summary-model
    # field existed once and is deliberately gone: a pinned name outlives a
    # model switch and then routes every summary to a retired endpoint. A
    # legacy ``model`` key in saved configs is accepted and ignored.
...
class WebToolsConfig(Base):
    ...
    # On by default on the swarm-integration line: an orchestrated worker
    # handles research requests as well as coding, so web_fetch (keyless) and
    # web_search (still needs its key) are part of the default surface.
    # Offline sandboxes and evaluation configs must pin this to false.
    enabled: bool = True
    proxy: str | None = None  # HTTP/SOCKS5 proxy URL, e.g. "http://127.0.0.1:7890" or "socks5://127.0.0.1:1080"

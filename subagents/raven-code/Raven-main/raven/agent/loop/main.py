...
from raven.agent.loop import compaction, completion_gates
...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        # A tool that cannot run must never be advertised: every call failing
        # wastes turns and teaches the model a dead path (17/17 failed calls in
        # one TB run gated web_search on its API key). The same holds for every
        # web call in an offline sandbox, so the whole group is opt-in.
        if self.web_enabled:
            ...
            logger.info("web tools not registered: tools.web.enabled is false")
        ...
        # The offer stand-in rides on the web-tools switch: it exists to sell an
        # unconfigured engine, which is noise for an agent that cannot browse.
        ...
        # only the agent's ability to schedule new ones is gone.
    ...
    def _emergency_shrink(cls, messages: list[dict]) -> tuple[list[dict], int]:
        ...
        """
        messages, tool_elided = compaction.prune_old_tool_results(messages, cls._SHRINK_KEEP_RECENT_TOOL_RESULTS)
        messages, reasoning_elided = compaction.prune_old_reasoning(messages, cls._SHRINK_KEEP_RECENT_REASONING)
        return messages, tool_elided + reasoning_elided
    ...
    def _compaction_limits(self, model: str | None) -> tuple[int, int]:
        ...
        Cap at the configured window: when the provider table reports a larger
        theoretical window than we operate in (e.g. deepseek-v4-flash advertises
        1M but the run is configured for 262k), the trigger must key off the
        window we actually run within, or it never fires before the real overflow.
        """
        resolved = resolve_context_window(model)
        limit = min(resolved, self.context_window_tokens) if resolved else self.context_window_tokens
        ...
        *,
        ...
        if cfg.prune:
            ...
            if elided:
                messages = pruned
                changed = True
                logger.warning("Compaction: pruned {} old tool result(s) / reasoning block(s)", elided)
    ...
    async def _run_agent_loop(
        ...
        # provider-reported ground truth when deciding overflow recovery. It
        # is also what arms the proactive compaction trigger: 0 until the first
        ...
        # reach a batch run's captured output.
                ...
                for tool_call in response.tool_calls:
                    ...
                    if (test_gate_enabled or verify_complete_enabled) and executed_name == "exec":
                        ...
                        if (
                            ...
                            turn_code_edits = True
                            if (gate_stale_enabled or gate_red_enabled) and path not in edits_since_test:
    ...
    async def _process_message(
        ...
        if (not self.profile.attended or self.profile.name == "legacy_oneshot") and any(outcome.gate_triggers.values()):
            ...
            # as the reply must not have to strip a trailer line.
            print(format_gate_counters(outcome.gate_triggers), flush=True)

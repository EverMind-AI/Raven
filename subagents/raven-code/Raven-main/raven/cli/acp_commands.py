...
def acp(
    ctx: typer.Context,
    config: str = typer.Option("", "--config", help="Config file to run under; defaults to the RAVEN_HOME one."),
) -> None:
    ...
    if config:
        ...
        set_config_path(Path(config).expanduser())
    try:
        asyncio.run(_serve())
...
def _file_log_level() -> str:
    ...
    DEBUG here is not a verbosity choice, it is a hang. LiteLLM's
    ``print_args_passed_to_litellm`` writes the whole request -- system prompt,
    history, every tool schema -- as one DEBUG record per call, tens of KiB at a
    time, and the sink is ``enqueue=True``, so those records cross a
    multiprocessing queue. Measured on a stuck run: the agent loop blocked in
    ``logging.emit`` while loguru's writer thread sat in ``_recv`` on the same
    queue, 0% CPU, and the turn never issued its request. The gateway's entry
    point takes this level from config for the same reason; this one had it
    hardcoded.

    ``RAVEN_ACP_LOG_LEVEL`` is the way back to a full trace, for a session where
    the payloads are the thing being debugged.
    """

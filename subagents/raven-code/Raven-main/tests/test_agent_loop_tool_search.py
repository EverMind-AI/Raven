...
def _make_loop(workspace: Path, cfg, strategies=None) -> AgentLoop:
    return AgentLoop(
        ...
        # web_search needs both the group switch and a key; these tests use it
        # as the representative cataloged domain tool.
        web_enabled=True,
...
async def test_enabled_loop_keeps_interaction_primitives_visible(workspace) -> None:
    ...
    # file/interaction/orchestration primitives (read_file / ask_user / spawn)
    # and the meta-tools keep their schema, while a cataloged domain tool
    ...
    assert {"read_file", "ask_user", "spawn"} <= names, "primitives must stay visible"

...
def test_should_compact_trigger_ratio_lowers_line():
    ...
    assert should_compact(130_000, 200_000, 20_000) is False  # below base, no ratio
    assert should_compact(130_000, 200_000, 20_000, 0.6) is True  # 130k >= 0.6*200k
    assert should_compact(119_999, 200_000, 20_000, 0.6) is False
    # Ratio never raises the line above the base trigger.
    assert should_compact(185_000, 200_000, 20_000, 0.95) is True
    # Out-of-range ratios are ignored (fall back to base trigger).
    assert should_compact(130_000, 200_000, 20_000, 0.0) is False
    assert should_compact(130_000, 200_000, 20_000, 1.0) is False
...
def test_compaction_config_defaults():
    ...
    # The retired summary-model field must stay a silently-ignored legacy key.
    assert not hasattr(CompactionConfig.model_validate({"model": "legacy"}), "model")
...
async def test_summary_always_uses_the_session_model(workspace):
    """A legacy config pinning a summary model must not route summaries away
    from the session model: after a model switch the pinned name points at a
    retired endpoint and every compaction would fail."""
    provider = _CompactionProvider()
    agent = AgentLoop(
        provider=provider,
        workspace=workspace,
        ...
        max_iterations=12,
        restrict_to_workspace=True,
        compaction=CompactionConfig.model_validate({"model": "retired-model"}),
    )
    out = await agent._process_message(
        TurnRequest(
            origin=Origin.USER,

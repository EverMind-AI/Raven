...
Two halves per bypass family: (1) the raw command REALLY writes or deletes
when run without the gate - in a throwaway directory - so the case matters;
(2) driven the way the tool registry drives it (gate verdict first, command
only on a None verdict), the gate intervenes BEFORE anything runs and the
target files stay untouched. Read-only forms keep flowing: no allocation
record, no pending question, command executes.
"""
...
def drive(repo: Path, base: Path, repos: Path, session: str, command: str):
    ...
    verdict = asyncio.run(gate("exec", {"command": command}))
    ran = None
    if verdict is None:
...
def test_mutating_forms_are_gated_before_execution(tmp_path, armed, command):
    base, repos = armed
    repo = tmp_path / "repo"
    make_repo(repo)
...
def test_readonly_git_forms_run_without_allocation(tmp_path, armed, command):
    base, repos = armed
    repo = tmp_path / "repo"
    make_repo(repo)
...
def test_readonly_git_with_spaced_absolute_path(tmp_path, armed):
    base, repos = armed
...
@pytest.mark.skipif(FD is None, reason="fd binary not installed")
def test_readonly_sort_and_fd_still_flow(tmp_path, armed):
    base, repos = armed
    repo = tmp_path / "repo"
    make_repo(repo)

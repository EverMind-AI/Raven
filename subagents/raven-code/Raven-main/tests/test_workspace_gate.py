...
Everything runs against throwaway git repos under tmp_path; nothing touches a
real checkout. The race test forks real processes because flock is per-fd and
the bug being guarded against - two instances both seeing "clean, unowned" and
both binding the primary checkout - only exists across processes.
"""
...
from raven.agent.workspace_gate import (
    WorkspaceGate,
...
def call(gate, name, params):
    ...
    return asyncio.run(gate(name, params))
...
def test_dirty_primary_asks(tmp_path):
    ...
    assert "未提交改动" in a["pending"]["question"]
...
async def test_worktree_escape_guard_checks_registry_canonical_parameters(tmp_path):
    ...
    from raven.agent.tools.shell import ExecSessionRegistry, ExecTool, ExecWriteTool
    ...
    registry.register(ExecWriteTool(sessions))
    registry.set_write_gate(gate)

    try:
        write_result = await registry.execute(
            "write_file",
            {"file_path": str(repo / "escaped.txt"), "content": "bad"},
        )
        relative_write_result = await registry.execute(
            "write_file",
            {
                "file_path": os.path.relpath(repo / "relative-escaped.txt", wt),
                "content": "bad",
            },
        )
        ...
        opened = await registry.execute(
            "exec",
            {"command": "pwd", "session": "shell", "timeout": 10},
        )
        session_exec_result = await registry.execute(
            "exec_write",
            {
                "session": "shell",
                "input": f"cd {repo} && touch session-escaped.txt",
                "timeout": 10,
            },
        )
        session_relative_result = await registry.execute(
            "exec_write",
            {
                "session": "shell",
                "input": f"touch {os.path.relpath(repo / 'session-relative-escaped.txt', wt)}",
                "timeout": 10,
            },
        )
    finally:
    ...
    assert "[session: shell]" in str(opened)
    assert all(
    ...
    assert not (repo / "session-relative-escaped.txt").exists()
    assert (repo / "a.py").read_text() == "x = 1\n"


@pytest.mark.asyncio
async def test_worktree_gate_blocks_sessions_opened_before_allocation(tmp_path):
    from raven.agent.tools.registry import ToolRegistry
    from raven.agent.tools.shell import ExecSessionRegistry, ExecTool, ExecWriteTool
    from raven.sandbox import DirectExecutor
    ...
    try:
        ...
        assert "[session: shell]" in str(opened)
        assert sessions.get("shell") is not None
        assert Path(sessions.working_dir("shell") or "").resolve() == repo.resolve()
        (alloc / "allocation.json").write_text(
            json.dumps(
                {
                    "state": "working",
        ...
        write_result = await registry.execute(
            "exec_write",
            {"session": "shell", "input": "touch persisted-shell-escape.txt", "timeout": 10},
        )
        exec_result = await registry.execute(
            "exec",
            {
                "command": "touch persisted-exec-escape.txt",
                "session": "shell",
                "timeout": 10,
            },
        )
    ...
    assert sessions.working_dir("shell") is None
    assert "WRITE BLOCKED" in str(write_result)
    assert "WRITE BLOCKED" in str(exec_result)
    assert not (repo / "persisted-shell-escape.txt").exists()
    assert not (repo / "persisted-exec-escape.txt").exists()
    assert not (wt / "persisted-shell-escape.txt").exists()
    assert not (wt / "persisted-exec-escape.txt").exists()


# --- the race -------------------------------------------------------------------


def _race_worker(repo: str, tmp: str, instance: str, q):
    import asyncio as _asyncio

    from raven.agent.workspace_gate import WorkspaceGate

    g = WorkspaceGate(Path(repo), Path(tmp) / "repos", instance=instance, alloc_dir=Path(tmp) / "instances" / instance)
    (Path(tmp) / "instances" / instance).mkdir(parents=True, exist_ok=True)
    verdict = _asyncio.run(g("write_file", {"path": "x"}))
...
def test_concurrent_first_write_single_primary(tmp_path):
    ...
    q = ctx.Queue()
    procs = [ctx.Process(target=_race_worker, args=(str(repo), str(tmp_path), f"instance-{i}", q)) for i in range(8)]
    for p in procs:
        p.start()
    for p in procs:
        p.join(timeout=60)
...
def test_in_turn_approve_creates_worktree_and_rebinds(tmp_path):
    ...
    assert "未提交改动" in prompt and "不包含" in prompt  # HEAD-based, dirty excluded
...
from raven.agent.workspace_gate import CHOICE_READONLY, CHOICE_RETRY, CHOICE_WORKTREE, parse_choice

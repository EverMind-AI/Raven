...
# --------------------------------------------------------------------------- #
# loop: a plain empty retry must change the prompt, not re-send it verbatim    #
# --------------------------------------------------------------------------- #


class _RecordingEmptyProvider(LLMProvider):
    """Returns nothing forever and keeps every prompt it was sent.

    Reproduces the failure mode observed on LHTB: the model spends its whole
    token budget inside the reasoning field and returns neither content nor a
    tool call, so the loop retried with a byte-identical prompt and the model
    had no reason to answer differently. prompt_tokens stayed frozen across six
    spins and two tasks made no progress for an entire run.
    """
...
@pytest.mark.asyncio
async def test_a_plain_empty_retry_does_not_resend_the_same_prompt(workspace):
    provider = _RecordingEmptyProvider()
    agent = _make_agent(
        workspace,
        provider,
        RecoveryLimits(thinking_prefill_max_retries=0, empty_content_max_retries=2),
    )
    ...
    assert len(provider.prompts) >= 2, "the retry budget should have been spent"
    first, second = provider.prompts[0], provider.prompts[1]
    assert second != first, (
        "re-sending a byte-identical prompt gives a deterministic model no reason "
        "to answer differently; every retry is then wasted"
    )
    assert len(second) > len(first), "the retry must add something the model can react to"
    assert any("no tool call" in str(m.get("content", "")) for m in second), (
        "the added message should say what went wrong"
    )


@pytest.mark.asyncio
async def test_the_retry_scaffolding_is_not_persisted(workspace):
    provider = _RecordingEmptyProvider()
    agent = _make_agent(
        workspace,
        provider,
    ...
    for m in session.messages:
        assert not m.get("_recovery_synthetic")
        assert "no tool call" not in str(m.get("content", ""))

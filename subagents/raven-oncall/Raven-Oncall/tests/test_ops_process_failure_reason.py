...
# The replies are consumed in order. A read that finds no result now asks whether
# the job is still running before it says anything about why it stopped -- a job
# mid-run has no result file either, and calling that "ended from outside" was
# simply false (measured 2026-08-17). So GONE sits between the failed cat and the
# log tail in every sequence below.
GONE = (0, "gone\n")


class FakeShell:
    def __init__(self, replies): self._replies = list(replies); self.commands = []
...
def _executor(replies):
    return ProcessExecutor(FakeShell(replies), remote_dir="/root/ops",
...
async def test_a_job_that_reached_its_own_exit_path_keeps_its_own_words():
    ...
    ex = _executor([(0, '{"status": "failed", "error_kind": "cuda_oom", '

...
def test_turn_outcome_is_frozen_with_usage_and_explicit_reply():
    """Pins the spine-agent seam's field set.

    ``usage_totals`` was added deliberately: ``usage`` carries the turn's final
    LLM call (numbers published from it exist, so it could not be redefined in
    place) and the summed spend needed somewhere to live that the cron wake
    handler can reach. It defaults, so every existing construction site is
    unaffected -- the two assertions below check both halves of that.

    Keep this assertion exact. It is what caught the field being added, which is
    the only reason the addition got argued for rather than slipped in.
    """
    ...
    assert fields == {"usage", "explicit_reply", "usage_totals"}
    assert o.usage_totals == UsageTotals(), "omitting it must mean 'not measured', not a crash"
    assert o.usage_totals.calls == 0

...
def campaign(tmp_path, monkeypatch):
    ...
    (d / "meta.json").write_text(json.dumps(
        {"backend": "process", "remote_dir": "/tmp/x", "command": "true",
         "seed_config": {"nu": "1e-3", "deltaT": "5e-4"}}), encoding="utf-8")
    monkeypatch.setattr(ops, "_ops_home", lambda: home)
    monkeypatch.setattr("raven.ops.make_ssh_runner",
...
async def _submit(campaign_dir, cfg, basis=""):
    ...
    return await OpsSubmitTool().execute(
        campaign="beam", configs=[cfg], round=0, eta_seconds=60,
        objective="x", basis=basis)
...
async def test_a_departure_with_a_reason_goes_through_and_is_recorded(campaign):
    out = await _submit(campaign, {"nu": "1e-6", "deltaT": "5e-4"},
                        basis="nu=1e-3 is not water; water is 1e-6 at 20C, three orders out")

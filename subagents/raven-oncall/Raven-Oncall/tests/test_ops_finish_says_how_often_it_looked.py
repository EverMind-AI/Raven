...
async def _finish(cdir: Path, msg: _Msg, **over):
    tool = OpsFinishTool(cron_service=None)
    tool.set_registry(_Registry(msg))
...
async def test_the_delivered_report_says_how_many_looks_it_took(tmp_path) -> None:
    ...
    out = await _finish(cdir, msg)

    assert "Accepted" in out
    body = msg.sent[0]["content"]
    assert "24 looks" in body
    assert "3h 42m" in body
...
async def test_a_watch_that_never_looked_cannot_look_like_one_that_did(tmp_path) -> None:
    ...
    await _finish(cdir, msg)

    assert "watch kept" not in msg.sent[0]["content"]

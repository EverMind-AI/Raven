...
def _tool():
    t = OpsSubmitTool(cron_service=_FakeCron())
...
async def test_a_dropped_key_refuses(tmp_path, monkeypatch):
    ...
    kinds = [json.loads(l).get("kind")
             for l in (cdir / "events.jsonl").read_text().splitlines()]
...
async def test_a_batch_at_round_zero_refuses(tmp_path, monkeypatch):
    ...
    cdir = _campaign(tmp_path, {"maxCo": 20, "run": 1})
    sent = _install(monkeypatch)
    out = await _submit(cdir, configs=[{"maxCo": 20, "run": 1}, {"maxCo": 5, "run": 2}],
                        round=0)
    assert "REFUSED" in out
    assert sent == []
...
async def test_round_one_may_change_anything(tmp_path, monkeypatch):
    ...
    _observed(cdir)
    out = await _submit(cdir, configs=[{"maxCo": 1, "run": 2}], round=1,
                        basis="ndcg 0.29 on the latest sample; maxCo 20 stalled")
    assert "Submitted" in out, out
    assert len(sent) == 2
...
async def test_a_campaign_that_already_has_records_is_past_its_start(tmp_path, monkeypatch):
    ...
    (cdir / "ledger.json").write_text(json.dumps({"version": 1, "records": {
        "lr5em06_epochs8": {"idem_key": "lr5em06_epochs8", "status": "succeeded",
                            "campaign": "c", "handle": None, "attempts": 1,
                            "escalated": False, "metrics": {"ndcg": 0.3255}},

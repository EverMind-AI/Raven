...
from raven.agent.tools.ops_escalation import _write_report_md


def test_the_report_is_written_next_to_the_campaign(tmp_path):
    md = _write_report_md(tmp_path, "blade", "blade sweep", "done",
                          {"points": 20}, None, "prose")
...
async def test_the_listing_says_where_a_finished_campaign_s_report_is(tmp_path, monkeypatch):
    ...
    (d / "concluded.json").write_text(json.dumps(
        {"concluded_at": "2026-08-18T21:41:00", "outcome": "done", "reason": "s"}), encoding="utf-8")

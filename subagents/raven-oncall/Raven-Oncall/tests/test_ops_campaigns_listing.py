...
from raven.agent.tools.ops import OpsCampaignsTool
...
def _campaign(home: Path, name: str, *, host="14.103.100.27", case="/home/cfd/work/case_a",
              total=150, unit="core-minute", concluded=None, trials=0) -> Path:
    d = home / name
    d.mkdir(parents=True, exist_ok=True)
    (d / "meta.json").write_text(json.dumps({
        "backend": "process", "host": host, "port": 22, "key": "~/.ssh/id_rsa",
        "command": "run {config} {job_dir}", "staged_case": case,
        "budget": {"unit": unit, "total": total, "overlap": "additive"},
    }), encoding="utf-8")
    if trials:
        (d / "ledger.json").write_text(json.dumps({"version": 1, "records": {
            f"t{i}": {"idem_key": f"t{i}", "status": "succeeded", "campaign": name,
                      "handle": None, "attempts": 1, "escalated": False, "metrics": {}}
            for i in range(trials)}}), encoding="utf-8")
    if concluded:
        (d / "concluded.json").write_text(json.dumps(
            {"concluded_at": "2026-08-14T01:00:00", "outcome": concluded,
             "reason": f"{name} done"}), encoding="utf-8")
    return d


def _spend(monkeypatch, minutes=115.47):
    class _Backend:
        async def spent_minutes(self): return minutes
        async def remaining_minutes(self): return 150.0 - minutes
...
async def test_spend_is_probed_for_live_campaigns_only(monkeypatch, tmp_path):
    ...
    class _Backend:
        ...
        async def spent_minutes(self):
            probed.append(self.host); return 12.5
        async def remaining_minutes(self): return 137.5

    monkeypatch.setattr("raven.ops.backends.backend_from_meta",
                        lambda meta: _Backend(meta.get("host", "?")))
...
async def test_a_host_that_cannot_answer_still_lists(monkeypatch, tmp_path):
    ...
    class _Dead:
        async def spent_minutes(self): raise OSError("connection refused")
        async def remaining_minutes(self): raise OSError("connection refused")
...
async def test_it_does_not_say_which_one_is_current(monkeypatch, tmp_path):
    ...
    home = _home(monkeypatch, tmp_path)
    _campaign(home, "a"); _campaign(home, "b")
    _spend(monkeypatch)
    out = await OpsCampaignsTool().execute()
    assert "current" not in out.lower() and "当前" not in out
...
def test_the_listing_says_what_each_campaign_is_optimising(tmp_path, monkeypatch):
    ...
    home = tmp_path / "ops"
    for name, obj in (("beam", {"metric": "collapse_load", "direction": "max"}),
                      ("block", {"metric": "max_penetration", "direction": "min"})):
        d = home / name
        d.mkdir(parents=True)
        (d / "meta.json").write_text(_j.dumps(
            {"backend": "process", "staged_case": "/same/case", "remote_dir": "/same",
             "budget": {"unit": "minute", "total": 150}, "objective": obj}), encoding="utf-8")

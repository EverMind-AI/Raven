...
def test_the_backend_does_not_read_a_look_count_as_minutes(tmp_path) -> None:
    ...
    from raven.ops.budget import Budget

    looks = ProcessExecutor(lambda cmd: (0, ""), remote_dir=str(tmp_path), command="run.sh",
                            budget=Budget("look", 40.0, meter="look"))
    compute = ProcessExecutor(lambda cmd: (0, ""), remote_dir=str(tmp_path), command="run.sh",
                              budget=Budget("core-minute", 150.0))
...
async def test_a_refused_check_later_leaves_the_turn_open(tmp_path, monkeypatch) -> None:
    ...
    assert str(out.model_text).startswith("REFUSED")

...
FULL = ["--non-interactive", "--software", "OpenFOAM (/opt/openfoam)",
        "--budget-unit", "core-minute", "--concurrency", "1"]
...
def reached(monkeypatch):
    ...
    monkeypatch.setattr("raven.cli.ops_connection_commands.probe",
                        lambda row, **kw: (True, "reached it", {"cores": 128, "kind": "gpu"}))
...
def test_the_flags_are_a_complete_path_because_a_tui_cannot_prompt(store, reached):
    ...
    out = run("add", "--id", "box", "--name", "the box", "--transport", "local",
              "--path", "/srv/arena/cases", *FULL)
...
def test_an_ssh_row_with_no_key_is_refused_even_without_a_probe(store, reached):
    out = run("add", "--id", "gpu", "--name", "GPU", "--host", "h", "--port", "22",
              "--user", "root", "--skip-probe", *FULL)

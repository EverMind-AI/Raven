...
class OnCallConfig(Base):
    """Running work on the owner's machines, watched, over hours or days."""

    enabled: bool = False
    """Whether this instance has the on-call surface at all.

    Off by default, and the default is the point: everything on-call adds is
    inert for someone who never runs work on another machine, and none of it is
    free. Thirteen tools carry about 4800 tokens of schema into every turn, and
    the judgement that reads a request as work-to-watch costs one model call per
    turn that touches a path. Measured 2026-08-19 while integrating with
    upstream, by upstream's own tests: the schemas took the room for history from
    130000 tokens to 127933, and the extra call made a usage total read 4000
    where the calls summed to 6000.

    Turned on by the owner, in one place, deliberately -- connecting a simulation
    platform is what makes it worth paying for, and an owner who connected one
    and is spending today chatting can turn it back off. Inferring it instead
    from "are there machines in the connection registry" was tried first and
    dropped: that answers whether this installation COULD do on-call work, never
    whether the owner wants it now, and it left no way to say no.
    """

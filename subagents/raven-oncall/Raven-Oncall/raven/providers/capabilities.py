...
def wire_overrides(provider: str | None, model: str | None) -> dict[str, Any]:
    ...
    for owner, needle, body in _WIRE_OVERRIDES:
        if normalize_provider_name(owner) == name and needle in lowered:
            out.update(body)
    # The same fact in vLLM's dialect, for a Qwen 3.x served by the owner rather
    # than by a gateway. Not a table row because it keys on a PROPERTY of the
    # provider (self-hosted or gateway) rather than on its name, and the set of
    # such names is whatever the owner happened to configure. Measured on the
    # LiveCode line: without it the model spends its whole token budget inside
    # the reasoning field and returns neither content nor a tool call, and the
    # loop retried that with a byte-identical prompt -- two tasks made no
    # progress for a whole run.
    # Only when the table said nothing for this provider: a row IS the dialect
    # for the provider it names, and adding a second one hands OpenRouter (a
    # gateway, so it matches the property below) a vLLM key it does not know.
    if "qwen" in lowered and not out:
        from raven.providers.registry import find_by_name

        spec = find_by_name(provider)
        if spec is not None and (spec.is_local or spec.is_gateway):

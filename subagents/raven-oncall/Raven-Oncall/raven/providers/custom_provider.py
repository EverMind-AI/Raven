...
class CustomProvider(LLMProvider):
    def __init__(
        ...
        # Vendor-specific request fields a self-hosted server needs and the
        # OpenAI schema has no place for. vLLM's ``chat_template_kwargs`` is the
        # case this exists for: without ``{"enable_thinking": false}`` a Qwen 3.6
        # server spends the whole token budget inside its reasoning field and

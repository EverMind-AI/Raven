...
class ConfirmBroker:
    ...
    async def await_confirm(self, prompt: str, *, default: bool) -> bool:
        ...
        try:
            ...
            await self._withdraw(request_id, "timeout")
            return default
        except asyncio.CancelledError:
            await self._withdraw(request_id, "cancelled")
            return default
            ...
            await self._withdraw(request_id, "error")
            return default
        finally:
            self._pending.pop(request_id, None)

    async def _withdraw(self, request_id: str, reason: str) -> None:
        """Tell the frontend this confirm stopped being pending.

        Same shape, and the same reason, as the ask-user withdrawal: the prompt is
        drawn from ``confirm.request`` and was taken down only by an answer or by
        the end-of-turn overlay reset. A wake turn reaches neither -- the hard
        limit fires long before the turn ends, and a cron turn runs in a
        conversation no TUI session subscribes to, so its end is never delivered
        at all. What stays on screen then reads as "it is waiting for me" while
        the caller has already taken the default and moved on.

        Send failures are logged and swallowed: the caller's contract is that it
        always gets a bool back, and a frontend that has gone away must not turn a
        ...
        try:
            ...
            logger.warning("confirm_broker: could not withdraw confirm {} ({})", request_id, reason)

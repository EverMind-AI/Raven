...
def _result_to_dict(res: JobResult) -> dict:
    return {"status": res.status.value, "metrics": res.metrics, "output": res.output,
            "error": res.error, "deliverable": res.deliverable}
...
def _record_from_dict(d: dict) -> JobRecord:
    ...
    return JobRecord(
        ...
        result=(
            JobResult(JobStatus(res["status"]), res.get("metrics", {}), res.get("output", {}),
                      res.get("error"), res.get("deliverable"))
            if res
            else None
        ),

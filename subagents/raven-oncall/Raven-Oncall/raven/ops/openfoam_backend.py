...
from raven.ops.backend import JobBackendError, JobHandle, JobResult, JobStatus
...
class OpenFoamExecutor(ProcessExecutor):
    ...
    async def fetch_result(self, handle: JobHandle) -> JobResult:
        ...
        rc, tail = await self._arun(
            f"tail -n 40 {job_dir}/job.log {job_dir}/log.* 2>/dev/null | tail -n 40"
        )

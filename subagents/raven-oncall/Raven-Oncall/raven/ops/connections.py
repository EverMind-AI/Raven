...
def usable(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [r for r in rows if not any(p.blocking for p in row_problems(r))]
...
def problems() -> list[Problem]:
    ...
    out.extend(
        ...
        for rid, n in seen.items() if n > 1
    )
    return out
...
def describe() -> str:
    ...
    if not rows:
        ...
        return (
            f"No machine is set up in this instance ({store_path()} {where}).\n\n"
            + _REQUEST
        )

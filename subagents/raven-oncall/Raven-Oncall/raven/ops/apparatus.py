...
def meta_sha(campaign_dir: Path) -> str:
    ...
    try:
        return _sha_text(p.read_text(encoding="utf-8"))
    except OSError:
        return "-"
...
def case_hashes(root: str, runner: Callable[[str], tuple[int, str]]) -> tuple[dict[str, str], bool]:
    ...
    q = shlex.quote(root)
    rc, out = runner(
        f"find {q} -maxdepth {_MAX_DEPTH} -type f -exec sha256sum {{}} + 2>/dev/null"
    )
    if rc != 0:
        return {}, False
    table: dict[str, str] = {}
...
def baseline_of(campaign_dir: Path, runner: Callable[[str], tuple[int, str]] | None) -> dict[str, Any]:
    ...
    if root and runner is not None:
        ...
        case, readable = {}, True
    return {"meta_sha": meta_sha(campaign_dir), "staged_case": root,
            "case": case, "case_readable": readable}
...
class Drift:
    ...
    def is_clean(self) -> bool:
        return not (self.meta_changed or self.case_changed or self.case_added
                    or self.case_removed)
...
def compare(before: dict[str, Any] | None, now: dict[str, Any]) -> Drift:
    ...
    if not now.get("case_readable", True):
        ...
        return Drift(meta_changed=before.get("meta_sha") != now.get("meta_sha"),
                     case_unreadable=True)
    ...
    return Drift(
        ...
        case_changed=sorted(p for p in old_case.keys() & new_case.keys()
                            if old_case[p] != new_case[p]),

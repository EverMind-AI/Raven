...
_INJECTED = {"budget_gpu_minutes", "run_dir", "job_dir",
...
def compare_config(submitted: dict[str, Any] | None,
                   effective: dict[str, Any] | None) -> list[str]:
...
def compare_against_declared(declared: dict[str, Any] | None,
    ...
    for k, want in declared.items():
        ...
        if not _same(want, effective[k]):
            out.append(
                f"{k} was not submitted; this campaign declares {want!r}, "
                f"the run used {effective[k]!r}"
            )
    return out


def compare_submitted_to_declared(declared: dict[str, Any] | None,
                                  submitted: dict[str, Any] | None) -> list[str]:
    ...
    for k, want in declared.items():
        ...
        if not _same(want, submitted[k]):
            out.append(f"{k}: this campaign declares {want!r}, this round submitted "
                       f"{submitted[k]!r}")
    return out

def describe(submitted: dict[str, Any] | None,
             effective: dict[str, Any] | None,
             declared: dict[str, Any] | None = None) -> list[str]:

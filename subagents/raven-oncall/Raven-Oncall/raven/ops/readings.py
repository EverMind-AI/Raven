...
_MUTATORS = frozenset({
    "rm", "rmdir", "mv", "dd", "kill", "pkill", "killall", "chmod", "chown",
    "truncate", "mkfs", "reboot", "shutdown", "mkdir", "touch", "tee",
})
...
async def take(
    ...
    *,
    ...
    if runner is None:
        return [
            _record(cdir, r, when, trial, error="the campaign's machine could not be reached")
            for r in due
        ]
    ...
    for reading in due:
        ...
        try:
            ...
            taken.append(_record(cdir, reading, when, trial,
                                 error=f"{type(exc).__name__}: {exc}"[:200]))
            continue
        if rc != 0:
            taken.append(_record(cdir, reading, when, trial,
                                 error=f"exit {rc}: {str(out).strip()[:200]}"))
            continue
...
def taken_for(cdir: str | Path, trial: str) -> set[str]:
    ...
    return {
        ...
        if row.get("trial") == trial and row.get("when") == AFTER_TRIAL
    }

...
from raven.ops.proposer import GridProposer, Proposer
...
def make_ssh_sync(
    host: str, port: int, key: str, *, user: str = "root", connect_timeout: int = 15
) -> SyncRunner:
    import subprocess
...
async def drive_polling(
    ...
    *,
    ...
    A plain poll loop for a one-shot run (e.g. the ``raven ops tune`` CLI): it is

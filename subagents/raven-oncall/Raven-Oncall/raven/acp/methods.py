...
def _channel_of(session_key: str) -> str:
    """The channel half of ``<channel>:<chat_id>``."""
    return session_key.split(":", 1)[0]


def _chat_id_of(session_key: str) -> str:
    """The chat-id half of ``<channel>:<chat_id>``, empty for a malformed key."""
...
class AcpMethods:
    ...
    def _stored_sessions(self) -> list[dict[str, Any]]:
        ...
        from raven.tui_rpc.methods.session import _manager_for
            ...
            try:
                accepted = await self._call(
                    "turn.send",
                    {
                        ...
                        "content": text,
                        "media": media,
                        # Spelled out so the turn's ``(channel, chat_id)`` rebuilds
                        # this very session key. Left to default they are
                        # ``("acp", "default")`` for every session at once -- and
                        # anything that later asks "which session was this turn
                        # for" by that pair, a cron job scheduled inside the turn
                        # above all, gets one answer for all of them.
                        "channel": _channel_of(session.session_key),
    ...
    def _validated_mcp_servers(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        Refused rather than ignored when this connection has no registry that can
        scope them: accepting the field would promise tools no turn could reach.
        """
        ...
        if self._per_session_registry() is None:
            logger.warning(
                "acp: dropping {} per-session MCP server(s): no tool registry to scope them in",
                len(entries),
            )
            return {}
    ...
    def _per_session_registry(self) -> Any:
        ...
        None says this connection has no agent loop to scope anything in (a build
        that failed, or a stub in a test), which is why the field is refused
    ...
    async def unsubscribe_all(self) -> None:
        ...
        # The backstop: a session dropped without close/delete (a client that
        # exits) still owns subprocesses, and nothing else would reap them.

...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    def _append_oncall_notes(tpl: Path, dest: Path) -> None:
        """Add the on-call section to this workspace's TOOLS.md, once, if it applies.

        Kept out of TOOLS.md itself because the file is shared by every instance
        and most of them never run work on another machine: a hundred lines about
        declaring campaigns is guidance an owner with no registered machines
        cannot act on. Appended rather than written as a second file, because the
        bootstrap segment reads TOOLS.md and nothing else.

        Idempotent by heading: ``_write`` above only creates a missing file, so an
        instance whose TOOLS.md predates this would never see the section without
        an append -- and an append that ran twice would say everything twice.
        """
        section = tpl / "TOOLS_ONCALL.md"
        if not section.is_file() or not dest.is_file():
            return
        try:
            ...
            if heading and heading in current:
                return
            dest.write_text(current.rstrip("\n") + "\n\n" + body, encoding="utf-8")
        except Exception:  # noqa: BLE001 -- guidance is not worth failing a workspace sync
            return
    ...
    _append_oncall_notes(tpl, workspace / "TOOLS.md")

...
class CronTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "\n"
            "When NOT to use cron at all (on-call campaigns):\n"
            "If you are watching an on-call campaign and want to come back to it\n"
            "later, use `ops_check_later`, not this tool. It records the basis for\n"
            "waiting, which the next wake reads back, and composes a wake message\n"
            "naming the campaign and its ledger; a wake turn starts with no history,\n"
            "so a bare reminder arrives with nothing to act on. Unrelated reminders\n"
            "still belong here.\n"
            "\n"
    ...
    def _add_job(
        ...
        before = {j.id for j in self._cron.list_jobs(include_disabled=True)}
        try:
            ...
            return f"Error: {exc}"
        if job.id in before:
            # Dedup returned an existing job. Reporting that as a creation is how a
            # loop loses its own schedule: measured 2026-08-06 on the CFD line, a
            # turn asked for a check 28 seconds from a pending wake, got that wake
            # back under the word "Created", read it as a duplicate of something it
            # had not asked for, and removed it -- leaving the campaign with no
            # pending wake at all. The run finished that night with nobody watching.
            return (
                f"Nothing new was created: an existing job already covers this, so it was "
                f"reused -- '{job.name}' (id: {job.id}). Do not remove it as a duplicate; "
                f"it is the one that will fire."
            )
    ...
    def _remove_job(self, job_id: str | None) -> str:
        ...
        campaign = self._cron.sole_pending_wake_for(job_id)
        if not self._cron.remove_job(job_id):
            return f"Job {job_id} not found"
        if campaign:
            return (
                f"Removed job {job_id}. It was the last pending wake for campaign "
                f"'{campaign}': nothing is scheduled to look at that campaign now, and a "
                f"wake fires once. If it is still running, schedule the next look with "
                f"ops_check_later."
            )

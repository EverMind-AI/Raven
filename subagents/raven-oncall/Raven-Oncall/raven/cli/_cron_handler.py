...
def _log_ops_wake_cost(job: "CronJob", outcome: Any) -> None:
    """Record one on-call wake turn's token cost on the campaign's event trail.

    The cost family in the eval plan (tokens and turns per campaign) can only be
    attributed at the turn boundary, which the tools running inside the turn
    cannot see -- this is the seam where the turn's usage is known and the job
    name ("ops:<campaign>:<round>") identifies the campaign. Best-effort: the
    cost record must never break the wake.
    """
    try:
        from raven.ops.instrument import campaign_dir, log_event

        parts = job.name.split(":")
        if len(parts) < 2 or not parts[1]:
            return
        usage = getattr(outcome, "usage", None)
        totals = getattr(outcome, "usage_totals", None)
        # Both calibers, under names that say which is which. The three original
        # fields are the turn's LAST call and keep that meaning, because published
        # numbers came from them; total_* is the sum over the turn's calls. A
        # reader must not add the two or merge them into one column.
        log_event(
            campaign_dir(parts[1]),
            "wake_turn",
            wake=job.name,
            prompt_tokens=getattr(usage, "prompt_tokens", 0) or 0,
            completion_tokens=getattr(usage, "completion_tokens", 0) or 0,
            total_tokens=getattr(usage, "total_tokens", 0) or 0,
            total_prompt_tokens=getattr(totals, "prompt_tokens", None),
            total_completion_tokens=getattr(totals, "completion_tokens", None),
            usage_calls=getattr(totals, "calls", None),
            usage_calls_without_usage=getattr(totals, "calls_without_usage", None),
        )
    except Exception:
        pass
...
def make_on_cron_job(
    *,
    ...
    async def on_cron_job(job: "CronJob") -> str | None:
        ...
        # Ops campaign wakes (name "ops:*") are self-driving: the campaign loop
        # schedules its own next wake and reports to the user's channel directly.
        # Feeding their outcomes to the heartbeat gave the campaign a SECOND
        # driver -- the heartbeat session cannot see the user's chat lane, so it
        # "followed up" on a campaign the user had already concluded (observed
        # live: an extra round submitted after the user accepted the result).
        is_ops_wake = job.name.startswith("ops:")
        try:
            outcome = await submit(req).result()
        except Exception as exc:
            if system_events is not None and wake is not None and not is_ops_wake:
                _emit_cron_event(system_events, wake, job, f"{type(exc).__name__}: {exc}", failed=True)
            raise
        if is_ops_wake:
            _log_ops_wake_cost(job, outcome)
        ...
        # run produced (and end its sleep early). Ops wakes are excluded (see above).
        if system_events is not None and wake is not None and not is_ops_wake:

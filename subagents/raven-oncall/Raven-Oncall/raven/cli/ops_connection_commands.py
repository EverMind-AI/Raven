...
def _ssh_argv(host: str, port: int, user: str, key: str, command: str) -> list[str]:
    return [
        ...
        "-p", str(port), "-i", os.path.expanduser(key), f"{user}@{host}", command,
    ]
...
def list_connections(as_json: bool = typer.Option(False, "--json")) -> None:
    ...
    faults = problems()
    if as_json:
        console.print_json(json.dumps({"path": str(store_path()), "state": found.state,
                                       "detail": found.detail, "connections": found.rows,
                                       "problems": [f.text for f in faults]}, ensure_ascii=False))
    ...
    for row in found.rows:
        ...
        table.add_row(str(row.get("id")), str(row.get("display_name") or ""), reach, what,
                      str(row.get("budget_unit") or "[red]-[/red]"), str(row.get("concurrency") or "[red]-[/red]"))
    console.print(table)
    for fault in faults:
...
def doctor(as_json: bool = typer.Option(False, "--json")) -> None:
    ...
    if as_json:
        console.print_json(json.dumps(
            {"path": str(store_path()), "state": found.state, "listed": len(found.rows),
             "usable": len(fit), "usable_ids": [str(r.get("id")) for r in fit],
             "machines": [shown(r) for r in fit],
             "blocking": [f.text for f in faults if f.blocking],
             "advisory": [f.text for f in faults if not f.blocking]}, ensure_ascii=False))
    else:

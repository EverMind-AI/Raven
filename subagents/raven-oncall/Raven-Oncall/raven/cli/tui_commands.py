...
def stale_bundle_sources(entry: Path, *, limit: int = 3) -> list[str]:
    """Frontend sources newer than the bundle that is about to run, newest first.

    The bundle is what runs; the sources are what the tests and the type checker
    read. Nothing connected the two, and nothing said so. Measured 2026-08-07: a
    fix for a prompt that outlived its own timeout was written, tested (906 green),
    type-checked, committed and pushed, and the TUI went on running a bundle built
    three weeks earlier -- the behaviour was unchanged in the round that shipped
    it, and the pre-registration recorded it as active. Both halves were true and
    neither implied the other.

    Empty when there is no source tree next to the bundle, which is the packaged
    wheel case: there, ``dist`` is the only copy there is and staleness is not a
    thing that can happen.
    """
    src = _UI_TUI_DIR / "src"
    if not src.is_dir() or not entry.exists():
        return []
    try:
        built = entry.stat().st_mtime
        newer = [p for p in src.rglob("*") if p.is_file() and p.stat().st_mtime > built]
    except OSError:
        return []
    newer.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    return [str(p.relative_to(_UI_TUI_DIR)) for p in newer[:limit]]
...
def apply_config_path(config: Optional[str]) -> None:
    """Point this process at a specific config file, if one was given.

    ``set_config_path`` is what makes ``get_data_dir()`` -- and with it the cron
    store and the ops ledgers -- resolve under that file's own directory, so two
    TUI instances started with different ``--config`` share no state. It must run
    before anything reads config or a runtime path; the onboarding gate in
    ``tui()`` reads config, so this cannot move later.
    """
    if config is None:
        return
...
def _build_tui_agent_loop():
    ...
    try:
        ...
        # The bootstrap files, which `agent` and `gateway` already write before
        # building their loop. TUI did not, and a workspace that never met `init`
        # simply had none of them -- no error, nothing on screen. TOOLS.md is the
        # one that shows: it is read every turn and it is where the ops surface is
        # named, so without it a turn asked to run an experiment on a remote host
        # has nothing telling it to use ops_submit. Measured 2026-08-07 on a
        # hand-made home: the loop called no ops tool for four minutes, wrote its
        # own trial script and its own ledger under a campaign name of its own
        # invention, on a model the task never mentioned.
        #
        # Silent because this process owns the terminal: `sync_workspace_templates`
        # prints through a Console of its own that the TUI's console patching does
        # not reach, so its "Created ..." lines would land in the middle of the UI.
        # The list still reaches the trail through the log.
        created = sync_workspace_templates(Path(config.workspace_path), silent=True)
        if created:
            from loguru import logger

            logger.info("TUI: created {} workspace template(s): {}", len(created), ", ".join(created))
...
async def _run_rpc_server_until_done(
    ...
    # Declared here, not beside its assignment: the assignment sits inside the
    # "loop built successfully" branch, while the shutdown path that cancels it
    # runs whether or not the loop was built.
    ops_watcher_task: asyncio.Task | None = None
    try:
    ...
    # ops_ask_owner takes one as well: during a campaign it is the only route to
    # the owner, and when nothing is running and nothing else is worth doing it
    # waits here rather than posting into the void.
    ...
    if agent_loop is not None:
        ...
        if agent_loop.cron_service is not None:
            ...
            # On-call event ingress. Without it the only thing setting detection
            # latency is the agent's own ETA guess: a real campaign scheduled a
            # 90-minute recheck, its job finished 30 minutes later, and the
            # terminal state went unnoticed for 29m59s. The watcher existed but
            # was wired into `raven gateway` only, while campaigns are driven
            # from the TUI.
            from raven.ops.event_watcher import OpsEventWatcher

            ops_watcher_task = asyncio.create_task(OpsEventWatcher(agent_loop.cron_service).run())
    ...
    try:
        ...
        if ops_watcher_task is not None:
            ops_watcher_task.cancel()
...
def tui(
    ...
    config: Optional[str] = typer.Option(
        ...
        "--config",
        help=(
            "Config file path. Its parent directory becomes this instance's runtime "
            "directory too (cron store, ops ledgers), so two TUIs started with "
            "different --config share no state."
        ),
    ),
) -> None:
    ...
    if dev:
        ...
        stale = stale_bundle_sources(dist_entry)
        if stale:
            # stderr, before the UI takes the terminal: a frontend change that was
            # never built is invisible from inside the running app, and reads as
            # "that feature was never written".
            print(
                f"⚠ TUI 构建产物比源码旧,这次跑的是旧的:{dist_entry}\n"
                f"  较新的源码:{', '.join(stale)}\n"
                f"  重建:cd {_UI_TUI_DIR} && npm run build\n",
                file=sys.stderr,
            )

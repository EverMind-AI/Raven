...
def acp(
    ...
    ``--config`` because a vendored sub-agent is not found the way an install is.
    An install resolves its config from ``RAVEN_HOME``; a folder under
    ``subagents/`` has its own ``config.json`` beside its checkout, and its
    launcher renders a copy of that with the credentials merged in and names the
    copy. Without this option the server would come up on whatever home the
    environment happens to point at -- and the failure reads as a schema error
    about a file nobody chose, or a model nobody configured.
    """
    ...
    if config is not None:
        ...
        # Before serving, not inside it: everything the stack builds -- provider,
        # tools, session manager -- resolves its paths off this at construction.

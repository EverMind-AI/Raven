...
def register(app: typer.Typer) -> None:
    ...
    def agent(
        ...
        # This process is never a cron runner -- one-shot, it exits with the
        # reply -- so by default no cron_service: registering CronTool would
        # create jobs nothing fires. Scripted reminder creation is `raven cron
        # add` with an explicit --channel.
        #
        # The exception is an on-call instance. There the store has a resident
        # reader (raven.ops.wake_shell claims the cli/tui channels), and the
        # write side is this very turn: ops_submit and ops_check_later schedule
        # the campaign's next look through their cron_service, which was None
        # here -- so a campaign started by `raven agent -m` recorded its submit
        # and then nothing ever came back for it. The service is constructed
        # but never start()ed: add_job is a plain locked file write, and firing
        # stays with whoever resides. Gated on the owner's on-call switch, not
        # on the connection registry, for the same reason as gate.py.
        cron_service = None
        ...
        if on_call_available():
            ...
            cron_service = CronService(get_cron_dir() / "jobs.json", allowed_channels={"cli"})

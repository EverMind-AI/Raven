            ...
            try:
                ...
                async def _question_to_channel(frame: dict) -> None:
                    ...
                    if frame.get("method") != "clarify.request":
                        # The broker also emits clarify.cancel when a question
                        # stops being pending. That withdraws a drawn prompt,
                        # which a chat channel does not have -- the question was
                        # an ordinary message and stays in the history. Rendering
                        # it here would post an empty message.
                        return
                ...
                await cron.start()
                # Event ingress for on-call campaigns (the definition's second
                # wake leg): a dumb resident poller that pulls a campaign's
                # pending wake forward the moment its jobs finish or a running
                # job's progress turns unhealthy. Part of the B layer: no LLM.
                from raven.ops.event_watcher import OpsEventWatcher

                ops_watcher_task = asyncio.get_running_loop().create_task(OpsEventWatcher(cron).run())

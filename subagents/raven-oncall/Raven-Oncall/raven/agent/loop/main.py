...
class AgentLoop:
    ...
    async def _run_agent_loop(
        ...
        turn_closed_by_tool = False
        post_tool_nudges = 0
        prefill_retries = 0
        empty_retries = 0

        # Turn totals live here, not in usage_sink: the sink is a point-in-time
        # gauge whose keys are on the TUI wire, and its token fields keep their
        ...
        turn_totals.setdefault("cost_usd", 0.0)
        ...
        while iteration < self.max_iterations:
            ...
            # A tool that ends the turn (an ops wake is scheduled, so the next
            # decision belongs to that wake) takes the tools away for the closing
            # call: the model can still write its reply, but cannot start another
            # step. Without this the loop ran to the iteration cap on successful
            # calls, which the failure-loop breaker below does not catch.
            tool_defs = None if turn_closed_by_tool else self.tools.get_definitions()
            ...
            # Turn totals, summed here at the provider boundary so every call is
            # counted -- including the ones the sink branch below skips, which is
            # where the under-reporting came from. A response that carries no
            # usage at all is counted separately rather than passed over: a
            # provider that stops reporting usage must not look like a cheap turn.
            if response.usage:
                turn_totals["prompt_tokens"] += int(response.usage.get("prompt_tokens", 0) or 0)
                turn_totals["completion_tokens"] += int(response.usage.get("completion_tokens", 0) or 0)
                turn_totals["total_tokens"] += int(response.usage.get("total_tokens", 0) or 0)
                turn_totals["cost_usd"] += usage_snapshot.estimated_cost_usd or 0.0
            else:
                turn_totals["calls_without_usage"] += 1
            turn_totals["calls"] += 1
            ...
            #
            # These five keys keep their original meaning: the turn's LAST call,
            # not its sum. context_* has to be point-in-time (a summed gauge reads
            # past 100% on any multi-call turn), and the token fields stay last-call
            # because numbers published from them exist -- redefining them here
            # would put "last call" and "turn total" in one column when old and new
            # runs are read together. The sum is on TurnOutcome.usage_totals, under
            # its own name. Adding a key here changes the TUI wire contract
            # (tui_rpc's UsageSnapshot is extra="forbid").
            ...
            if response.has_tool_calls:
                ...
                if action is RecoveryAction.RETRY:
                    ...
                    # Say what went wrong before re-requesting. Retrying without
                    # appending anything re-sends a byte-identical prompt, so a
                    # model that spent its whole budget inside reasoning produces
                    # the same empty response every time -- observed directly on
                    # LHTB, where prompt_tokens stayed frozen across six spins and
                    # two tasks made no progress at all for a whole run. The
                    # (empty) assistant turn keeps the transcript alternating,
    ...
    async def run_turn(
        ...
        *,
        ...
        from raven.spine.runner import TurnOutcome, UsageTotals
        ...
        # Unchanged meaning: the turn's LAST call. The sum is alongside it in
        # usage_totals rather than replacing it, so a reader merging old and new
        # runs cannot end up with both meanings in one column.
        usage = Usage(
        ...
        totals = UsageTotals(
            prompt_tokens=int(totals_sink.get("prompt_tokens", 0) or 0),
            completion_tokens=int(totals_sink.get("completion_tokens", 0) or 0),
            total_tokens=int(totals_sink.get("total_tokens", 0) or 0),
            cost_usd=float(totals_sink.get("cost_usd", 0.0) or 0.0),
            calls=int(totals_sink.get("calls", 0) or 0),
            calls_without_usage=int(totals_sink.get("calls_without_usage", 0) or 0),
        )

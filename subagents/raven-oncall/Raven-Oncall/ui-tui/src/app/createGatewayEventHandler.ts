...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `raven -c` /
    // `raven --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'clarify.request':
        ...
        return
      case 'clarify.cancel': {
        // The question stopped being pending server-side (timed out, superseded,
        // or errored).  Nothing else takes this prompt down in time: the answer
        // does, and end-of-turn does, but an on-call wake gets neither -- the
        // timeout fires ten minutes before the turn ends, and a cron turn runs
        // in a conversation this session is not subscribed to, so its end never
        // arrives.  Measured 2026-08-06: the box was still on screen four
        // minutes after the loop had decided for itself and spent the budget.
        //
        // Withdraw only the prompt named by the frame.  A late cancel must not
        // take down a newer question the loop is still waiting on, which would
        // leave that one unanswerable with nothing on screen to answer.
        const current = getOverlayState().clarify
        ...
        turnController.pushActivity(
          ev.payload?.reason === 'timeout'
            ? 'question timed out with no answer · it went on without one'
            : 'question withdrawn · it is no longer waiting for an answer',
          'info'
        )
      ...
      case 'confirm.cancel': {
        // Same shape as clarify.cancel, and the same reason: the prompt is taken
        // down by an answer or at end of turn, and a wake turn reaches neither.
        // Only the confirm this frame names is withdrawn -- a late cancel must
        // not remove a newer one the caller is still blocked on.
        const currentConfirm = getOverlayState().confirm
        const confirmId = ev.payload?.request_id

        if (!currentConfirm || (confirmId && currentConfirm.requestId !== confirmId)) {
          return
        }
        ...
        turnController.pushActivity(
          ev.payload?.reason === 'timeout'
            ? 'confirmation timed out · it went on with the default'
            : 'confirmation withdrawn · it is no longer waiting for an answer',
          'info'
        )
        setStatus(statusFromBusy())

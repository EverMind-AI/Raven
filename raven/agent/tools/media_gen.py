...
- ``image_generate``  → ``modalities:["image","text"]`` → ``message.images[].image_url.url``
  ...
  (``google/gemini-2.5-flash-image``). OpenAI's dedicated image models
...
# OpenAI's dedicated image models: only the Images API serves them.
_IMAGE_API_PREFIXES = ("gpt-image",)
    ...
    parameters = {
        ...
        "properties": {
            ...
            "aspect_ratio": {
                "type": "string",
                "enum": list(_SIZES),
                "description": "Output aspect ratio; honoured by gpt-image models, ignored by chat-routed ones",
            },
            "quality": {
                "type": "string",

...
# providers"), where a provider's API key is put right.
...
def read(cfg: Any, said: str) -> tuple[str, str, Remedy] | None:
    ...
    knows how to fix, which leaves them to be shown as they are.
    """
    ...
    if "subscription does not have access" in low or "upgrade your plan" in low:
    ...
    if "authorization grant is invalid" in low or "re-login required" in low:
        return (
            ...
            f"sign in again{f' with `{login.command}`' if login.command else ''} and connect again",
            login,
        )
    ...
    if keyless := re.search(r"provider (\S+) has no credential configured", text):
        if keyless.group(1) == "managed:kimi-code":
            return (
                "it is not signed in to a Kimi account",
                f"sign in{f' with `{login.command}`' if login.command else ''} and connect again",
                login,
            )
        return (
    ...
    if low.startswith("no model configured"):
        return (
    ...
    if code in ("401", "403"):
        return (
            ...
            f"replace it with `{_EDIT_PROVIDER[1]}` in `{_EDIT_PROVIDER[0]}` and connect again",
            Remedy("setup", *_EDIT_PROVIDER),
        )
    if code == "402":
        return (
...
async def explain(cfg: Any, failure: BaseException | None, *, prompt: str) -> tuple[str, Remedy | None] | None:
    ...
    if refusal is not None:
        ...
        return await _ask(cfg, shown, prompt) if said.strip().rstrip(".").lower() == _BARE_AUTH else None
    ...
    if failure is None or _in_chain(failure, AcpEmptyTurnError) is not None:
        return await _ask(cfg, shown, prompt)
    return None
...
async def explain_refusal(cfg: Any, detail: str, *, needs_auth: bool, prompt: str) -> tuple[str, Remedy | None] | None:
    ...
    if (known := read(cfg, reason)) is not None:
        return _told(known, reason)
    return await _ask(cfg, detail, prompt) if reason.rstrip(".").lower() == _BARE_AUTH else None


async def _ask(cfg: Any, shown: str, prompt: str) -> tuple[str, Remedy | None] | None:
    ...
    if said.lower().startswith("no model configured") and (broken := await doctor(cfg)) is not None:
        ...
        return (
            f"it cannot read its config file; `kimi doctor config` names the line. It said: {broken}",
            Remedy("config", "kimi doctor config"),
        )
    if (known := read(cfg, said)) is not None:
        return _told(known, said)
    return f"{shown}; asked again with `kimi -p`, it said: {said}", None
...
async def ask(cfg: Any, prompt: str) -> tuple[int | None, str]:
    ...
    ``(None, "")`` when it could not be run or did not finish in time.
    """
...
async def doctor(cfg: Any) -> str | None:
    ...
    # "ERROR config.toml <path>", and the reason on the line after it.

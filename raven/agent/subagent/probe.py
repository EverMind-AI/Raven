...
async def _test_acp(cfg: Any, *, source: Source, elapsed: Any) -> TestResult:
    ...
    *recorded* snapshot, so consulting it here would replay a stale failure (one
    ...
    # `SnapshotStore.load` matches on a fingerprint of the launch fields, so a
    # row recorded under a preset's name is returned only to a config that
    # launches the same way: one the user never adds is a row nothing asks for,
    # and one they add under another name does not match it.
...
async def _verify_missing_snapshots(manager: Any, rows: list[Any], *, configured: set[str] | None = None) -> None:
    ...
    for row in rows:
        ...
        try:
            ...
            # A recorded credential refusal never goes stale -- staleness asks
            # whether the launch config moved, and signing in does not move it --
            # so it is re-measured too, or the row says "Unauthorized" across
            # every restart after the sign-in that cured it.
            if (
                snapshot is not None
                and not snapshot.stale
                and not outdated_menu
                and not getattr(snapshot, "needs_auth", False)
            ):
                continue

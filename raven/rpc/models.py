...
class SubagentRemedy(_Strict):
    ...
    kind: Literal["sign_in", "setup", "api_key", "download"] = Field(
        ...
        description=(
            ...
            "npx could not fetch the agent -- fixed in the network, the npm registry or the proxy, and `command` "
            "is the row's own launch command, which fetches it from a terminal with no time limit."
        ),
    )

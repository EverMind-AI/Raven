...
class Dispatcher:
    ...
    async def dispatch(self, frame: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("rpc: SystemExit in handler {}: code={}", method, exc.code)
            return _err_frame(
                frame_id,
                INTERNAL_ERROR,
                "internal_error",
            ...
            tb_tail = _truncate_traceback(traceback.format_exc())
            logger.exception("rpc: unhandled exception in handler {}", method)
            return _err_frame(
                frame_id,
                INTERNAL_ERROR,
                "internal_error",
        ...
        if not isinstance(result, dict):
            logger.error("rpc: handler {} returned non-dict result", method)
            return _err_frame(
                frame_id,
                INTERNAL_ERROR,
                "internal_error",

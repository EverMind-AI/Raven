"""The box detector every deepdoc graph but the OCR pair is built on.

Layout detection and table-structure recognition are the same operation over
two models: run a page through a graph that answers with boxes and class
scores, drop the overlaps, and label what is left. This is that operation, plus
the geometry helpers the parser above it reasons with -- which box contains
which, which line comes first, how much two regions overlap.

Ported from RAGFlow's ``deepdoc/vision/recognizer.py`` (Apache-2.0; see
NOTICES.md). The model directory argument is gone: one install has one set of
weights, and :mod:`._onnx` is where they live.
"""
...
from raven.knowledge.parser.deepdoc import _operators as operators
from raven.knowledge.parser.deepdoc._onnx import session
from raven.knowledge.parser.deepdoc._operators import preprocess


class Recognizer:
    def __init__(self, label_list, task_name):
        """
        Args:
            label_list (`list[str]`):
                What each class index the model answers with is called. The
                order is the model's, not a preference.
            task_name (`str`):
                Which graph to run: the name it was fetched under.
        """
        self.ort_sess = session(task_name)
    ...
    def layouts_cleanup(boxes, layouts, far=2, thr=0.7):
        ...
        while i + 1 < len(layouts):
            ...
            while j < min(i + far, len(layouts)) and (
                layouts[i].get("type", "") != layouts[j].get("type", "") or not_overlapped(layouts[i], layouts[j])
            ):
                j += 1
    ...
    def preprocess(self, image_list):
        ...
        if "scale_factor" in self.input_names:
            ...
            for img in image_list:
                h, w = img.shape[:2]
                # No BGR-to-RGB swap, which upstream does here: it reads pages
                # through cv2, whose arrays are BGR, and this tree renders them
                # through PyMuPDF, whose are RGB. Swapping a page that is
                # already RGB feeds the model blue text on a yellow ground --
                # which it still answers, a little worse, saying nothing.
    ...
    def postprocess(self, boxes, inputs, thr):
        # Three output shapes, and the published weights use the third. A
        # PaddleDetection graph takes a `scale_factor` input and answers one
        # row per detection as [class, score, x0, y0, x1, y1]; a YOLO graph
        # answers a transposed block of boxes and per-class scores; and the
        # model on the hub today is a DocLayout-YOLO export that answers
        # [x0, y0, x1, y1, score, class] per query, already in corner form.
        #
        # Upstream has only the first two, so its YOLO branch transposes this
        # one into nonsense and indexes the label list with a query number.
        # Detected by shape rather than by name: the graph states neither.
        if boxes.ndim == 3 and boxes.shape[-1] == 6 and "scale_factor" not in self.input_names:
            width, height = inputs["scale_factor"]
            found = []
            for row in boxes[0]:
                x0, y0, x1, y1, score, class_id = (float(v) for v in row[:6])
                index = int(class_id)
                if score < thr or not 0 <= index < len(self.label_list):
                    continue
                found.append(
                    {
                        "type": self.label_list[index].lower(),
    ...
    def __call__(self, image_list, thr=0.7, batch_size=16):
        ...
        for i in range(batch_loop_cnt):
            ...
            for ins in inputs:
                bb = self.postprocess(
                    self.ort_sess.run(None, {k: v for k, v in ins.items() if k in self.input_names}, None)[0], ins, thr
                )
                res.append(bb)

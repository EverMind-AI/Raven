"""Recovering a table's structure from how it is drawn.

`find_tables` in PyMuPDF keys on ruling lines. A table ruled with whitespace --
which is most tables in a paper, and many in a report -- it cannot see at all,
and a table whose lines are drawn as a background image it sees as one cell.
This model was trained on the appearance instead: it answers with the regions
that are column headers, row headers, spanning cells and the table body, and
the rows are rebuilt from where the text sits inside them.

Ported from RAGFlow's ``deepdoc/vision/table_structure_recognizer.py``
(Apache-2.0; see NOTICES.md). Its two calls into RAGFlow's tokenizer are the
reason :mod:`raven.core.tokenizer` exists here: deciding whether a cell is a
header means asking whether its words are nouns, and a tokenizer that splits
Chinese per character cannot answer that.
"""
...
from raven.knowledge.parser._tables import cell_type
...
class TableStructureRecognizer(Recognizer):
    ...
    def blockType(b):
        ...
        Delegates to the one implementation of this taxonomy in the package.
        Upstream carries it twice as well -- once here and once in its Word
        parser -- and carrying two copies here meant two copies of the same
        Chinese date and quarter patterns, written as escapes, drifting apart
        the first time either was touched. Only one label carries a decision
        anywhere (`Nu`, a numeric cell), and both copies agreed on it.
        """
        return cell_type(b["text"].strip())
    ...
    def construct_table(boxes, is_english=False, html=True, **kwargs):
        ...
        logger.debug("deepdoc: cell type {}", max_type)
        ...
        if len(rows) >= 4:
            ...
            while j < len(tbl[0]):
                ...
                ff = (j + 1 < len(tbl[ii]) and tbl[ii][j + 1] and tbl[ii][j + 1][0].get("text")) or j + 1 >= len(
                    tbl[ii]
                )
                if f and ff:
                    j += 1
                    continue
                bx = tbl[ii][j][0]
                logger.debug("deepdoc: relocating a single column cell")
        ...
        if len(cols) >= 4:
            ...
            while i < len(tbl):
                ...
                bx = tbl[i][jj][0]
                logger.debug("deepdoc: relocating a single row cell")
        ...
        if html:
            return TableStructureRecognizer.__html_table(
                cap, hdset, TableStructureRecognizer.__cal_spans(boxes, rows, cols, tbl, True)
            )

        return TableStructureRecognizer.__desc_table(
            cap, hdset, TableStructureRecognizer.__cal_spans(boxes, rows, cols, tbl, False), is_english
        )

    @staticmethod
    def __html_table(cap, hdset, tbl):
        """The reconstructed grid as an HTML table.

        Two deviations from upstream, both about what the string is for. The
        text of every cell and of the caption is escaped: it comes out of a
        PDF, which can carry anything, and this string is indexed, handed to a
        model, and rendered in a page -- upstream interpolates it raw, so a
        cell holding a `<` produces markup that is no longer a table. And the
        span attributes are quoted and only written when there is a span,
        where upstream emits `<td  >` for every ordinary cell; the difference
        is thousands of stray characters in a document's worth of embedded
        tables, each one a token spent on nothing.
        """
        html = "<table>"
        if cap:
            html += f"<caption>{escape(cap)}</caption>"
        for i in range(len(tbl)):
            row = "<tr>"
            txts = []
            tag = "th" if i in hdset else "td"
            for j, arr in enumerate(tbl[i]):
                if arr is None:
                    continue
                if not arr:
                ...
                if arr[0].get("colspan"):
                    spans += f' colspan="{int(arr[0]["colspan"])}"'
                if arr[0].get("rowspan"):
                    spans += f' rowspan="{int(arr[0]["rowspan"])}"'
                row += f"<{tag}{spans}>{escape(txt)}</{tag}>"
    ...
    def __desc_table(cap, hdr_rowno, tbl, is_english):
        ...
        logger.debug("deepdoc: table {}x{}, header row {}", rowno, clmno, hdr_rowno)
        row_txt = []
        for i in range(rowno):
            if i in hdr_rowno:
                continue

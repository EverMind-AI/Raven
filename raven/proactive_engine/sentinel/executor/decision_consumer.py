...
class DecisionConsumer:
    ...
    async def _consume(
        ...
        if decision is None:
            logger.warning(
                "DecisionConsumer: router consumed but pending decision {} "
                "is no longer live; treating as no-op",
        ...
        if result.confirm_intent == "confirm":
            if result.option is None:
                logger.warning(
                    "DecisionConsumer: confirm result missing option for "
                    "decision {}; cannot execute (mark cancelled)",
                    decision.decision_id,
                )
                return await self._handle_cancel(decision, req, now_ms)
            return await self._handle_confirmed_execute(
                decision=decision, option=result.option,
                inbound=req, route_result=result, now_ms=now_ms,
            )
    ...
    def _lookup_decision(
        self, decision_id: str | None, now_ms: int
    ...
    async def _handle_skip(
        ...
        ok = self.pending_store.mark_consumed(
            decision.decision_id, picked_option_id=None,
            consumed_at_ms=now_ms, require_pending=True,
        )
        if not ok:
            logger.info(
                "DecisionConsumer: skip no-op for {} "
                "(state shifted to awaiting_confirm or consumed)",
                decision.decision_id,
            )
            return MenuReply(
        ...
        if self.feedback is not None:
            try:
                self.feedback.record_dismissed(
                    decision.decision_id, reason="user_skip_via_menu",
                )
            except Exception as exc:
    ...
    async def _handle_pick(
        ...
        *,
        ...
        self.pending_store.mark_consumed(
            decision.decision_id, picked_option_id=option.id,
        ...
        if self.feedback is not None:
            try:
                self.feedback.record_accepted(
                    decision.decision_id,
                    context=(
                        f"option={option.id} kind={option.exec_kind} "
                        f"method={route_result.raw_match_method}"
                    ),
                )
            except Exception as exc:
    ...
    async def _handle_pick_with_confirm(
        ...
        *,
        ...
        if outcome == PendingDecisionStore.AWAIT_OK:
            ...
            logger.info(
                "DecisionConsumer: decision {} already consumed; "
                "skipping confirm/execute",
                decision.decision_id,
            )
            return None
            ...
            logger.info(
                "DecisionConsumer: decision {} already in awaiting_confirm "
                "(idempotent re-prompt)", decision.decision_id,
            )
        ...
        if self.feedback is not None:
            try:
                self.feedback.record_accepted(
                    ...
                    context=(
                        f"option={option.id} kind={option.exec_kind} "
                        f"confirmed_via={route_result.raw_match_method}"
                    ),
                )
            except Exception as exc:
                logger.warning("FeedbackTracker.record_accepted: {}", exc)
    ...
    async def _handle_cancel(
        ...
        cancelled = self.pending_store.cancel_confirm(
            decision.decision_id, cancelled_at_ms=now_ms,
        )
        if not cancelled:
            logger.info(
                "DecisionConsumer: cancel_confirm no-op for {} "
                "(state already shifted)",
                decision.decision_id,
            )
            return MenuReply(
                channel=msg.source.channel,
...
def _render_user_facing(
    result: ActionExecutionResult, option: TaskOption
) -> str:

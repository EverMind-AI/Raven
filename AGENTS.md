...
- The application source trees (`bridge/`, `ui-tui/`, `ui-webui/`) are exempt from that extension list: a product frontend carries its own entry HTML and icon SVGs as source, not as report assets. Everything else in this section, including the 1 MiB limit, still applies to them.

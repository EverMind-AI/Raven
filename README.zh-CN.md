...
**数字人是它搭出来的第一个东西。** 说清楚你想要什么样的助手，Raven 就为你搭一个：一个跟你对话的主角色，几个从你已有的 agent 里挑出来的专职下属。你提的要求不会停在一段提示词里，而是整个落进这个助手的 **harness**——成为它的分工、它能用的工具、和它动手前必须过的检查。它会根据你的反馈不断改这个助手，直到它把活干好。

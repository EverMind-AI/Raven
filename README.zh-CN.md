...
**Raven 从底层就是为这件事造的。** 它的 agent loop 拆成四个解耦的策略模块——**Memory**、**Planning**、**Capability**、**Action**——因此一个 agent 被交代了什么、什么算做完、能用哪些工具、动手前要过哪几关，全都可以**在它运行的过程中**改写。改的东西只属于这个 agent 自己：一条设置，或者一小段专为它写的判断代码。同一段对话的下一轮就跑在新的上面，不用重启任何东西。

**数字人是它搭出来的第一个东西。** 说清楚你想要什么样的助手，Raven 就为你搭一个：一个跟你对话的主角色，几个从你已有的 agent 里挑出来的专职下属。你提的要求不会停在一段提示词里，而是整个落进这个助手的 **harness**——成为它的分工、它能用的工具、和它动手前必须过的检查。它会根据你的反馈不断改这个助手，直到它把活干好——之后你只要把它要的那几项给它，它就能跑起来。

> 说一次你要什么。Raven 把助手搭出来，在你使用的过程中持续改进它，之后一句话就能再让它干活。

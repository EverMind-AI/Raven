...
**Curator 改写的不止是提示词。** 它所使用的工具与外部服务、所遵循的技能与流程、在每个环节上的判断，都可以被替换。改写按轮次进行：你使用它完成工作，指出不足，它据此改写后再来一轮，直到你认可、它判断已无可改之处，或轮次预算用尽。质量由两点保障：装配前必须通过校验，未通过则返工；常规轮次的反馈在交给它之前会剥掉参考答案，避免它直接抄入。Curator 目前仍是实验性的：它随仓库提供，不包含在安装包中。

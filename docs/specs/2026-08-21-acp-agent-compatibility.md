...
| Scope of a model change | **The calling session** -- said in the option's own description | This surface sends the session id, and a model change scoped to a session is now what raven stores; a connection with two sessions open no longer changes both. The protocol's shape says otherwise, so the caveat goes in `description`, which the schema declares as text for the client to display -- a person reads it where they make the choice. |

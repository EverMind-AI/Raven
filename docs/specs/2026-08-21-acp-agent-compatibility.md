...
| `session/new` `mcpServers` | **Supported, stdio only** | Each stanza is connected through a registry that session owns, and its tools are made visible only while a turn of that session runs -- a task-local overlay in `ToolRegistry`, entered where the turn's task begins, so the next session on the same process cannot reach them and two sessions' concurrent turns each see their own. Reclaimed on `session/close`, `session/delete`, and the connection's teardown. An http or sse stanza is refused (`-32602`): `mcpCapabilities` declares neither, and a dispatcher hands a sub-agent one stdio stanza pointing at a host endpoint whatever the upstream transport is, so the definition and its credentials never enter this process. An empty array is the normal value. |
...
| `session/resume` | **Supported** | Reopens a stored session without replaying the transcript: the session becomes promptable again and the client keeps the history it already has. The schema says exactly this -- "resumes an existing session without returning previous messages (unlike `session/load`)". Same refusals as `session/load` (unknown id is `-32002`, cwd is validated and bound from the client). Declared because `sessionCapabilities.resume` is what a raven acting as this agent's client reads to report the row resumable, which is what makes an `instance` handle continue a conversation rather than start a fresh one. |
| `session/close` | **Supported** | Drops the session from this connection: any turn still running is cancelled (the spec says to treat it as `session/cancel`), the stream is unbound, and a later prompt answers `-32002` like any unknown session. The stored conversation is left in place -- closing is "I am done with this stream". Unknown id is `-32002`. |
| `session/delete` | **Supported** | Removes the stored session through the engine's session manager, and drops it from this connection if held. Existence is judged before the delete (a session minted and never prompted lives in the cache alone, so "file deleted" is not the right test), and an unknown id is `-32002` rather than a silent ack. |
...
| `session_info_update` | **Supported** | Sent after a turn when the session's auto title changed; only once per title, and nothing is emitted for a session whose title never changes. `updatedAt` is not sent -- the field is optional and raven has no last-activity stamp at the emit site. |
...
| `tool_call_update` status | **Supported** | `completed` or `failed`, from the emit site's verdict: the loop derives it from the registry's own failure convention (the model-facing text of a failed tool starts with `Error`), which is the same rule the retry-hint logic uses. A tool that reports a friendly `Error: ...` message is therefore classified as failed here too -- breaking the convention mislabels the transcript row as well. |
...
| `available_commands_update` | **Supported** | A deliberate white-list of six user-facing actions (`deep-research`, `playbook`, `cron`, `sentinel`, `sessions`, `tracing`), announced once per session on `session/new`, `session/load` and `session/resume`. Not a reflection of the Typer app: the CLI surface includes verbs whose meaning is internal to a terminal, and reflecting it would put those in a command panel and drift with every new verb. No per-turn updates -- the list is static for the life of a connection. |
...
| A sub-agent's output | **Tagged, not suppressed** | A delegated agent's turn runs on its own lane but is emitted onto the session's subscription. Untagged it would render as the main agent's words; suppressed, a turn would look idle while a sub-agent talks for a minute. It carries `_meta["raven.target"]`. |
...
| Model list length | **Capped at 40 per account, and the option says so** | A client renders the options it is given, and how it renders them is not ours to decide -- there is no folding to do on its behalf and the schema sets no limit -- so the only lever on this wire is how many are sent. An account can serve two hundred. What was wrong was the silence: a menu of forty looks complete, so the rest were models nobody could see the absence of. The option's `description` now names each shortened account and its real total, the way the replay announces its own truncation. Raven's own surfaces do not read this list at all -- they draw the live catalogue, uncut. |
...
| Scope of a model change | **The calling session** -- said in the option's own description, which `config_options.py` keeps in step | This surface sends the session id, and `_set_model` takes its session-scoped branch before anything writes `agents.defaults`, so a connection with two sessions open no longer changes both. The wire value carries the provider slug in front, and `set_model` sends it as its own field: `config.set model` requires the provider, and two groups can otherwise offer the same id on different accounts. The protocol's shape says otherwise, so the caveat goes in `description`, which the schema declares as text for the client to display -- a person reads it where they make the choice. |
| A model change during a turn | **Accepted; takes effect on the session's next turn** | The running turn holds the binding it entered with, so nothing has to be swapped under it and nothing is refused. `-32009` is gone with the guard that raised it. A refusal the runtime does still make -- a value it will not write -- travels out as `-32011` rather than a flattened internal error, because a client can act on it. |
...
| An `mcpServers` `env` pair | Accepted and never echoed. It reaches the server's subprocess environment, the same road the agent's own MCP config takes, and no frame carries it back -- the acceptance log names the server and a tool count. A bridged stanza carries no `env` at all. |

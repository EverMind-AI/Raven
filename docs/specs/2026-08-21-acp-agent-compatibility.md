...
| `stopReason` | `end_turn`, `cancelled`, `refusal` | `max_tokens` and `max_turn_requests` have no source in Raven and are never sent. `refusal` comes from the runtime's own `action_blocked` notice. |
...
| **A sub-agent's commands** | **Do not ask** -- known gap | A sub-agent builds its own `ExecTool` with its own policy, and only the main loop's tool is given an approval responder. So a delegated agent's `git push` runs unannounced even though the main agent's asks. Closing it means routing a lane's conversation id into a task that outlives its turn, and registering the families there without that would make such commands *refused* rather than asked -- a different product decision. |

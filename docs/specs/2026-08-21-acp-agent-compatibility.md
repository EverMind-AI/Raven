...
| A model change during a turn | **Refused, `-32009`** | `config.set` guards on whether the session is busy rather than swapping the provider under a running request. The code travels out intact, because a client can act on it and a flattened internal error leaves a person retrying something that will keep failing. |

...
| **T1.5** | A21 | yes | Two saves within a second, with the everos pid sampled every 200ms throughout. Exactly **two** pids may appear over the episode -- the original and one replacement -- and the survivor must have booted with the **second** save's model. A third pid is a second chain; the first save's model surviving is the ordering lost. | Two chains run concurrently (a third pid appears), or the first chain's value wins | effect: the pid samples; the survivor's `llm_client_built` line |
...
| **T6.5** | A18 | yes | Two halves, both on the page. (1) The card renders the stored provider without reverse-looking-up a base url -- checked with a provider whose address matches no `apiBase`, which the old code rendered blank. (2) The "key set" indicator follows the **provider's** credential: remove that provider's key and the indicator drops, with no edit to the role. | The page still guesses the vendor from the address; or the key indicator still reflects a field raven no longer stores | visible: the rendered provider name; the indicator before and after removing the provider's key |

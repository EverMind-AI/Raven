...
it reads as one no result came back for (it may have been cancelled mid-flight or never reached;
an acp `tool_call` frame means initiated, and the transcript cannot tell the two apart), and a
folded step counts those calls beside its failures. A step opens on a
